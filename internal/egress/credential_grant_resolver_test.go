...
func TestCredentialGrantResolver_WithinLoaderFallthrough(t *testing.T) {
	t.Parallel()
	...
	// Non-principal subject can't use the provider-token grant but should
	// fall through to the secret grant within the same loader.
	mat, err := r.ResolveCredential(context.Background(), Subject{Kind: SubjectSystem, ID: "sys"}, Target{Provider: "vendorx", Host: "api.test"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if mat.Authorization != "secret-val" {
		t.Fatalf("expected within-loader fallthrough to secret grant, got %q", mat.Authorization)
	}
}
