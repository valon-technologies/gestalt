...
type providerManifestWire struct {
	...
	Expose            *providerManifestExposeWire                            `json:"expose,omitempty" yaml:"expose,omitempty"`
}
...
type providerManifestExposeWire struct {
...
func validateManifestWire(wire *manifestWire) error {
	if wire == nil {
		return fmt.Errorf("manifest is required")
	}
	if wire.Provider == nil && wire.WebUI == nil {
		return fmt.Errorf("manifest must define provider or webui")
	}
	if wire.Provider != nil && wire.WebUI != nil {
		return fmt.Errorf("webui manifest cannot be combined with provider")
	}
	...
	apiSurfaceCount := 0
	if wire.Provider.Surfaces.REST != nil {
		apiSurfaceCount++
	}
	if wire.Provider.Surfaces.OpenAPI != nil {
		apiSurfaceCount++
	}
	if wire.Provider.Surfaces.GraphQL != nil {
		apiSurfaceCount++
	}
	if apiSurfaceCount > 1 {
		return fmt.Errorf("provider can define only one of surfaces.rest, surfaces.openapi, or surfaces.graphql")
	}
	...
	for name, conn := range wire.Provider.Connections {
		...
		if name != "default" {
			if len(conn.Params) > 0 {
				return fmt.Errorf("provider.connections.%s.params are only supported on provider.connections.default", name)
			}
			if conn.Discovery != nil {
				return fmt.Errorf("provider.connections.%s.discovery is only supported on provider.connections.default", name)
			}
		}
	}
...
func wireManifestToInternal(wire *manifestWire) *pluginmanifestv1.Manifest {
	...
	if wire.Provider != nil {
		...
		if wire.Provider.Expose != nil {
			manifest.Provider.MCP = wire.Provider.Expose.MCP
		}
...
func internalManifestToWire(manifest *pluginmanifestv1.Manifest) *manifestWire {
	...
	if manifest.Provider.MCP {
		provider.Expose = &providerManifestExposeWire{MCP: true}
	}

	if manifest.Provider.Auth != nil || len(manifest.Provider.ConnectionParams) > 0 || manifest.Provider.PostConnectDiscovery != nil {
