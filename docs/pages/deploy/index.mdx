...
| `server/Dockerfile` | Single-container server image with `gestaltd` as the entrypoint. |
| `server/deploy/helm/gestalt` | Helm chart for Kubernetes. |

## Docker

`server/Dockerfile` builds:
...
If your config depends on remote REST or GraphQL upstreams, or packaged plugins, run `gestaltd init` ahead of time and either:

- bake the prepared state into a derived image
- or prepare it in a separate init or release step and point `serve --locked` at that config

To build and run from source:

```sh
docker build -f server/Dockerfile -t gestaltd .
docker run -p 8080:8080 \
  -v ./your-config.yaml:/etc/gestalt/config.yaml:ro \
  -v gestalt-data:/data \
  gestaltd
```

For a starting-point config and published-image examples, see `examples/deploy/`.
