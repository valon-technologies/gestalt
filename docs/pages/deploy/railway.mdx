import { Callout } from 'nextra/components'

# Deploy to Railway

## Before you begin

- A [Railway](https://railway.app) account.
- Your Toolshed repository on GitHub.

## Deploy from GitHub

1. Create a new project in the Railway dashboard.
2. Select **Deploy from GitHub repo** and connect your repository.
3. Set **Root Directory** to `toolshed/`.
4. Railway detects the Dockerfile and begins the first build.

<Callout>
  The app will crash without a database and environment variables. Cancel the initial deploy or let it fail, then continue below.
</Callout>

## Add a database

1. In your Railway project, click **New** then **Database** then **PostgreSQL**.
2. Link the variable: in the Toolshed service's **Variables** tab, add `DATABASE_URL` set to `${{Postgres.DATABASE_URL}}`.

## Set environment variables

Add these in the Toolshed service's **Variables** tab:

| Variable | Value |
|---|---|
| `TOOLSHED_BASE_URL` | `https://<domain>.up.railway.app` |
| `TOOLSHED_ENCRYPTION_KEY` | Output of `openssl rand -base64 32` |
| `TOOLSHED_SESSION_SECRET` | Output of `openssl rand -base64 32` |
| `GOOGLE_CLIENT_ID` | Your Google OAuth client ID |
| `GOOGLE_CLIENT_SECRET` | Your Google OAuth client secret |
| `DATABASE_URL` | `${{Postgres.DATABASE_URL}}` |

## Configure

Commit a config file at `toolshed/config.railway.yaml`:

```yaml
server:
  port: 8080
  base_url: "${TOOLSHED_BASE_URL}"
  encryption_key: "${TOOLSHED_ENCRYPTION_KEY}"

auth:
  provider: google
  config:
    client_id: "${GOOGLE_CLIENT_ID}"
    client_secret: "${GOOGLE_CLIENT_SECRET}"
    session_secret: "${TOOLSHED_SESSION_SECRET}"

datastore:
  provider: postgres
  config:
    dsn: "${DATABASE_URL}"
```

Override the container start command in Railway's service settings:

```
/toolshed --config /build/toolshed/config.railway.yaml
```

<Callout>
  The path depends on how Railway copies your repo into the build context. Check the build logs if `/build/toolshed/` does not resolve.
</Callout>

## Verify

Find the public URL in **Settings** then **Networking** then **Public Networking**.

```sh
curl -s https://<domain>.up.railway.app/health
```

## Custom domain

1. Go to **Networking** then **Custom Domain** in service settings.
2. Follow the DNS instructions Railway provides.
3. Update `TOOLSHED_BASE_URL` to the new domain.
