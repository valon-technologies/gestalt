...
Commit a config file at `gestalt/config.railway.yaml`:
...
```
/gestalt --config /build/gestalt/config.railway.yaml
```

<Callout>
  The path depends on how Railway copies your repo into the build context. Check the build logs if `/build/gestalt/` does not resolve.
</Callout>
