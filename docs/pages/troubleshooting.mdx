...
**Fix**: Ensure each integration has at least one upstream with a valid `type` and `url`. For deterministic production startup, run `gestaltd prepare --config your-config.yaml` during build, then `gestaltd serve --config your-config.yaml` at runtime.
