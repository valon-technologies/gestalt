...
Toolshed resolves providers at startup. If neither `openapi` nor `provider` is set on an integration entry, and no matching file exists in `provider_dirs`, the server logs an error and skips the integration.
...
Toolshed derives callback URLs from `server.base_url`. If it is unset or points to the wrong host, the identity provider rejects the redirect.
...
**Fix**: Run `tshed integrations connect <name>` or use the web UI.
...
- If using the CLI, check that `TOOLSHED_API_KEY` is set correctly and is not overridden by an expired session token.

## Web UI does not talk to the local API

The web UI needs to know where the API server lives.

**Fix**: Use `toolshed dev` which starts both and wires them together automatically. If running separately, set `TOOLSHED_API_URL` to point to the API server.

## Config validation fails

Run `toolshed config check -config your-config.yaml` to see exactly which field is invalid. Common causes:
