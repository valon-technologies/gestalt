...
**Fix**: Use `gestalt-server dev` which starts both and wires them together automatically. If running separately, set `GESTALT_API_URL` to point to the API server.

## Config validation fails

Run `gestalt-server config check -config your-config.yaml` to see exactly which field is invalid. Common causes:
