...
**Fix**: Run `gstl integrations connect <name>` or use the web UI.
...
**Fix**: Use `gestalt dev` which starts both and wires them together automatically. If running separately, set `GESTALT_API_URL` to point to the API server.

## Config validation fails

Run `gestalt config check -config your-config.yaml` to see exactly which field is invalid. Common causes:
