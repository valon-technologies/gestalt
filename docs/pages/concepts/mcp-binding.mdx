# MCP Binding

When `mcp.enabled` is set in the config, Toolshed mounts a Streamable HTTP MCP endpoint at `/mcp`. This gives AI assistants like Claude direct access to your integration operations.

## What MCP is

The [Model Context Protocol](https://modelcontextprotocol.io) (MCP) is an open standard for connecting AI assistants to external tools. An MCP server advertises a list of tools and handles tool calls. Toolshed's MCP binding maps each allowed integration operation to an MCP tool.

## Tool names

Each tool name is built from the provider name and operation ID:

```
{provider}_{operation}
```

For example, `search_issues` on the `linear` provider becomes `linear_search_issues`.

Set `tool_name_prefix` in the `mcp` config block to prepend a string to every tool name. A prefix of `ts_` produces `ts_linear_search_issues`.

## Auth

MCP requests use the same auth middleware as the REST API. Clients authenticate with a Bearer token in the `Authorization` header.

## Relationship to the HTTP API

The MCP binding is an alternative interface, not a replacement. It shares:

- The same providers and integration definitions from config.
- The same datastore for users, tokens, and credentials.
- The same invoker: credential resolution, automatic token refresh, and upstream execution.
- The same allow-list enforcement on operations.

The `mcp.providers` filter lets you expose a subset of integrations to MCP clients without affecting the HTTP API.

## Tool metadata

Toolshed populates MCP tool annotations automatically:

| HTTP method | Annotation |
|---|---|
| GET, HEAD | Read-only |
| PUT | Idempotent |
| DELETE | Destructive |

Input schemas from OpenAPI specs are passed through as JSON Schema, giving MCP clients type information for parameters. Providers that implement the catalog interface can supply richer metadata for their operations.

## Limitations

Adding or removing integrations requires restarting the server.
