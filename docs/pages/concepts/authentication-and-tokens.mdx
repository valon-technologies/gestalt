...
The CLI uses a dedicated non-expiring API token for stored credentials after `gestalt auth login`. That avoids browser-style 24-hour session expiry for CLI usage while keeping the token revocable.
