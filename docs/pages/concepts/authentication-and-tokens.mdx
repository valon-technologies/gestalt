...
Gestalt authenticates users through a configurable auth provider. The provider handles the login flow and issues a session token.
...
Integration tokens are the credentials Gestalt stores for a user's connection to an upstream API. When a user completes an OAuth flow (or pastes manual credentials), Gestalt encrypts the tokens and persists them in the datastore.
