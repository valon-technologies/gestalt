...
The host and plugin negotiate a protocol version during startup. The plugin advertises its supported range via `min_protocol_version` and `max_protocol_version` in `ProviderMetadata`. The host checks that its own version falls within that range, then sends the chosen version in `StartProviderRequest.protocol_version`. The plugin echoes it back in `StartProviderResponse.protocol_version` to confirm.

If the versions are incompatible, the host refuses to start the plugin. Plugins that omit the version fields (both zero) are treated as compatible for backward compatibility.

The current protocol version is `1`, defined as `CurrentProtocolVersion` in `sdk/pluginapi/v1`.
...
Called once at startup immediately after the gRPC connection is established. Returns the provider's identity, connection mode, supported auth types, connection parameter definitions, protocol version range, and feature flags.
...
| `config_schema_json` | `string` | Optional JSON Schema describing the `plugin.config` block. Used for startup validation for `plugin.command`, and for static validation when a packaged plugin is referenced via `plugin.ref`. |
...
Called once after `GetMetadata`, before `ListOperations`. Delivers the integration name, the `plugin.config` values from YAML, the plugin mode (`REPLACE` or `OVERLAY`), and the negotiated protocol version.

The plugin should use this call to apply config and confirm the protocol version. Return the same `protocol_version` that was sent in the request. If the plugin returns `UNIMPLEMENTED`, the host treats it as a legacy plugin and continues without error.

Key fields in `StartProviderRequest`:

| Field | Type | Notes |
|---|---|---|
| `name` | `string` | The integration name from `gestalt.yaml`. |
| `config` | `google.protobuf.Struct` | The `plugin.config` values from YAML. |
| `mode` | `PluginMode` | `REPLACE` (full provider) or `OVERLAY` (only operations, host handles auth/session). |
| `protocol_version` | `int32` | The protocol version the host selected. |

**`ListOperations(google.protobuf.Empty) → ListOperationsResponse`**

Called once at startup, right after `StartProvider`. Returns the full list of operations this provider exposes. Each `Operation` has a `name`, `description`, HTTP `method`, and a list of `Parameter` definitions.
...
An overlay plugin uses `PLUGIN_MODE_OVERLAY` mode. It only owns its operations and `Execute` logic; authentication, session management, and connection parameters are handled by the host using the declarative config for that integration. This is useful when you want to add custom operations to a provider that already has working auth configured via `upstreams`.

Overlay plugins must still implement `GetMetadata`, `StartProvider`, `ListOperations`, and `Execute`. The host ignores the overlay plugin's `auth_types` and `connection_mode` in metadata, using the declarative config values instead.

## Provider boot sequence

The full startup sequence for a provider plugin:

1. Host spawns the plugin process with `GESTALT_PLUGIN_SOCKET`.
2. Plugin binds a gRPC server on the socket.
3. Host connects and calls `GetMetadata` to learn the plugin's identity and protocol version range.
4. Host calls `checkProtocolCompatibility` to verify its version falls within `[min_protocol_version, max_protocol_version]`.
5. Host calls `StartProvider` with the integration name, config, mode, and protocol version.
6. Host calls `ListOperations` to discover available operations.
7. The plugin is now live and ready to receive `Execute` calls.
...
5. Gestalt polls the socket every 25ms. Once the connection succeeds, it runs the boot sequence: `GetMetadata` → `checkProtocolCompatibility` → `StartProvider` → `ListOperations` (for providers) or `Start` (for runtimes).
...
When `plugin` is set, `upstreams` cannot be used on the same integration (unless using overlay mode). The `plugin.config` map is delivered to the plugin via `StartProvider`.
...
`prepare` resolves the ref to a manifest and platform-specific executable path, validates `plugin.config` against the packaged schema, and records the result in `gestalt.lock.json`.
...
| `config` | No | Arbitrary key-value map delivered to the plugin at startup via `StartProvider` or `StartRuntime`. |
...
A complete working example lives in `examples/plugins/provider-go/`. Here is the minimal structure for a provider plugin using `sdk/pluginsdk`:
...
func (p *myProvider) Description() string                      { return "A custom provider." }
func (p *myProvider) ConnectionMode() pluginsdk.ConnectionMode { return pluginsdk.ConnectionModeNone }
func (p *myProvider) ListOperations() []pluginsdk.Operation    { return nil }

func (p *myProvider) Execute(ctx context.Context, op string, params map[string]any, token string) (*pluginsdk.OperationResult, error) {
    return &pluginsdk.OperationResult{Status: 200, Body: `{}`}, nil
}
