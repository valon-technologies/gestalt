...
|---|---|---|
| Structured response check | `structured_response_check` | Declarative response validation using body matching and error extraction. Preferred approach. |
| Response check | `response_check` | Validate a successful-looking response body. **Deprecated** — use `structured_response_check` instead. |
| Token parser | `token_parser` | Turn the stored token into auth headers. **Deprecated** — use `auth_mapping` or `auth_header` instead. |
| Request mutator | `request_mutator` | Rewrite or augment outgoing requests before execution. |
| Auth response hook | `auth.response_hook` | Validate token exchange responses from OAuth providers. **Deprecated** — use `auth.structured_response_check` instead. |

### Structured response check

The `structured_response_check` field replaces string-based `response_check` and `auth.response_hook` references with a declarative definition. It supports two modes that can be used together or independently:
...
When both `structured_response_check` and `response_check` are set, the structured check takes precedence and the string hook is ignored.

### Legacy hooks

String-based hook references (`response_check`, `token_parser`, `auth.response_hook`) are registered by name in Go code and referenced by that name in config. They continue to work but emit deprecation warnings at startup.
