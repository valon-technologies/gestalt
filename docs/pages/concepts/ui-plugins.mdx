...
For production, use `gestaltd init` to prepare lock state:
...
The prepared state includes the resolved UI plugin assets and lockfile. `--locked` prevents automatic resolution at startup.
