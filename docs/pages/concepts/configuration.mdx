...
4. `~/.gestalt/config.yaml` (legacy path)
...
That produces `gestalt.lock.json` next to the config file plus prepared artifacts under `.gestaltd/` in the configured artifacts directory.

See [Prepared State](/concepts/prepared-state) for how lockfiles, `server.artifacts_dir`, and locked startup fit together.
