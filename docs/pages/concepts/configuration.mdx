...
**relative paths**: `provider`, `provider_dirs`, and `icon_file` paths resolve relative to the config file's directory, not the current shell working directory.

**provider packaging**: `gestaltd bundle --config ... --output-dir ...` is the recommended production packaging flow. It writes a generated `config.yaml` plus local provider artifacts so runtime startup is deterministic. `gestaltd compile-providers` remains available as the low-level artifact-only command.
