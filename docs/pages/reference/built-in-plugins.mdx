...
No named providers are currently built into the binary. BigQuery, Jira, and Slack are available as external plugins under `plugins/`.

## Scope

This page lists only components wired into `cmd/gestaltd`. Test fixtures, examples, and external plugins (`plugins/`) ship in the repository but are not compiled into the binary.
