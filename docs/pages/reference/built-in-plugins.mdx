...
> **Deprecation notice:** String-based hook references (`response_check`, `auth.response_hook`, `token_parser`) are deprecated. Use the declarative alternatives instead: `structured_response_check` for response validation, `auth.structured_response_check` for auth response validation, and `auth_mapping` or `auth_header` for token parsing. See the [integrations concept page](/concepts/integrations) for details.
