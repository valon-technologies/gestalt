...
directly and renders a lightweight operator-facing view without introducing a
second admin JSON API.

The admin UI intentionally stays basic:

- cumulative counters and histograms from `/metrics`
- provider and operation breakdowns derived from metric labels
- recent throughput computed from browser-side scrape deltas
