...
`/metrics` is served behind the same auth middleware as the rest of the
protected server surface. The embedded admin UI at `/admin` reads `/metrics`
directly in the browser and derives a few lightweight ECharts views without
