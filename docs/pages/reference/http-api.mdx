...
| `POST` | `/api/v1/egress/credential-grants` | Create an egress credential grant. |
| `GET` | `/api/v1/egress/credential-grants` | List egress credential grants. |
| `DELETE` | `/api/v1/egress/credential-grants/{id}` | Delete an egress credential grant. |

The same `server.admin_emails` gate also controls global-scope egress client creation, listing, deletion, and token management through the regular `/api/v1/egress-clients` routes.
