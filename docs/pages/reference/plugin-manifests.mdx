...
Gestalt plugins and plugin release archives are described by a manifest file named one of:
...
- Pure executable source manifests used with `from.source.path` or `gestaltd plugin release` may omit `artifacts` and `entrypoints.provider`.
