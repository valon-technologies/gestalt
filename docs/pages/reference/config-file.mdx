...
| `provider_dirs` | Directories searched for provider artifacts at startup. Relative paths resolve from the config file's directory. |
...
| `provider` | (`rest` and `graphql`) Path to a local provider definition file. Used instead of live spec loading or introspection. Relative paths resolve from the config file's directory. |
...
If a `rest` or `graphql` upstream has neither `url` nor `provider`, Gestalt falls back to searching `provider_dirs` for a matching provider artifact (`.yaml`, `.yml`, or `.json`) by integration name.

For deployment bundles generated by `gestaltd bundle`, Gestalt writes explicit relative `provider` paths into the generated config and no longer needs `provider_dirs`.
