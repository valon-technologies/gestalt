...
datastore:
  ...
  config:
    dsn: postgres://user:pass@host:5432/toolshed
```

| Provider | Config | Notes |
|---|---|---|
| `sqlite` | `path: ./toolshed.db` | Default. Single-instance only. |
| `postgres` | `dsn: postgres://...` | Recommended for production. |
| `mysql` | `dsn: user:pass@tcp(host:3306)/toolshed` | Alternative SQL backend. |
| `dynamodb` | `table`, `region`, optional `endpoint` | AWS DynamoDB. |
| `mongodb` | `uri`, optional `database` (default: `toolshed`) | MongoDB. |
...
auth_profiles:
  slack_shared:
    ...
    redirect_url: https://toolshed.example.com/api/v1/auth/callback
    auth:
