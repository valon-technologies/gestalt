...
| `source.version` | Required with `source.ref`. Invalid with `source.path` or `package`. |
| `command` | Execute a local binary directly. Requires `source.path`. |
...
- Exactly one provider loading mode: local `from.source.path`, managed `from.source.ref`, or `from.package`
- `from.source.version` is required with `from.source.ref` and invalid with `from.source.path` or `from.package`
