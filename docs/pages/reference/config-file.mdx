...
| `postgres` | `dsn`, `version` | Recommended production datastore. |
| `mysql` | `dsn`, `version` | SQL-backed production option. |
| `dynamodb` | `table`, `region`, `endpoint` | `table` defaults to `gestalt`. |
| `mongodb` | `uri`, `database`, `version` | `database` defaults to `gestalt`. |
| `oracle` | `dsn`, `version` | Enterprise SQL option. |
| `firestore` | `project_id`, `database` | Firestore-backed deployment. |
| `sqlserver` | `dsn`, `version` | SQL Server-backed deployment. |

For versioned datastores, `version` defaults to `auto`, which detects the
connected backend and validates it against the supported release series.

- `postgres`: `15`, `16`, `17`, `18`
- `mysql`: `8.0`, `8.4`, `9.6`
- `mongodb`: `7`, `8`
- `oracle`: `19c`, `26ai`
- `sqlserver`: `2017`, `2019`, `2022`, `azure`
...
`sqlserver` datastore targets SQL Server, Azure SQL Database, and Azure SQL
Managed Instance; Synapse and Fabric are not supported.
...
| `metrics.dashboard.enabled` | No | `true` | Enables the built-in operation dashboard data source. |

Outputs structured logs to standard output. Traces remain disabled, but metrics
are collected locally for the default web UI and for the Prometheus-compatible
`/metrics` endpoint. Built-in dashboard metrics use a fixed one-hour window with
one-minute buckets. This is the default provider when `telemetry` is omitted.
...
Datadog Agent) while still keeping local `/metrics` and the built-in dashboard
available by default. The built-in dashboard remains fixed to a one-hour window
with one-minute buckets.
...
This also disables the built-in dashboard metrics feed and `/metrics`.
...
When `ui` is omitted, `gestaltd` serves the embedded frontend. When `ui.plugin` is configured, the plugin's assets are served instead. See [UI Plugins](/reference/ui-plugins) for the full workflow.
