...
It writes lock state (`gestalt.lock.json`) and extracts prepared artifacts under `.gestaltd/`. See [Prepared State](/concepts/prepared-state) for the deployment model around `init` and `serve --locked`.
