...
gestaltd prepare --config config.yaml
...
| Command | Purpose |
|---|---|
| `gestaltd dev --config config.yaml` | Prepare REST/GraphQL providers if needed, then start the server from the source config. |
| `gestaltd prepare --config config.yaml` | Write `gestalt.lock.json` plus `.gestalt/providers/*.json` next to the source config. |
...
`gestaltd prepare` writes a lockfile and hidden provider cache next to the source config. `gestaltd serve` reads the same source config and uses the prepared artifacts, so production startup no longer depends on live spec downloads or GraphQL introspection.
