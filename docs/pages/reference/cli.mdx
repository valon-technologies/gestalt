...
gestaltd bundle --config config.yaml --output-dir ./dist/gestalt
gestaltd compile-providers --config config.yaml --output-dir ./providers
```
...
| `gestaltd bundle --config config.yaml --output-dir ./dist/gestalt` | Write a deployable config bundle with local provider artifacts. |
| `gestaltd compile-providers --config config.yaml --output-dir ./providers` | Resolve REST/GraphQL upstreams into deterministic local provider artifacts. |
...
```sh
gestaltd bundle --config config.yaml --output-dir ./dist/gestalt
gestaltd --config ./dist/gestalt/config.yaml
```

Writes a generated `config.yaml` plus `providers/*.json` artifacts. REST and GraphQL upstreams in the bundled config are rewritten to use local `provider` files, so production startup no longer depends on live spec downloads or GraphQL introspection. This is the recommended production packaging workflow.

### Compile provider artifacts

```sh
gestaltd compile-providers --config config.yaml --output-dir ./providers
```

Writes one JSON provider definition per REST or GraphQL integration. Integrations with only MCP upstreams are skipped. Use this low-level command when you want just the artifacts and will handle config rewriting yourself.
