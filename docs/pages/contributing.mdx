...
| Directory | Purpose |
|---|---|
| `cmd/gestalt` | Main Go binary. Registers all plugins and starts the server. |
...
| `client/cli` | Rust CLI source (`gstl`). |
| `web` | Next.js web UI. |
...
3. Register the factory in `cmd/gestalt/factories.go` so the runtime resolves it by name from config.
...
3. Register them by name in `cmd/gestalt/factories.go`.
