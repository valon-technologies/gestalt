...
| Directory | Purpose |
|---|---|
| `cmd/gestalt-server` | Main Go binary. Registers all plugins and starts the server. |
...
3. Register the factory in `cmd/gestalt-server/factories.go` so the runtime resolves it by name from config.
...
3. Register them by name in `cmd/gestalt-server/factories.go`.
...
# Validate config
gestalt-server config check -config gestalt.dev.yaml

# Run locally
gestalt-server dev -config gestalt.dev.yaml
