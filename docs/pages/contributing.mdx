...
4. Add functional tests that exercise the plugin through the same interface the runtime uses. Do not add package-level unit tests.
...
```sh
# Enforce the test policy and run the functional suite
bash scripts/check-functional-tests.sh
cd gestalt
go test ./...
...
This repository keeps functional tests only.

- Go tests live under `test/functional`.
- CLI tests live in `client/cli/tests/functional.rs` and run the compiled binary.
- Web tests live in `web/e2e`.
- Inline Rust `#[cfg(test)]` modules and package-level/unit-style Go tests are not allowed.
