...
| Directory | Purpose |
|---|---|
| `cmd/toolshed` | Main Go binary. Registers all plugins and starts the server. |
...
| `client/cli` | Rust CLI source (`tshed`). |
| `web` | Next.js web UI. |
...
3. Register the factory in `cmd/toolshed/factories.go` so the runtime resolves it by name from config.
...
3. Register them by name in `cmd/toolshed/factories.go`.
...
# Validate config
toolshed config check -config toolshed.dev.yaml

# Run locally
toolshed dev -config toolshed.dev.yaml
