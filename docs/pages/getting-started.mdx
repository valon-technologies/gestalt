...
```sh
go install github.com/valon-technologies/gestalt/cmd/gestaltd@latest
```

**Docker**

The `valontechnologies/gestalt` image ships `gestaltd` and supports both `amd64` and `arm64` (Apple Silicon).

```sh
docker run -p 8080:8080 -v ./gestalt.yaml:/etc/gestalt/config.yaml valontechnologies/gestalt:latest
```
