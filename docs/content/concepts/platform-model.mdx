...
| Source or published plugin | A separate process that communicates with `gestaltd` over gRPC. See [Plugins](/plugins). |

A source or published plugin can also expose spec-backed surfaces alongside its own operations. This is referred to as a hybrid provider.
