...
<Tabs items={['Go', 'Python', 'Rust', 'TypeScript']}>
  ...
  <Tabs.Tab>
    ...
    The plugin root needs a real `Cargo.toml` package with a `[package]` section, not just a workspace manifest, and `src/lib.rs` should export the provider with `gestalt::export_provider!(...)`, `gestalt::export_auth_provider!(...)`, or `gestalt::export_indexeddb_provider!(...)`.
  </Tabs.Tab>
  <Tabs.Tab>
    Verify the module resolves from the provider root:
...
listed in `allowedHosts`. Add every upstream domain the plugin contacts to the
...
provider-load failures instead. See
[Plugin Sandboxing](/architecture/plugin-sandboxing).
