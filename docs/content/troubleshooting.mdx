...
A "provider not found" error means the app key in the request does not match any entry in `plugins`. Verify the spelling and confirm the app loaded without errors at startup.
...
allowlist in the plugin's config entry or manifest. If the deployment does not
...
When a app requires an explicit connection, it has more than one configured connection and no default. Pass `_connection` on the invocation request or use `--connection` in the client CLI to disambiguate.
