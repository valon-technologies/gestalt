...
The caller must declare the allowed call graph in `apps.<caller>.invokes`; at
runtime the client sends the current request context to the host, and the host
...
The nested call must fit inside the caller's configured `invokes` authorization relationship. Pass
