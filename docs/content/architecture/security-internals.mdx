...
Long-lived API access is provider-owned. Gestalt mints grants through RFC 8693 token exchange against the caller's session, and the identity provider persists grant metadata plus SHA-256 token hashes.
...
**Validation.** On each request, Gestalt asks the identity provider to introspect the bearer token.
