...
| --- | --- |
| `gestaltd` core | `users`, `managed_subjects`, `api_tokens`, `oauth_registrations` |
...
| `permissions_json` | string | JSON-encoded provider operation permissions. |
...
Dynamic OAuth client registrations for `mcp_oauth` connections. One client per
(authorization server, redirect URI) pair, shared by every server instance so
the authorize URL and the token exchange always present the same `client_id`
regardless of which instance serves each request.

| Field | Type | Notes |
| --- | --- | --- |
| `id` | string, PK | `auth_server_url` + `"\n"` + `redirect_uri`. |
| `client_id` | string | Dynamically registered client ID. |
| `client_secret_encrypted` | string | **Encrypted.** AES-256-GCM. Empty for public clients. |
| `expires_at` | time | Null when the registration does not expire. |
