...
Gestalt stores most persistent state through the configured [IndexedDB provider](/providers/indexeddb). At startup, gestaltd provisions three object stores and their indexes through the same `CreateObjectStore` interface that plugins use. Any backend that implements the IndexedDB contract can store this data. Migrations run automatically on startup.
...
Platform sessions (Google, OIDC, local) are not stored in the IndexedDB provider. They are HTTP-only cookies (`session_token`) with `Secure` (when HTTPS), `SameSite=Lax`, and a default 24-hour TTL (configurable via `sessionTtl` for OIDC). Sessions cannot be revoked server-side; they expire on TTL. Logout clears the client cookie.
