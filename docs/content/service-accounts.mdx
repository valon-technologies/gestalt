...
3. Grant plugin roles to the service account through
...
normal plugin authorization and credential lookup use that subject.
...
It does not create the service account, grant plugin access, or create upstream
credentials.
...
When this schedule fires, Gestalt invokes `github.issues.list` as
...
Executable plugins and agent tool calls may also use `runAs` only when the
