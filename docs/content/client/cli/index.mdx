...
| `gestalt app disconnect NAME` | Disconnect a plugin, removing stored credentials. |
...
Subject token creation accepts `--permission plugin:operation`,
`--action plugin:action`, `--scopes`, or `--permissions-file` with a raw JSON
...
`--action plugin:provider_dev.attach`.
