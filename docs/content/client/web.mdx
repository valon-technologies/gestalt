...
When `providers.auth` points to an interactive authentication provider such as OIDC, the web UI redirects to your identity provider for login. When `providers.auth` is omitted, every request is anonymous.
