...
- New custom UI bundles should check in frontend source and declare top-level
  `build.command` plus `spec.assetRoot` in the UI manifest. Gestalt verifies
  the generated `out`, `dist`, or `build` directory and serves those static
  assets after preparation.
