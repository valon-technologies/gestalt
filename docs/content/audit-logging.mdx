...
| Field | Meaning |
| --- | --- |
| `subject_id` | Canonical caller subject such as `user:<id>` or [`service_account:<id>`](/service-accounts) |
| `credential_subject_id` | Credential subject when it differs from `subject_id` |
| `run_as_subject_id` | Delegated run-as subject for the invocation, when present |
...
| `outcome` | Terminal result for operation and connection events: `success`, `rejected`, or `failed` |
| `failure_cause` | Owning boundary for a rejection or failure, such as `caller`, `connection`, `gestalt`, `provider`, `upstream`, or `unknown` |
| `failure_reason` | Bounded actionable reason shared with the corresponding metrics and trace |
| `caller_app` | App that initiated a nested invocation or workflow call |
...
| `created_by` | Authenticated subject that created the workflow resource or initiated the audited workflow operation. |
...
emit one audit record per backend for fanout event delivery.

- `workflow.definition.apply`
- `workflow.definition.delete`
- `workflow.definition.pause`
- `workflow.activation.pause`
- `workflow.run.start`
- `workflow.run.signal`
- `workflow.run.signal_or_start`
- `workflow.run.cancel`
- `workflow.event.deliver`
...
delivery from HTTP, CLI, and app SDK paths. Read-only workflow list/get
...
| Activation | `workflow_activation` | Activation ID | Empty |
| Run | `workflow_run` | Run ID when available | Empty |
| Delivered event | `workflow_event` | Empty | Event type |
...
| Workflow-manager mutations and event delivery | No dedicated semantic metric family | Yes | Workflow control-plane actions are audited with `source=workflow_manager`. |
