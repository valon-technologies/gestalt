...
| Field | Meaning |
| --- | --- |
| `caller_plugin` | App that called `WorkflowManager`, such as a Slack event provider. |
...
| `workflow_target_component` | Target subcomponent involved in a workflow target authorization denial, such as `target`, `app_target`, `agent_provider`, or `agent_tool_ref`. |
...
field is the selected workflow backend, not the app or agent target. They
...
publishing from HTTP, CLI, and app SDK paths. Read-only workflow list/get
...
key. Targeted workflow actions include `workflow_target_kind`, and app
