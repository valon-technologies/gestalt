...
When no config file exists, `gestaltd` generates one at `~/.gestaltd/config.yaml` and starts immediately with SQLite storage via the [RelationalDB](https://github.com/valon-technologies/gestalt-providers/tree/main/indexeddb/relationaldb) implementation of the [IndexedDB provider](/providers/indexeddb), the [default home app](https://github.com/valon-technologies/gestalt-providers/tree/main/app/default) (`gestalt-providers/app/default`) serving its static bundle at `/`, no authentication, and an [HTTPBin](https://github.com/valon-technologies/gestalt-providers/tree/main/app/httpbin) implementation of the [plugin provider](/providers/apps) on port 8080.
...
See what the httpbin app can do:

```sh
gestalt app invoke httpbin
```
...
Both access the same underlying operation. Anything you can do with `gestalt app invoke` you can also do with `curl` or any HTTP client.

## Connecting over MCP

Gestalt exposes all MCP-enabled apps at a single endpoint: `http://localhost:8080/mcp`. Any MCP-compatible client can connect to it.
...
Once connected, your agent can call any operation exposed by the server. The same auth model applies: when apps require credentials, users connect through the Gestalt UI or CLI before the agent can invoke those tools.
...
- [Service Accounts](/service-accounts) covers automation identities, API tokens, and `runAs`.
