...
Create a file called `.gestaltd/config.yaml` with a provider definition. This example
adds [HTTPBin](https://httpbin.org), a public API that requires no
authentication. Because the config lives under `.gestaltd/`, project-root files
such as the OpenAPI document are referenced with `../...`:
