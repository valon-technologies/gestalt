...
The HTTP API uses "integrations" in route paths (e.g. `/api/v1/integrations`) as a stable code surface. The CLI uses the `app` command, while config still uses the `apps` map. See [Config File](/reference/config-file#providersapps) for context.
...
| Method | Path | Purpose |
| --- | --- | --- |
| `GET` | `/api/v1/integrations` | List apps, aggregate status, icons, and nested connection metadata. Authentication types, instances, credential fields, and connection parameter hints are returned per item in `connections[]`. |
| `DELETE` | `/api/v1/integrations/{name}` | Disconnect a app. Use `?_connection=...` and `?_instance=...` to target one stored connection. |
| `GET` | `/api/v1/integrations/{name}/operations` | List operations for one app. Supports `_connection` and `_instance` selectors. |
| `GET` or `POST` | `/api/v1/{integration}/{operation}` | Invoke a app operation. Supports `_connection` and `_instance`. |

`GET /api/v1/integrations` returns one object per app. Use top-level `status`,
