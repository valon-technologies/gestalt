...
| `GET`, `POST`, `PUT`, `PATCH`, or `DELETE` | `/api/v1/{app}/webhooks/{hostedPath}` | Invoke an app-hosted HTTP binding declared by `spec.http` or `apps.<name>.http`. A binding with `path: /` mounts at `/api/v1/{app}/webhooks`. The route is verified by the binding's `security` scheme, not by Gestalt session authentication. |

See [Hosted HTTP Bindings](/reference/app-manifests#hosted-http-bindings) for
path configuration and migrating registered webhook URLs from the removed aliases.
...
Each `instances[]` item may include `identity`. `identity` contains
display-oriented facts about the linked provider account. Internal grouping keys
are not part of the public response.
