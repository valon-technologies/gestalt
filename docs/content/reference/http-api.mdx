...
The HTTP API uses "integrations" in route paths (e.g. `/api/v1/apps`) as a stable code surface. The CLI uses the `app` command, while config uses the `apps` map. See [Config File](/reference/config-file#apps) for context.
