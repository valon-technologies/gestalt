...
Schedules and event triggers are configured in YAML under top-level
`workflows:`. Run responses return exactly one target under `target.plugin` or
`target.agent`. Plugin targets use `target.plugin` as an object with `name`,
`operation`, and optional `connection`, `instance`, and `input` fields.
