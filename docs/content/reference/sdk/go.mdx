...
`ValueFromAny`, and `TimestampFromTime` for protobuf well-known type
conversions. The `gen/v1` subpackage re-exports selected generated bindings for
transport fixtures, and the IndexedDB codec helpers convert records, keys, and
typed values without importing internal packages.
