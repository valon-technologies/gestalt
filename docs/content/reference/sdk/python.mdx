...
`AgentHost` includes plain Python helpers such as `list_tools_for_turn`,
`execute_tool_for_turn`, and `resolve_connection_for_turn`. For protocol-level
fixtures, import `protocol.v1` or `testing` from `gestalt` instead of importing
from private generated modules. Helpers such as `struct_from_dict`,
`value_from_json`, `timestamp_from_datetime`, `message_to_dict`,
`message_from_dict`, and the public IndexedDB conversion functions cover common
protobuf well-known type, protobuf JSON, and datastore shapes. Agent providers
that need the protobuf JSON form of `AgentMessage` can use
