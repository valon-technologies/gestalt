...
transport boundary. `AgentHost` exposes plain-object helpers named
`listToolsForTurn`, `executeToolForTurn`, and `resolveConnectionForTurn`. The
package also exports `@valon-technologies/gestalt/protocol/v1` for generated
schemas, services, and message types needed by transport fixtures, plus public
IndexedDB conversion helpers for lower-level datastore tests. Use root exports such as
`structFromJsonObject`, `jsonObjectFromStruct`, `valueFromJson`,
