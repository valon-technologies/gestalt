...
the transport boundary. `AgentHost` includes plain-input helpers named
`list_tools_for_turn`, `execute_tool_for_turn`, and
`resolve_connection_for_turn`. Use
...
system_time_from_timestamp}` for lower-level protobuf well-known type fixtures,
