...
runtime:
  providers:
    modal:
      source: https://artifacts.example.com/runtime/modal/v0.0.1-alpha.1/provider-release.yaml
      default: true
      config:
        app: gestalt-runtime
...
Workflow providers back workflow runs plus config-managed schedules and
triggers. They are configured under `providers.workflow.<name>`, then
referenced by top-level `workflows.*` config. The public workflow API and CLI
publish events and inspect or cancel runs.
...
Plugins and installable runtime providers are published separately from [`valon-technologies/gestalt-providers`](https://github.com/valon-technologies/gestalt-providers). Plugins are configured under `plugins`; installable hosted runtimes are configured under `runtime.providers`:
