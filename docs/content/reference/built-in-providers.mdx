...
Gestalt does not compile authentication, authorization, workflow, IndexedDB, or S3
...
Authentication providers handle platform login. They are configured under `providers.auth.<name>` in your config file.
...
To disable platform authentication entirely, omit the `providers.auth` block.
