...
Datastore providers back the persistent state layer. They are configured under `datastore.provider` in your config file, also using the `provider:` key. Gestalt does not compile datastore drivers into the binary; it starts the configured external datastore provider process at runtime.
