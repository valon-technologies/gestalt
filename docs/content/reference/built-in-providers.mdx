...
Gestalt does not compile identity, authorization, agent, workflow, runtime,
...
Identity providers handle platform login. They are configured under
`providers.identity.<name>` in your config file. Omit
`providers.identity` for local or unauthenticated deployments.
...
To disable platform identity entirely, omit the `providers.identity` block.
...
apps:
  support:
    ...
    runtime:
      image: ghcr.io/example/support-app:2026-04-21
```
...
Workflow providers back durable workflow definitions, activations, and runs. They are
configured under `providers.workflow.<name>`, then referenced by top-level
`workflows.definitions.*` config or the global workflow API/CLI.
...
| Name | Purpose |
| --- | --- |
| `indexeddb` | Stores workflow definitions, activations, runs, and per-step state in IndexedDB and invokes app operations through the workflow provider. |
...
Apps and installable runtime providers are published separately from [`valon-technologies/gestalt-providers`](https://github.com/valon-technologies/gestalt-providers). Apps are configured under `apps`; installable hosted runtimes are configured under `runtime.providers`:
