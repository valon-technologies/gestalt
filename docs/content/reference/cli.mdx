...
| `gestaltd provider add PACKAGE --kind KIND --name NAME` | Choose the provider kind and config entry name explicitly. Deprecated `kind: ui` also required `--set path=/mount`. |
...
gestaltd provider add github.com/acme/gestalt-providers/ui/console --config ./config.yaml --repo acme --kind ui --name console --set path=/console # deprecated
