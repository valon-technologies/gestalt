...
[Applications > Local Development](/applications#local-development) for
Vite, Angular, webpack-dev-server, and Parcel examples.
...
Run one Valon Tools app locally while delegating other configured providers to
`https://valon.tools`. Check in only the remote URL; pass your bearer token at
serve time with `--remote-token` (or `GESTALT_API_KEY` in the environment).

Create a token with `POST /api/v1/tokens`, the Gestalt token UI at
https://valon.tools/tokens, or `gestalt tokens create`.
...
Run CI/CD and Valon Profile locally while other apps stay remote:
...
gestaltd serve \
  ...
  --remote-token "$GESTALT_API_KEY" \
  valon-tools/apps/ci-cd \
  valon-tools/apps/valon-profile
```

See [Configuration > server.remote](/reference/config-file#server) and
[App providers > Testing locally](/providers/apps#testing-locally).
