...
| Command | Purpose |
| --- | --- |
| `gestaltd provider dev --path PATH` | Run one local provider-dev session around a target source plugin inside a synthesized Gestalt config. Serves `/admin` and any configured or inferred public UIs. |
...
not currently own file watching or automatic restarts for provider packages.
