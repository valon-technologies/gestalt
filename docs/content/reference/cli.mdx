...
Mark a single frontend-only run command with `frontendOnly: true` only when it
never serves `GESTALT_PROVIDER_SOCKET`. The frontend must bind the supplied
`GESTALT_DEV_PORT` exactly, honor `GESTALT_DEV_BASE_PATH`, and use gestaltd's
public mounted URL; automatic port fallback or an internal dev-server click
target is not supported. Apps without this declaration retain the full
configured backend socket readiness timeout.
