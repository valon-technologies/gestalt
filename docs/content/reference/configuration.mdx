...
| Public app frontends (`apps.<name>.static`) | [Applications](/applications) |
...
Gestalt defaults `server.public.port` to `8080`; if it is in use, `gestaltd serve` tries the
next free port. The runtime secrets manager defaults to built-in `env` when `providers.secrets` is omitted,
