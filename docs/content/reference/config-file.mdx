...
host-provider selection, and runtime defaults), `authorization` (shared
human/workload access policy), `providers` (named authentication, secrets,
telemetry, audit, UI, indexeddb, authorization, workflow, cache, and s3
entries), `runtime` (named plugin runtime backends), `workflows`
...
| Field | Description |
| --- | --- |
| `providers.<name>.driver` | **Required.** Runtime driver identifier such as `local` or `modal`. |
...
| `providers.<name>.config` | Driver-specific config blob. Some drivers, such as `local`, reject custom config entirely. |

### `runtime.providers.*.config` for `modal`

The built-in `modal` runtime creates hosted Linux sandboxes in a Modal app and
...
When a plugin uses the `modal` runtime, `plugins.<name>.runtime.image` is
...
| `allowedHosts` | Outbound host allowlist. For executable plugins, this is only a complete boundary when the selected runtime preserves hostname-based egress controls. |
...
| `runtime` | Optional hosted-runtime config. Setting this opts the plugin into runtime-backed execution. See `plugins.*.runtime` below. |
...
`plugins.<name>.runtime` opts an executable plugin into a hosted runtime. The
plugin stays host-local when this block is omitted.
...
When `provider` is omitted, Gestalt uses `server.runtime.provider` or the one
`runtime.providers.<name>` entry marked `default: true`. `template`, `image`,
and `metadata` are passed through to the runtime backend as opaque backend-
specific selectors or labels.

| Field | Description |
| --- | --- |
| `provider` | Named entry from `runtime.providers`. Optional when `server.runtime.provider` or one runtime default is available. |
| `template` | Optional runtime-specific template name forwarded when the session starts. |
| `image` | Optional runtime-specific image identifier forwarded when the session starts. |
| `metadata` | Optional string map forwarded to the runtime backend. Keys must be non-empty. |
...
boundary when the selected runtime preserves Gestalt's hostname-based policy
model. Runtimes that expose only CIDR or firewall controls cannot silently
stand in for `allowedHosts`.
...
complete security boundary when the plugin is running inside a runtime backend
that preserves hostname-based egress controls rather than relying on
...
Each `runtime.providers.<name>` entry must set `driver`. At most one runtime
entry may set `default: true`. `server.runtime.provider` must reference an
existing `runtime.providers.<name>` entry when set. `plugins.<name>.runtime`
must reference an existing runtime entry unless `server.runtime.provider` or a
runtime default can satisfy the selection. `plugins.<name>.runtime.metadata`
keys must be non-empty. Driver-specific `runtime.providers.<name>.config`
validation depends on the selected driver; for example, the built-in `local`
driver rejects custom config.
