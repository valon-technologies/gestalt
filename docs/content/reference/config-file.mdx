...
server:
  ...
  featureFlags:
    gcs:
      bucket: my-project-gestaltd-feature-flags
  providers:
...
`featureFlags.gcs.bucket` selects a Google Cloud Storage bucket containing
deployment feature flags. Gestaltd reads the root-level objects `agent` and
`workflow` once during server startup using application-default credentials.
Each object must contain exactly `true` or `false` after surrounding whitespace
is removed. Both flags default to `false` when the bucket or corresponding
object is absent. Other GCS read errors and invalid contents fail startup. A
running process does not poll for changes, so update the object and restart or
redeploy gestaltd to apply a new value. The bucket may be written as a bare name
or a `gs://` URL without path segments.
