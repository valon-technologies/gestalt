...
| `auth` | Optional plugin route auth reference. Use `auth.provider` to reference either the reserved `server` alias or a named entry from `providers.auth`. In this Phase 1 schema/validation change, the field is parsed and validated only; runtime enforcement lands in a later phase. |
...
For one compatibility release, a token-only sibling `auth` block is still
accepted as deprecated source auth on release-backed sources. `plugins.<name>.auth`
is otherwise reserved for plugin route auth overrides.
...
This field is schema/validation-only in this phase. It does not change runtime
route authentication behavior yet.
...
| `auth` | Optional plugin route auth reference. Use `auth.provider` to reference either `server` or a named `providers.auth` entry. In this Phase 1 schema/validation change, it is not enforced at runtime yet. |
| `env` | Extra environment variables passed to the provider process. |
| `allowedHosts` | Outbound host allowlist. For executable plugins, this is only a complete boundary when a supported sandboxed runtime is active. |
...
| `source` | A local UI manifest path, a published `provider-release.yaml` metadata URL, `source.url`, or `source.githubRelease` for a GitHub-hosted release asset. |
| `source.auth` | Optional metadata-fetch auth for release sources, including direct metadata URLs and `source.githubRelease`. |
