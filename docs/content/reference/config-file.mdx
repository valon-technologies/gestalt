...
Every `gestaltd` deployment reads a single YAML config file. The two top-level keys are `server` (listener, encryption, egress, and indexeddb selection) and `providers` (auth, fileapis, secrets, telemetry, audit, mounted UI bundles, indexeddbs, and plugins):
...
plugins. Configure reusable named entries under `providers.fileapis`, then bind
one or more of them into a plugin with `providers.plugins.<name>.fileapis`.
Unlike `server.indexeddb`, there is no single global File API selector.
...
The exact config fields depend on the selected external fileapi provider
package. Gestalt loads the configured provider process and passes
`providers.fileapis.<name>.config` through to that provider unchanged.

Each bound name is exposed to the plugin through a Unix socket environment
variable derived from the binding name, such as
`GESTALT_FILEAPI_SOCKET_ASSETS`. When a plugin binds exactly one File API,
Gestalt also populates `GESTALT_FILEAPI_SOCKET` for SDK compatibility.
...
| `fileapis` | Optional list of named `providers.fileapis` bindings exposed to the executable plugin through the SDK transport. A single binding also populates the default FileAPI SDK env var for compatibility. |
...
Auth providers, fileapi entries, and indexeddb entries use the same external provider shape. When `providers.auth`, `providers.fileapis.<name>`, or `providers.indexeddbs.<name>` is set, `source.ref` or `source.path` must be present. `providers.auth.config`, `providers.fileapis.<name>.config`, and `providers.indexeddbs.<name>.config` are only allowed when their corresponding source is set.
