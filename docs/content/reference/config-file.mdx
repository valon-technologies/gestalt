...
config API version is `gestaltd.config/v4`, and every config file must declare
...
`authorization` (shared subject access policy), `providers` (named
...
one or more named entries under `providers.workflow`; user-created schedules
and triggers select those providers through `/api/v1/workflow/schedules`,
...
| Field | Description |
| --- | --- |
| `default` | Marks this named workflow entry as the default selection when a workflow API request omits `provider`. |
| `source` | Either a local provider manifest path, a published `provider-release.yaml` metadata URL, or `source.githubRelease` for a GitHub-hosted release asset. |
| `auth` | Optional inline source auth for remote release sources, including direct metadata URLs and `source.githubRelease`. |
...
Connections declare how users authenticate to the upstream service. A provider
manifest can declare connection defaults under `spec.connections`; deployment
YAML can override or extend those defaults under `plugins.<name>.connections`.
When both declare the same connection name, deployment values are merged on top
of the manifest defaults. Only `connections.default` should define `params` and
`discovery` today. See [Provider Manifests](/reference/plugin-manifests#connections)
for the manifest-side schema.
