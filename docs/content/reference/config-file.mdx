...
config API version is `gestaltd.config/v7`, and every config file must declare
...
`authorization.models` defines deployment-owned authorization model fragments as generic resource types, relations, actions, allowed targets, and per-resource `defaultAccessPolicy`. `authorization.relationships` defines static relationship tuples against those resource types.
...
| `authorizationPolicy` | Optional named policy used by host-side operation filtering and execution checks for this app. |
...
`authorization.models.<model>.resourceTypes.<type>.defaultAccessPolicy` must be `allow` or `deny` when set.
