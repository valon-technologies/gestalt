...
`remote` and `remoteToken` configure gestaltd-to-gestaltd delegation. When `server.remote` is set, local `gestaltd` routes configured providers that are not served locally through the remote instance's generated public gRPC APIs. Local providers always win over remote routing for the same name. The remote `gestaltd` performs identity introspection and authorization on the bearer token; do not commit production tokens into config overlays.
...
`gestaltd serve --remote` and `--remote-token` override `server.remote` and `server.remoteToken`. Layer a URL-only local overlay and pass the token at serve time:

```yaml
# deploy/local/config.yaml
apiVersion: gestaltd.config/v8
server:
  remote: https://valon.tools
```
