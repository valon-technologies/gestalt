...
| `allowedOperations` | Allowlist with optional `alias`, `description`, `allowedRoles`, `api`, and `mcp` overrides per operation. Set `api: false` to remove an operation from the public HTTP/gRPC API and CLI, or `mcp: false` to remove it from MCP discovery and invocation. Internal app-to-app calls remain available. |
