...
The secrets manager resolves `secret://` references at startup. Unlike `auth` and `datastore`, secret providers are built into the `gestaltd` binary and selected by the `builtin` field.
