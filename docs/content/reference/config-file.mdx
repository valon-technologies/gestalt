...
| `target.steps[].agent` | Optional agent turn using `provider`, `model`, `sessionKey`, `prompt`, `messages`, `tools`, `output`, and `modelOptions`. |
| `target.steps[].when` | Optional guard with `value` and scalar `equals`; `value.stepOutput.stepId` may reference only earlier steps. |
| `runAs.subject.id` | Optional service-account subject ID, such as `service_account:roadmap-sync`, used as the runtime principal. Without [`runAs`](/service-accounts#runas), config-managed workflows run as `system:config` and cannot target subject-scoped credentials. |
| `runAs.subject.credentialSubjectId` | Optional credential subject when it differs from `runAs.subject.id`. |
...
| `target.steps[].agent` | Agent step turn fields: `provider`, `model`, `sessionKey`, `prompt`, `messages`, `tools`, `output`, and `modelOptions`. |
| `target.steps[].when` | Optional guard with `value` and scalar `equals`; `value.stepOutput.stepId` may reference only earlier steps. |
| `runAs.subject.id` | Optional service-account subject ID used as the runtime principal. See [`runAs`](/service-accounts#runas). |
| `runAs.subject.credentialSubjectId` | Optional credential subject when it differs from `runAs.subject.id`. |
| `invokes` | Optional additional app-operation grants as `{ app, operation }` entries. The direct target operation is always included. These grants are a token permission ceiling; they do not create credentials or replace `apps.<target>.invokes` for nested app calls. |
