...
Controls distributed traces, metrics, and structured logs. Gestalt uses [OpenTelemetry](https://opentelemetry.io) under the hood. Every HTTP request, broker invocation, and plugin gRPC call is automatically traced. See [Observability](/observability) for the emitted metric inventory and Prometheus name translation.
...
| `connections` | Named connections. `default` is common but not required if every surface names its connection explicitly. |
| `headers` | Static headers applied to declarative or spec-backed requests. |
| `managed_parameters` | Fixed header or path values injected into OpenAPI or GraphQL-backed operations before exposure. |
...
| `mcp` | MCP exposure settings such as `enabled` and `tool_prefix`. |
...
Use `egress` to control outbound requests made through providers and to grant
operator-managed credentials to specific outbound destinations.
...
| Field | Purpose |
| --- | --- |
| `default_action` | Global fallback policy. `allow` or `deny`. |
| `policies` | Static allow or deny rules matched by provider, operation, method, host, path, or subject. |
| `credentials` | Secret-backed outbound credential grants matched by subject, method, host, or path. |

For credential grants, `auth_style` may be `bearer`, `raw`, `basic`, or
`none`.
...
[Packaging and Releasing](/plugins/packaging-and-releasing) for the full
workflow.
