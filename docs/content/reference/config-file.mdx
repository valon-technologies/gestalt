...
`remote` points a local `gestaltd` at another deployment for configured providers that are not served locally. Local `gestaltd serve PATH` wins for the same app or selected IndexedDB provider. Keep tokens out of committed config: set `server.remote` in a local overlay and pass `--remote-token "$GESTALT_API_KEY"` (or `${GESTALT_API_KEY}` expansion) at serve time. Create API tokens with `gestalt tokens create`, the Gestalt token UI, or `POST /api/v1/tokens` on the remote deployment.
...
| `remote` | | Remote `gestaltd` origin for delegating configured providers not served locally. |
| `remoteToken` | | Bearer token for `server.remote`. Prefer CLI `--remote-token` over committing tokens. |
