...
Every `gestaltd` deployment reads a single YAML config file. The top-level keys are `server` (listener, encryption, egress, and indexeddb selection), `authorization` (shared human/workload access policy), `providers` (auth, fileapi, secrets, telemetry, audit, mounted UI bundles, and indexeddb), and `plugins` (executable integrations):
...
File API providers expose browser-style `Blob`, `File`, and object URL services to executable plugins. Configure one or more named entries under `providers.fileapi`, then opt a plugin into those host services with `plugins.<name>.fileapi`.
...
providers:
  fileapi:
    uploads:
      source:
        path: ./providers/uploads-fileapi/manifest.yaml

plugins:
  document_processor:
    source:
      path: ./plugins/document-processor/manifest.yaml
    fileapi:
      - uploads
```

Each `plugins.<name>.fileapi` entry must name a configured `providers.fileapi.<name>` provider. Gestalt serves each binding to the plugin over the same provider-host transport used for other executable provider dependencies. When a plugin binds exactly one File API provider, Gestalt also sets the default File API SDK socket env var for compatibility.
...
| `fileapi` | Optional list of named File API bindings. Each item must match a configured `providers.fileapi.<name>` entry. Gestalt serves those bindings to the plugin over provider-host sockets. A single binding also populates the default File API SDK env var for compatibility. |
...
Auth providers, fileapi entries, and indexeddb entries use the same external provider shape. When `providers.auth`, `providers.fileapi.<name>`, or `providers.indexeddb.<name>` is set, `source.ref` or `source.path` must be present. `providers.auth.config`, `providers.fileapi.<name>.config`, and `providers.indexeddb.<name>.config` are only allowed when their corresponding source is set.
