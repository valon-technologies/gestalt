...
| `allowedOperations` | Allowlist with optional `alias`, `description`, `allowedRoles`, `api`, `mcp`, and `internalCallers` overrides per operation. Set `api: false` to remove an operation from the public HTTP/gRPC API and CLI, or `mcp: false` to remove it from MCP discovery and invocation. Set `internalCallers` to exact verified refs such as `app:data-platform-dashboard` to fail closed for other internal callers. |
