...
The legacy spelling `providers.authentication` (and `server.providers.authentication`) is still accepted for compatibility, but `providers.identity` is canonical. Using both spellings in the same config is a validation error.
