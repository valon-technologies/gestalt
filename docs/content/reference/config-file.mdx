...
| `allowedOperations` | Deployer-owned allowlist with optional `alias`, `description`, and `allowedRoles` overrides per operation. Operation roles may only be set here; app manifests and provider implementations cannot set them. Omitting `allowedRoles` leaves the operation without a role-list override. |
