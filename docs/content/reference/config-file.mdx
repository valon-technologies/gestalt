...
`admin.ui` optionally pins `/admin` to one named deprecated UI bundle. The
selected bundle must ship `admin/index.html` inside its prepared asset root.
When `admin.ui` is omitted, Gestalt first auto-discovers admin assets from the
root `apps.<name>.static` mount when that bundle contains `admin/index.html`,
then falls back to the built-in admin shell.
...
| `admin.ui` | | **Deprecated.** Optional named UI bundle whose `admin/` assets should back `/admin`. When omitted, Gestalt auto-discovers `admin/` assets from the root `apps.<name>.static` mount before using the built-in fallback. |
...
When `server.admin.ui` is set, it must reference an existing `providers.ui.<name>`
entry (**deprecated**). Runtime startup then requires that entry's prepared asset
root to include `admin/index.html`. When unset, Gestalt auto-discovers
`admin/index.html` from the root `apps.<name>.static` mount before falling back
to the built-in admin shell.
