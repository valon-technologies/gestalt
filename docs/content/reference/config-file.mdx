...
authorization, workflow, cache, and s3 entries), `workflows` (config-managed
schedules and event triggers), and `plugins` (executable integrations and
optional plugin-backed UI mounts):
