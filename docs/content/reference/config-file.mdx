...
The exact config fields depend on the selected external datastore provider package. Gestalt itself does not have a built-in datastore driver matrix; it loads the configured provider process and passes the `datastore.config` block through to that provider.
