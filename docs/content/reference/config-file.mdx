...
and host-provider selection), `authorization` (shared human and identity-token access
...
`authorization` configures shared human and identity-token access policy at the top level.
...
`authorization.identityTokens` configures static bearer tokens that authenticate directly as managed identities. Each token uses the `gst_wld_` prefix for compatibility, but the runtime treats it as an identity-bound caller, not a separate workload type. In v1, these tokens may bind only to `identity` or `none` providers. For `identity` providers, the map key names the managed identity that owns the upstream credential; `connection` and `instance` resolve that stored credential, `instance` defaults to `default`, and `connection` falls back to the provider's default connection when omitted. For `none` providers, omit `connection` and `instance`.

`authorization.workloads` remains accepted as a deprecated compatibility alias for `authorization.identityTokens`. New configs should use `identityTokens`.
...
| `authorization.identityTokens` | | Static bearer tokens that authenticate as managed identities, with per-provider allowlists and identity credential bindings. |
