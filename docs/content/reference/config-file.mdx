...
identity, secrets, telemetry, audit, indexeddb, authorization,
...
definitions), and `apps` (executable integrations and optional static frontend
mounts):
...
`admin.authorizationPolicy` names the policy used to protect the built-in `/admin` route. `admin.authorizationAction` names the action checked on that policy and defaults to the policy name for backward compatibility. `admin.allowedRoles` controls which roles may load the built-in admin UI, defaulting to `[admin]`.
...
| `admin.authorizationAction` | `admin.authorizationPolicy` | Action checked on the admin authorization policy. |
...
Outputs structured logs to standard output. Traces remain disabled, but metrics are collected locally and exposed through the Prometheus-compatible `/metrics` endpoint. The built-in admin UI reads that same scrape surface. This is the default when `providers.telemetry` is omitted.
...
Exports to any OpenTelemetry-compatible collector (Jaeger, Grafana Alloy, Datadog Agent) while still keeping local `/metrics` available by default. The built-in admin UI reads that same local scrape surface.
...
| `allowedOperations` | Allowlist with optional `alias`, `description`, `allowedRoles`, `api`, and `mcp` overrides per operation. Set `api: false` to remove an operation from the public HTTP/gRPC API and CLI, `api: browserSession` to require a browser session on HTTP, or `mcp: false` to remove it from MCP discovery and invocation. When both are explicitly false, internal calls require existing authorization grants for both the original user and verified caller; no caller-specific config field is needed. |
...
| `static.publicConfig` | `{}` | App-owned JSON object served at `{mount}/public-config.json` before the static or development handler, with the mount's access policy. Use only non-secret presentation data. Provider `config` and secrets are never included automatically. |
| `static.public` | `false` | When `true`, serve static assets, public config, and theme endpoints without platform authentication or static-mount authorization. API routes for the app still require a session. |
...
omitted and must follow mount path rules.

Each name in `apps.<name>.s3` must reference an existing enabled
`providers.s3.<name>` entry.

App static mounts are served on the public listener only.
