...
Platform OAuth2 connections can also use a deployer-owned refresh token when a
real service mailbox or upstream account must be used instead of client
credentials:
...
| `grantType` | | OAuth2 grant type. `client_credentials` and `refresh_token` are supported on `mode: platform` connections. |
| `refreshToken` | | OAuth2 refresh token for `grantType: refresh_token` platform connections. Use a secret ref for production deployments. |
