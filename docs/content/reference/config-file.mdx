...
config API version is `gestaltd.config/v5`, and every config file must declare
...
invoke an explicit `target.steps` plan. Each step runs exactly one plugin call
or one agent turn.
...
| `target.steps` | Required ordered workflow steps. Each step has `id` and exactly one of `plugin` or `agent`. |
| `target.steps[].inputs` | Optional named values available to the step. Values use the same source shape as plugin input values. |
| `target.steps[].plugin.name` | Target plugin key from `plugins`. Required for plugin steps. |
| `target.steps[].plugin.operation` | Target operation ID. Required for plugin steps. |
| `target.steps[].plugin.connection` | Optional target connection name. |
| `target.steps[].plugin.instance` | Optional target instance name. |
| `target.steps[].plugin.input` | Optional workflow value that must resolve to a JSON object. Use `literal` for static params. |
...
| `target.steps[].outputDelivery.plugin` | Optional plugin call that receives this step's output. Its `input` uses workflow values such as `stepOutput`. |
...
| `provider` | Optional named `providers.workflow` entry. When omitted, Gestalt uses the sole/default workflow provider. |
| `target.steps` | Required ordered workflow steps. Each step has `id` and exactly one of `plugin` or `agent`. |
| `target.steps[].plugin` | Plugin step call fields: `name`, `operation`, optional `connection`, optional `instance`, and optional workflow-value `input`. |
...
| `target.steps[].when` | Optional guard with `value` and scalar `equals`; `value.stepOutput.stepId` may reference only earlier steps. |
| `target.steps[].outputDelivery.plugin` | Optional plugin call that receives this step's output. |
...
| `connections` | Named connection bindings. A binding usually uses `ref` to attach a top-level connection to the local plugin connection name; simple single-use `none` connections may be declared inline. See [connections](#connections) below. |
...
```yaml
source: https://github.com/valon-technologies/gestalt-providers/releases/download/plugins/github/v1.2.3/provider-release.yaml
```
...
```yaml
source:
  url: https://github.com/valon-technologies/gestalt-providers/releases/download/plugins/github/v1.2.3/provider-release.yaml
  auth:
...
Do not put release-download credentials in `plugins.<name>.auth`. That field
selects route authentication for the mounted plugin. Use `source.auth` for
...
`plugins.<name>.connections` or `providers.agent.<name>.connections` maps with
...
plugins:
  slack:
    source: ./plugins/slack/manifest.yaml
    connections:
      default:
        ref: slack-workspace
```
...
plugins:
  gmail:
    source: ./plugins/gmail/manifest.yaml
    connections:
      service:
