...
Gestalt provider packages and release archives are described by a manifest file: `provider.yaml`, `provider.yml`, or `provider.json`. YAML is the easiest format to author by hand.
...
`provider.yaml` stays YAML and `provider.json` stays JSON in the release archive.
