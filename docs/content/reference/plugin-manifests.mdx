...
override or extend these defaults under `plugins.<name>.connections`; see
...
Hosted HTTP routes are declared in the manifest. Source plugins written in Go,
...
written in JSON or YAML. For plugin connections, put `params` and `discovery`
...
release preserves those blocks in the packaged manifest. Other executable
provider kinds use the same release flow, but they do not generate plugin
operation catalogs. See [Plugins](/providers/plugins) for the end-to-end
authoring flow.
