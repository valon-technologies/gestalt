...
| `http` | map[string]HTTPBinding | Optional hosted HTTP bindings layered on top of operations. Each binding mounts under `/api/v1/{app}/webhooks` and targets an operation ID. |
...
`spec.http.<name>.path` is relative to `/api/v1/{app}/webhooks`. The configured
app key determines `{app}`: for a deployment entry named `example`,
`path: /command` mounts at `/api/v1/example/webhooks/command`, and `path: /`
mounts at `/api/v1/example/webhooks`. Gestalt adds the `/webhooks` namespace;
do not add it to the binding path yourself.

Legacy unnamespaced webhook aliases are no longer served. Before upgrading,
update external webhook registrations from `/api/v1/{app}/{hostedPath}` to
`/api/v1/{app}/webhooks/{hostedPath}`. For example, change
`/api/v1/example/command` to `/api/v1/example/webhooks/command` while keeping
`path: /command` and the signing configuration unchanged. The authenticated
`/api/v1/{app}/{operation}` API is unchanged and is not a webhook alias.
...
| Field | Type | Required | Purpose |
| --- | --- | --- | --- |
| `path` | string | yes | Route path relative to `/api/v1/{app}/webhooks`. |
...
| `api`          | bool                     | Set to `false` to hide and reject this operation on the public HTTP/gRPC API and CLI. Omitted defaults to `true`. Internal app-to-app calls are unaffected. |
| `mcp`          | bool                     | Set to `false` to hide and reject this operation on MCP. Omitted defaults to `true`. Internal app-to-app calls are unaffected. |
| `internalCallers` | string[]              | Optional non-empty allowlist of verified internal caller refs (`app:name`, `workflow:name`, or `agent:name`). When set, internal calls fail closed unless the caller matches exactly. Omission preserves unrestricted internal calls. Public HTTP/MCP requests still obey `api`/`mcp`; calls through another Gestalt deployment's public API fail closed. |
...
| `accountIdentity`   | bool   | Marks a `token_response` parameter as the provider's stable account identifier.           |
