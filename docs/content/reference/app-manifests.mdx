...
| Kind        | Purpose                                              |
| ----------- | ---------------------------------------------------- |
| `plugin` | App (REST, OpenAPI, GraphQL, MCP, or executable). |
...
The `spec` block for a `kind: plugin` manifest describes a app. It supports declarative REST operations, spec-loaded surfaces (OpenAPI, GraphQL, MCP), executable code, or any combination.
...
| `auth` | RouteAuthRef | Optional app route-auth reference. Use `auth.provider` to bind the plugin’s mounted routes to a named request-auth provider. Upstream auth belongs in `spec.connections.<name>.auth`. |
...
`spec.http.<name>.path` is plugin-relative. The configured app key determines
...
Declarative-only app packages do not require `artifacts`. Pure executable source manifests used with `apps.*.provider.source.path` or `gestaltd provider release` may omit them.

## Full Example

This manifest shows a app with an executable entrypoint, multiple connections, OpenAPI and MCP surfaces, pagination, allowed operations, and managed parameters.
...
`plugin`, `authentication`, `authorization`, `agent`, `cache`, `indexeddb`,
...
behavior you need. OpenAPI and GraphQL may be declared together on one plugin,
