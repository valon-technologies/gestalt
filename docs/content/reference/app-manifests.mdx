...
| `api`          | bool or `browserSession` | Set to `false` to hide and reject this operation on the public HTTP/gRPC API and CLI, or to `browserSession` to require a verified browser-login session, including for internal calls. Omitted defaults to `true`. When `api` and `mcp` are both explicitly `false`, internal calls require grants for both the original user and verified caller. |
| `mcp`          | bool                     | Set to `false` to hide and reject this operation on MCP. Omitted defaults to `true`. When `api` is also explicitly `false`, internal calls require grants for both the original user and verified caller. |
