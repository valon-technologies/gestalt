...
The REST facade hides MCP tools that locally declare `_connection` or `_instance`
inputs, because those names are reserved HTTP selectors. External JSON Schema
`$ref` targets are not fetched for this check, so a tool with only an external
input schema remains visible unless local schema siblings declare a reserved
selector.
