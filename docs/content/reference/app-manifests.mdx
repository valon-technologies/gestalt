...
| `ui` | OwnedUIRef | **Deprecated.** Optional owned UI reference. Use unified `build` static output instead. |

### Owned UI

<Callout type="warning">
  `spec.ui` is deprecated. Ship frontend assets from the same `kind: app`
  package using `build` commands that write to `GESTALT_BUILD_STATIC`. See
  [Applications](/applications#application-shape).
</Callout>
...
Released app manifests keep the same `spec.ui.path` shape, but the path is
...
<Callout type="warning">
  `kind: ui` manifests and this section are deprecated. Unified `kind: app`
  packages embed static assets via [Command Phases](#command-phases) and
...
<Callout type="warning">
  The following `kind: ui` example is deprecated. Use unified `kind: app`
  [Command Phases](#command-phases) instead.
</Callout>
...
There is no `release.build` field and no separate `build.output` field.
