...
- **Object** — `{ command: [...], workdir, env, inputs, readyTimeout, frontendOnly }`
...
| `frontendOnly` | boolean | (`run` only) Explicitly declares that this single run command never serves `GESTALT_PROVIDER_SOCKET`; it may skip backend socket readiness. |

`frontendOnly: true` is valid only when `run` has exactly one command. The
frontend must bind the supplied `GESTALT_DEV_PORT` exactly and honor
`GESTALT_DEV_BASE_PATH`; framework-selected port fallback is unsupported. The
frontend is mounted behind gestaltd, so it should use the public mounted URL
and must not advertise its own internal click target. Omit `frontendOnly` for
any app that may start a backend provider: gestaltd waits through the full
configured `readyTimeout` so a slow backend is not misclassified.

For a static frontend with no provider process, declare the capability on its
single run command:
