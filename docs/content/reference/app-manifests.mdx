...
| `api`          | bool                     | Set to `false` to hide and reject this operation on the public HTTP/gRPC API and CLI. Omitted defaults to `true`. When `mcp` is also explicitly `false`, internal calls require grants for both the original user and verified caller. |
