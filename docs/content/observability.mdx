...
These metrics are recorded at the single operation invoke authorization gate in
`Broker.authorizeOperation` and `Broker.checkAuthorizationAccess` before
provider dispatch. They cover v1 HTTP UI/CLI, HTTP bindings, cross-app
`App.Invoke`, workflow runner invokes, and GraphQL authorization checks.
Remote-delegated providers and MCP invokes are out of scope for this family.

<Tabs items={['OpenTelemetry', 'Prometheus']}>
  <Tabs.Tab>
    | Metric | Type | Meaning |
    | --- | --- | --- |
    | `gestaltd.invoke.authorization.count` | Counter | Invoke authorization decisions |
    | `gestaltd.invoke.authorization.error_count` | Counter | Denied invoke authorization decisions |
    | `gestaltd.invoke.authorization.duration` | Histogram | Invoke authorization decision duration |
  </Tabs.Tab>
  <Tabs.Tab>
    | Metric | Type | Meaning |
    | --- | --- | --- |
    | `gestaltd_invoke_authorization_count_total` | Counter | Invoke authorization decisions |
    | `gestaltd_invoke_authorization_error_count_total` | Counter | Denied invoke authorization decisions |
    | `gestaltd_invoke_authorization_duration_seconds` | Histogram | Invoke authorization decision duration |
  </Tabs.Tab>
</Tabs>

These metrics carry low-cardinality attributes: `gestalt.provider` (target app),
`gestalt.operation`, `gestaltd.invocation.surface` (`http`, `http_binding`,
`workflow`, `cross_app`, or `unknown`), `gestaltd.invoke.authorization.decision`
(`allow` or `deny`), `gestaltd.invoke.authorization.deny_reason` on denies
(`relation_denied`, `role_denied`, or `authorization_error`),
`gestaltd.subject.kind`, and `gestaltd.subject.id` (normalized caller email or
service-account name when available, otherwise `unknown`). P1 ingress tags such
as `gestaltd.ingress.kind` and `gestaltd.subject.label` remain HTTP telemetry
only and are not repurposed for invoke authorization decisions.
...
| Pre-invoker authorization denials | Yes: `gestaltd.invoke.authorization.*` | Yes | Denied subject access before guarded invocation dispatch is audited with the attempted operation or `operations.list`. |
