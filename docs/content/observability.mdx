...
Telemetry is selected as a `gestaltd` host provider. It is not an invocable
provider process. The selected `providers.telemetry` entry configures
`gestaltd` itself and is also translated into standard `OTEL_*` environment
variables for executable provider processes.

That keeps provider telemetry on the same OpenTelemetry configuration as the
host without routing telemetry through another Gestalt RPC service. Non-Python
providers should initialize their language's OpenTelemetry SDK from the
injected environment. Python providers using `gestalt-sdk` get exporter setup
automatically from the runtime and can import `telemetry` from `gestalt` for
GenAI semantic-convention helpers.

Provider SDKs expose GenAI semantic-convention helpers for provider-owned model,
agent, and tool work. These helpers record through the process-global
OpenTelemetry SDK for each language.
...
<Tabs items={['Go', 'Python', 'Rust', 'TypeScript']}>
  <Tabs.Tab>
    ...
    func callModel(ctx context.Context) (err error) {
        ctx, operation := gestalt.ModelOperation(ctx, gestalt.ModelOperationOptions{
            ProviderName: "openai",
        ...
        response, err := client.Responses.Create(ctx, request)
        if err != nil {
            return err
        }

        operation.SetResponseMetadata(response.ID, response.Model, "stop")
        operation.RecordUsage(gestalt.TokenUsage{
            InputTokens:  gestalt.TokenCount(response.Usage.InputTokens),
            OutputTokens: gestalt.TokenCount(response.Usage.OutputTokens),
        })
        return nil
    }
    ```
  ...
  <Tabs.Tab>
    ...
    def call_model() -> None:
        with telemetry.model_operation(
            ...
            request_options={"max_tokens": 512, "temperature": 0.2},
        ) as operation:
            response = client.responses.create(...)
            operation.set_response_metadata(response, finish_reasons=["stop"])
            telemetry.record_openai_usage(operation, response)
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```rust
    use gestalt::telemetry::{
        self, ModelOperationOptions, RequestOptions, TokenUsage,
    };

    async fn call_model() -> Result<(), Box<dyn std::error::Error>> {
        let mut operation = telemetry::model_operation(
            ModelOperationOptions::new("openai", "gpt-4.1").with_request_options(
                RequestOptions {
                    max_tokens: Some(512),
        ...
        let response = client.responses_create(request).await?;
        operation.set_response_metadata(Some(&response.id), Some(&response.model), &["stop"]);
  ...
  <Tabs.Tab>
    ```ts
    import { withModelOperation } from "@valon-technologies/gestalt/telemetry";

    await withModelOperation(
      {
        providerName: "openai",
        requestModel: "gpt-4.1",
        requestOptions: { max_tokens: 512, temperature: 0.2 },
      },
      async (operation) => {
...
<Tabs items={['OpenTelemetry', 'Prometheus']}>
  <Tabs.Tab>
    ...
    | `gestaltd.operation.error_count` | Counter | Unsuccessful operation invocations (compatibility) |
  ...
  <Tabs.Tab>
    ...
    | `gestaltd_operation_error_count_total` | Counter | Unsuccessful operation invocations (compatibility) |
...
Use the terminal outcome attributes instead of HTTP status to measure
reliability:

| Attribute | Values | Meaning |
| --- | --- | --- |
| `gestalt.outcome` | `success`, `rejected`, `failed` | Whether the invocation succeeded, was rejected before useful execution, or failed after admission |
| `gestalt.failure_cause` | `none`, `caller`, `connection`, `gestalt`, `provider`, `upstream`, `unknown` | The boundary that owns the failure |
| `gestalt.failure_reason` | A bounded reason such as `authorization_denied`, `reconnect_required`, `upstream_timeout`, or `execution_error` | The actionable failure class |

An availability calculation should use `success` and `failed` as its
denominator and exclude `rejected`. The legacy `error_count` instrument remains
available for compatibility and includes both `rejected` and `failed`; it is
derived from the same terminal classification as `count`.

Gestaltd classifies all host-owned failures and declarative REST or GraphQL
upstream failures directly. An executable app result that contains only an
HTTP-like error status is `failed` with cause `unknown`, because the host cannot safely
infer caller, provider, or upstream ownership from a response body.
...
These metrics are recorded around app connection authentication flows including
...
<Tabs items={['OpenTelemetry', 'Prometheus']}>
  <Tabs.Tab>
    ...
    | `gestaltd.connection.auth.error_count` | Counter | Unsuccessful connection authentication attempts (compatibility) |
  ...
  <Tabs.Tab>
    ...
    | `gestaltd_connection_auth_error_count_total` | Counter | Unsuccessful connection authentication attempts (compatibility) |
...
or `subject`). They also carry the same `gestalt.outcome`,
`gestalt.failure_cause`, and `gestalt.failure_reason` attributes as operation
metrics. This separates invalid caller input from missing Gestalt
configuration, upstream token exchange failures, and provider-owned connection
setup failures.
...
These metrics are recorded around the configured top-level identity provider used
...
`GET /api/v1/apps/{name}/operations` when `gestaltd` resolves a
...
- `gestalt.agent.provider` identifies the configured agent provider on
  provider-bound metric families.
...
tool paths visible before work reaches an app provider.
...
- `gestalt.provider` identifies the app provider when the path has resolved
  one.
- `gestalt.operation` identifies the app operation for catalog operation
  resolution.
...
- `db.collection.name` identifies the IndexedDB object store being accessed,
  such as `users`, `external_credentials`, or `authentication_grants`.
...
analytics for users or apps. Those need separate semantic decisions or an
...
<Tabs items={['OpenTelemetry', 'Prometheus']}>
  ...
  <Tabs.Tab>
    | Metric | Type | Meaning |
    | --- | --- | --- |
    | `rpc_client_call_duration_seconds` | Histogram | Outbound app RPC duration |
...
include the HTTP middleware metrics, broker metrics, app gRPC client
