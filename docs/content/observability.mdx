...
These metrics carry low-cardinality attributes: `gestalt.store` (the object
store name, such as `users`, `integration_tokens`, or `api_tokens`) and
`gestalt.method` (the operation, such as `Get`, `Put`, `Add`, `Delete`,
`GetAll`, `Count`, `Index.Get`, or `Index.GetAll`).
