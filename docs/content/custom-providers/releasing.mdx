...
Gestalt uses the same release system for executable provider packages, auth, fileapi, and datastore component packages, and packaged web UI bundles. Every package is described by a provider manifest file: `manifest.yaml`, `manifest.yml`, or `manifest.json`.
...
static assets and lets provider/auth/fileapi/datastore packages generate
support files such as config schemas before packaging.
...
the host-platform artifact by default. Go and Rust auth/fileapi/datastore
