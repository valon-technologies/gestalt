  ...
  <Tabs.Tab>
    ...
    class ExampleRuntime(gestalt.PluginRuntimeProvider):
        ...
        def BindHostService(self, request, context):
            return pb.PluginRuntimeHostServiceBinding(
                id="binding-1",
  ...
  <Tabs.Tab>
    ...
    impl proto::plugin_runtime_provider_server::PluginRuntimeProvider for ExampleRuntime {
        ...
        async fn bind_host_service(
            &self,
            request: Request<proto::BindPluginRuntimeHostServiceRequest>,
        ) -> Result<Response<proto::PluginRuntimeHostServiceBinding>, Status> {
            let req = request.into_inner();
            Ok(Response::new(proto::PluginRuntimeHostServiceBinding {
                id: "binding-1".into(),
...
configuration. `StartPlugin` then carries the manifest-derived `command` and
`args` to execute inside the image. Execute those values exactly as provided by
the host after applying any environment variables, host-service bindings, and
egress policy in the request.
