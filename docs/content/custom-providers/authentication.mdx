...
An authentication provider manifest declares `kind: auth`. You can optionally include a `configSchemaPath` so Gestalt validates provider config at startup.
...
To use a Google authentication provider in a Gestalt deployment, reference it as a named entry in `providers.auth`:
