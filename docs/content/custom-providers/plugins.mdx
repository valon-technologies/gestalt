...
The endpoint declaration usually belongs to the plugin package, either in
`spec.http` / `spec.securitySchemes` in the manifest or as SDK-generated
metadata. Deployment config selects the plugin and may override fields, but
most deployments do not need a separate route declaration. If the configured
plugin key is `slack` and the binding path is `/commands/support`, Gestalt
mounts the route at `/api/v1/slack/commands/support` on `server.baseUrl`.
...
<Tabs items={['Manifest', 'Python', 'TypeScript']}>
  <Tabs.Tab>
    ```yaml
    kind: plugin
    source: github.com/your-org/plugins/slack
    ...
    plugin = gestalt.Plugin(
        ...
        http={
            "support_command": {
                ...
                "ack": {"status": 200, "body": {"status": "accepted"}},
            },
        },
    )
...
`securitySchemes` / `http` metadata. Gestalt writes that metadata into the
effective manifest during local source execution and `gestaltd provider release`.
Hand-authored manifest `spec.securitySchemes` / `spec.http` entries are still
