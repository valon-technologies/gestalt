...
An application in Gestalt is not a separate provider kind. It is a deployment
pattern built from a plugin provider, an optional mounted UI bundle, and
host-managed provider bindings for authentication, authorization, state,
object storage, workflows, and calls to other plugins.

Use [Plugin](/providers/plugins) for the plugin manifest, SDK operations,
hosted HTTP routes, release flow, and local provider development. Use the
provider pages as the source of truth for individual service blocks:
[Authentication](/providers/authentication),
[Authorization](/providers/authorization),
[IndexedDB](/providers/indexeddb),
[S3](/providers/s3),
[Workflow](/providers/workflow), and [UI](/providers/ui).
...
The UI bundle is static assets. Route rules are required when the mounted
plugin binds an `authorizationPolicy`. For UI package details, see
