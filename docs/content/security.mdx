...
**API grants.** Long-lived API access is issued by the configured authentication provider through token exchange. The provider stores only SHA-256 hashes of bearer tokens plus grant metadata (owner subject, scopes, expiration, revocation). The plaintext bearer is returned once at creation. Use `POST /api/v1/tokens` to mint a grant and `GET`/`DELETE /api/v1/tokens` to list or revoke them.
