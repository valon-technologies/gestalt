...
### Plugin runtimes and isolation

Executable plugin providers run locally by default. A plugin only moves into a
hosted runtime when it sets `plugins.<name>.runtime`.

When a plugin stays local, `gestaltd` can still add host-local isolation when
`allowedHosts` is configured or when `server.egress.defaultAction` is `deny`.
That path depends on host-specific runtime support, so it should not be treated
as a portable security boundary by itself.
...
Hosted runtimes must preserve that same hostname-based policy model before
Gestalt will use them for plugins that declare `allowedHosts`. A backend that
only exposes CIDR or firewall controls cannot faithfully reproduce rules such
as `api.github.com` versus another hostname on shared infrastructure, so
Gestalt rejects those combinations rather than silently weakening policy.

The current `modal` runtime falls into that category: it provides hosted
execution and gRPC tunneling, but not hostname-aware egress enforcement or host
service tunnels. Gestalt therefore only uses Modal for plain executable
plugins that do not require those controls, including workflow-manager access
