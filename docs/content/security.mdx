...
Gestalt sits between callers and upstream APIs. It authenticates users, stores encrypted credentials, and isolates plugin processes. For implementation details — datastore schemas, encryption mechanics, token lifecycle internals, and secret resolution flow — see [Architecture](/architecture).
