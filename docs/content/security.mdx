...
All app tokens are encrypted before storage using AES-256-GCM.
...
Access tokens, refresh tokens, pending connection state, and OAuth state parameters are all encrypted with AES-256-GCM. OAuth state carries a 10-minute TTL. Authentication grant bearer tokens are stored by the identity provider as one-way SHA-256 hashes. User emails and display names are stored in plaintext.

## Token lifecycle

**Platform sessions.** Users log in through the configured identity provider. Gestalt validates the callback, checks `email_verified`, and issues a session cookie. Sessions expire based on the identity provider's TTL (default 24 hours).

**API grants.** Long-lived API access is issued by the configured identity provider through token exchange. The provider stores only SHA-256 hashes of bearer tokens plus grant metadata (owner subject, scopes, expiration, revocation). The plaintext bearer is returned once at creation. Use `POST /api/v1/tokens` to mint a grant and `GET`/`DELETE /api/v1/tokens` to list or revoke them.

**App tokens (OAuth).** OAuth state is encrypted with a 10-minute TTL to prevent state injection. After the authorization code exchange, access and refresh tokens are encrypted separately and stored. Gestalt refreshes expiring tokens on demand during invocation, and connections that declare `credentialRefresh` can be maintained proactively by a supporting external credentials provider.

## Request authentication

Protected routes authenticate in this order:

1. `session_token` cookie when present
2. `Authorization: Bearer` when the cookie is absent or invalid

The resolved token is validated through the identity provider's `Introspect` endpoint. Unauthenticated HTML requests to protected static mounts redirect to login; other requests return `401`.
...
Executable apps may also opt into a runtime provider. In that model,
...
Executable app providers can run with additional OS-level isolation when
...
| `providers.identity` | Omitting `providers.identity` disables all authentication. Do not use in production. |
