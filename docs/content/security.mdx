...
Gestalt sits between callers and upstream APIs. It authenticates users, stores encrypted credentials, and isolates plugin processes.
...
Changing `server.encryption_key` requires re-encrypting all stored tokens; there is no automated key rotation. Platform sessions cannot be explicitly revoked and expire based on the configured TTL. Rate limiting and CORS must be configured at the reverse proxy layer. Gestalt encrypts tokens at the application level, but the database may store metadata in plaintext, so appropriate database access controls are still important.
