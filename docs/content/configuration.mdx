...
providers:
  ...
  fileapis: ...             # named Blob/File storage providers
...
- **[File API.](/providers/fileapi)** Blob/File-style object storage mounted into plugins. Use this for plugin-managed binary objects such as uploads, generated documents, and object URLs.
...
## Mounting a File API into a plugin

Declare one or more reusable file providers under `providers.fileapis`, then
bind the ones a plugin needs in `providers.plugins.<name>.fileapis`:
...
Inside the plugin, the Gestalt SDK exposes that binding as a File API socket.
If exactly one File API is bound, the SDK default env var is populated. Named
bindings are always available through `GESTALT_FILEAPI_SOCKET_<NAME>`.
