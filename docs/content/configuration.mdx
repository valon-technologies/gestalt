...
1. `GESTALT_CONFIG`
2. `.gestaltd/config.yaml`
3. `~/.gestaltd/config.yaml`

If nothing is found, `gestaltd` generates a default local config at `~/.gestaltd/config.yaml`.
For project-local server state, prefer `.gestaltd/config.yaml` so Gestalt does not collide with other tools that also use `config.yaml`.
Container deployment guides can still point at `/etc/gestalt/config.yaml` because they pass that path explicitly with `--config`.
...
```sh
gestaltd serve --config .gestaltd/config.yaml
```
