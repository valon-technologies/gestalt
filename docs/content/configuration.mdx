...
For published providers, prefer a package source. Gestalt resolves the package
through the provider registry during `gestaltd lock` and records the exact
release in the lockfile. Use an exact `provider-release.yaml` URL or
`source.githubRelease` only when the provider is outside a package index or
when you need to pin directly to one release asset. First-party providers are
published from `github.com/valon-technologies/gestalt-providers/`.
...
See [Providers](/providers#building-providers) if you need to build your own.
...
With multiple bindings, use the named helper such as `S3("archive")` in Python
or `new S3("archive")` in TypeScript. See [S3 Providers](/providers/s3) for backend-specific
...
Or point at a published UI bundle:
...
[Agent](/providers/agent#building-your-own-agent-provider) if you need to implement
your own backend.
