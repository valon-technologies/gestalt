...
are active, but the runtime has to preserve Gestalt's hostname-based policy
model. Backends that only support CIDR or firewall controls cannot faithfully
implement `allowedHosts` by themselves, so Gestalt rejects those
...
- `modal` for hosted Linux sandboxes reached over a gRPC tunnel
