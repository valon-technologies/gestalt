...
server:
  ...
  providers:
    auth: ...               # optional selected authentication provider
...
providers:
  auth:                     # named authentication providers
...
| Multi-user deployment | `providers.auth.<name>` with a published OIDC or other authentication provider |
