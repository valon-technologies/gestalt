...
A Gestalt config has five top-level keys: `server` for host settings and host-provider selection, `authorization` for shared subject access policy, `providers` for named host-scoped providers and UIs, `runtime` for named plugin runtime backends, and `plugins` for executable integrations and optional plugin-backed UI mounts.
...
The `providers.workflow` block declares workflow backends. Schedules and
triggers are not declared in YAML; create them through the global workflow API
or CLI. Agents can create schedules through workflow system tools. Those
requests select a provider with `provider`, or inherit the sole/default
`providers.workflow` entry when that field is omitted.
...
Schedules and triggers are created through the workflow HTTP API or CLI. See
