...
Gestalt defaults `server.public.port` to `8080`, `secrets.provider` to `env`, and `telemetry.provider` to `stdout`. The required fields are `datastores`, `datastore`, and `server.encryptionKey`. Platform auth is optional.
