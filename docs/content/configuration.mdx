...
For project-local server state, prefer `./.gestaltd/config.yaml`. That keeps Gestalt from colliding with other tools that also use a generic `config.yaml` file name.
The examples below assume that layout, so project-root references step back out of `.gestaltd/` with `../...`.
