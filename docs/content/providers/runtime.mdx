...
`imagePullCredentials`, and `metadata` are forwarded to the runtime backend:
they are runtime-specific session hints, not universal plugin semantics. Keep
the manifest release and image digest in sync; Gestalt does not inspect the
