...
resolve to non-user credentials unless the workflow declares `runAs`. Mixed
plugins are still supported when the target explicitly selects a platform-backed
connection.
