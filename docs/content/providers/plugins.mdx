...
normally declared by the plugin package in its manifest or generated SDK
metadata. Server config selects the package and provides deployment-specific
settings; the mounted URL is built from the configured plugin key and the
binding path. For example, a `plugins.slack` entry with a binding path of
`/commands/support` is served at `/api/v1/slack/commands/support`.
