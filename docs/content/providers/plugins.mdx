...
Integration plugins are providers with `kinds: [plugin]`. They expose operations that users invoke through the CLI, the HTTP API, or MCP. A plugin can define operations as executable code (handled by the provider process), as passthrough surfaces (REST, OpenAPI, GraphQL, or MCP endpoints forwarded by the host), or both.
