...
In TypeScript plugin code, open host services from the request context. The
plugin does not know database DSNs, S3 credentials, or workflow storage details;
those come from the deployment bindings above.
...
export const plugin = definePlugin({
  operations: [
    operation({
      id: "items.save",
      method: "POST",
      description: "Save one review item",
      input: s.object({
        id: s.string(),
        title: s.string(),
        status: s.string(),
      ...
      async handler(input) {
        ...
        const items = db.objectStore("items");
        await items.put({
          id: input.id,
          title: input.title,
          status: input.status,
    ...
    operation({
      ...
      method: "POST",
      description: "Generate the nightly review report",
      input: s.object({
        bucket: s.string(),
      }),
      output: s.object({
      ...
      async handler(input, request) {
        const db = new IndexedDB();
        const reports = db.objectStore("reports");
        ...
        const object = s3.object(input.bucket, `nightly/${request.idempotencyKey}.json`);
        await object.write(body, { contentType: "application/json" });
