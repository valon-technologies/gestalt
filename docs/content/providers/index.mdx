...
compiling them into `gestaltd`. Plugins, auth backends, File API backends,
IndexedDB backends, external secret managers, and public web UIs all use the
same provider model: you reference a package in config, the host resolves it,
...
| `fileapi` | Blob/File-style storage backends mounted into plugins | `providers.fileapis.<name>` |
...
| File API | [`fileapi/`](https://github.com/valon-technologies/gestalt-providers/tree/main/fileapi) | `providers.fileapis.<name>` |
...
- [File API](/providers/fileapi): blob and file storage backends for plugins
