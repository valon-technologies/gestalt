...
<Cards num={6} className="provider-kind-cards">
  ...
  <Cards.Card title="UI (Deprecated)" href="/providers/ui" />
...
identity and datastore component packages, and packaged UI bundles. Every
...
<Callout type="warning">
  `kind: ui` source builds are deprecated. New apps use `kind: app` manifests
  with list-form `build` commands that write static assets to
  `GESTALT_BUILD_STATIC`. See [Applications](/applications#building-the-frontend).
</Callout>
...
<Callout type="warning">
  Released `kind: ui` packages are deprecated. New releases ship static assets
  inside `kind: app` archives under `static/`.
</Callout>
