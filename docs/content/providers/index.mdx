...
Providers are distributed separately from `gestaltd`, and each source form
describes where Gestalt should find provider metadata, code, or release
artifacts. Use indexed packages for published providers, local manifest paths
for in-repository development, exact release metadata for one-off releases, and
commit-pinned git sources when a provider should resolve from a specific
