...
<Cards num={6} className="provider-kind-cards">
  <Cards.Card title="Identity" href="/providers/identity" />
...
packaging. Executable source manifests declare `install`, `build`, and `run`
command lists. The `build` phase may write a provider executable and/or static
assets to `GESTALT_BUILD_STATIC`; Gestalt packages the latter as `static/` in
release archives.
...
apps:
  dashboard:
    source: ./apps/dashboard/manifest.yaml
    static:
      mount: /dashboard
```
...
The app mounted at `/` owns the app workspace's curated prompt examples. Put
them in that app's deployment `config.prompts` block. The app key is
deployment-defined (`home` in this example); Gestalt identifies the root app by
its `/` mount.

```yaml
apps:
  home:
    source: ./apps/default/manifest.yaml
    static:
      mount: /
    config:
      prompts:
...
Each app may define up to five prompts. Every prompt needs a unique `id` within
that app and a non-empty `text` of at most 280 characters. Gestalt exposes the
validated prompt definitions to the Home UI through `/api/v1/apps`; the rest of
the root app config remains provider-owned.
...
apps:
  github:
    source: https://github.com/valon-technologies/gestalt-providers/releases/download/app/github/v0.0.1-alpha.1/provider-release.yaml
```

```yaml
apps:
  default:
    source: https://github.com/valon-technologies/gestalt-providers/releases/download/app/default/v1.2.0/provider-release.yaml
    static:
      mount: /
```
...
```text
gestalt  github.com/valon-technologies/gestalt-providers/app/github  GitHub
```
...
```sh
gestaltd provider repo add gestalt https://registry.gestaltd.ai/provider-index.yaml
```
...
identity and datastore component packages, and unified app packages with static
frontends. Every
