...
- Executable plugins stay local by default; `plugins.<name>.runtime` opts an
  individual plugin into a named backend from `runtime.providers`.
- The built-in runtime drivers are `local` and `modal`.
...
- `allowedHosts` and `server.egress` declare outbound policy for executable
  providers, with complete enforcement depending on whether the selected
  runtime preserves hostname-based egress controls.
