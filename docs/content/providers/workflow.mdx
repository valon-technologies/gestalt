...
| Method | Purpose |
| --- | --- |
| `PlanWorkflow` | Validate a deployment spec and return provider-local plan identity |
| `ApplyWorkflowDeployment` / `GetWorkflowDeployment` / `ListWorkflowDeployments` / `DeleteWorkflowDeployment` | Manage workflow deployments |
| `SetWorkflowDeploymentPaused` / `SetWorkflowActivationPaused` | Pause or resume a whole deployment or one activation |
...
| `DeliverWorkflowEvent` | Deliver an event into matching deployment activations |
| `GetWorkflowRunEvents` / `GetWorkflowRunOutput` | Read provider-owned run event and output state |
| `PutExecutionReference` / `GetExecutionReference` / `ListExecutionReferences` | Store and read authority snapshots used by delayed host callbacks |
...
provider code should focus on durable deployment, activation, run, signal, event,
and output state.
...
<Tabs items={['Go', 'Python', 'Rust', 'TypeScript']}>
  <Tabs.Tab>
    ...
    func (p *Provider) PlanWorkflow(context.Context, *gestalt.PlanWorkflowRequest) (*gestalt.PlanWorkflowResponse, error) {
        return &gestalt.PlanWorkflowResponse{ProviderPlanID: "plan-1", ProviderPlanDigest: "plan-digest"}, nil
    }
    ...
    func (p *Provider) ApplyWorkflowDeployment(_ context.Context, req *gestalt.ApplyWorkflowDeploymentRequest) (*gestalt.WorkflowDeployment, error) {
        return &gestalt.WorkflowDeployment{Spec: req.Spec, Status: gestalt.WorkflowDeploymentStatusValueActive}, nil
    }
  ...
  <Tabs.Tab>
    ...
    class ExampleWorkflow(WorkflowProvider):
        ...
        def apply_deployment(
            self, request: ApplyWorkflowDeploymentRequest
        ) -> WorkflowDeployment:
            return WorkflowDeployment(
                spec=request.spec,
                status=WORKFLOW_DEPLOYMENT_STATUS_ACTIVE,
            )
  ...
  <Tabs.Tab>
    ...
    impl gestalt::WorkflowProvider for ExampleWorkflow {
        async fn plan_workflow(
            &self,
            _req: gestalt::PlanWorkflowRequest,
        ) -> gestalt::Result<gestalt::PlanWorkflowResponse> {
            Ok(gestalt::PlanWorkflowResponse {
                provider_plan_id: "plan-1".into(),
                provider_plan_digest: "plan-digest".into(),
                ..Default::default()
            })
        }
        ...
        async fn apply_workflow_deployment(
            &self,
            req: gestalt::ApplyWorkflowDeploymentRequest,
        ) -> gestalt::Result<gestalt::WorkflowDeployment> {
            Ok(gestalt::WorkflowDeployment {
                spec: req.spec,
                status: gestalt::WorkflowDeploymentStatus::Active,
                ..Default::default()
            })
        }
  ...
  <Tabs.Tab>
    ...
    export const provider = defineWorkflowProvider({
      ...
      async planWorkflow(_request) {
        return { providerPlanId: "plan-1", providerPlanDigest: "plan-digest" };
      },
...
resp, err := host.InvokeWorkflowAction(ctx, gestalt.InvokeWorkflowActionInput{
    Selector: &gestalt.WorkflowHostActionSelector{
        ExecutionRef:           "workflow_schedule:nightly",
        ExecutionRefGeneration: 1,
        ExecutionRefSeal:       "host-seal",
        RunID:                  "run-1",
        StepID:                 "sync_items",
        ActionID:               "step/sync_items/plugin",
        AttemptNumber:          1,
        IdempotencyKey:         "run-1:sync_items:plugin:1",
    ...
    Plugin: &gestalt.WorkflowPluginActionPayload{
        Input: map[string]any{"cursor": "latest"},
    },
})
```
