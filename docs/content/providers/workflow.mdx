...
Workflow capabilities gate the workflow method only. The workflow target still
...
<Tabs items={['Go', 'Python', 'Rust', 'TypeScript']}>
  ...
  <Tabs.Tab>
    ```python
    from gestalt import Request, WorkflowEvent, WorkflowPublishEvent
  ...
  <Tabs.Tab>
    ```rust
    use gestalt::{WorkflowEvent, WorkflowPublishEvent};
    use serde_json::json;
  ...
  <Tabs.Tab>
    ...
    export async function publishSlackEvent(
      ...
      const event = await workflows.publishEvent({
        providerName: "local",
        event: workflowEvent({
          type: "slack.event.received",
          source: "slack",
