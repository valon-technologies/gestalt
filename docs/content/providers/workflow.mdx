...
[Workflow](/providers/workflow#building-your-own-workflow-provider) for the
...
[Workflow](/providers/workflow#building-your-own-workflow-provider).
  ...
  <Tabs.Tab>
    ...
    class ExampleWorkflow(gestalt.WorkflowProvider):
        ...
        def GetRun(self, request, context):
            return gestalt.BoundWorkflowRun(id=request.run_id)

        def ListRuns(self, request, context):
            return gestalt.ListWorkflowProviderRunsResponse(runs=[])

        def CancelRun(self, request, context):
            return gestalt.BoundWorkflowRun(
                id=request.run_id,
                status=gestalt.WORKFLOW_RUN_STATUS_CANCELED,
            )
  ...
  <Tabs.Tab>
    ...
    with gestalt.WorkflowHost() as host:
        resp = host.invoke_operation(
            gestalt.InvokeWorkflowOperationInput(
                target=gestalt.BoundWorkflowTargetInput(
                    plugin=gestalt.BoundWorkflowPluginTargetInput(
                        plugin_name="roadmap",
                        operation="sync_items",
...
[Workflow](/providers/workflow#building-your-own-workflow-provider) or the
