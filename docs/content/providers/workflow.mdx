...
requests, or the app-facing `WorkflowManager`.
...
older `{ app, operations }` shape. Do not set both on the same workflow.
...
## Using `WorkflowManager` from a app
...
<Tabs items={['CLI', 'HTTP', 'UI']}>
  ...
  <Tabs.Tab>
    ...
    curl -H "Authorization: Bearer $GESTALT_API_KEY" \
      "$GESTALT_URL/api/v1/workflow/runs?app=github&status=failed"
...
Execution references store the subject, credential subject, caller app, and
        ...
        .invoke_operation(gestalt::InvokeWorkflowOperationInput {
            target: Some(gestalt::BoundWorkflowTarget {
                steps: vec![gestalt::WorkflowStep {
                    id: "sync_items".into(),
                    action: gestalt::WorkflowStepAction::App(
