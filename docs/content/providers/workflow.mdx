...
Schedules and triggers are created through the global API or CLI. Agents can
create schedules through workflow system tools. Workflows execute as the subject
that created them.
...
Workflow provider selection is global, not plugin-scoped. Schedules and
triggers choose a provider with `provider`, or inherit the sole/default
...
<Tabs items={['CLI', 'HTTP']}>
  <Tabs.Tab>
    ...
    gestalt workflow schedules update <schedule-id> --provider local --cron "0 9 * * 1-5"
...
The `provider` field and CLI `--provider` flag can be omitted when you have a
sole/default workflow provider; include them when you need to select a specific
workflow backend.
...
<Tabs items={['CLI', 'HTTP']}>
  <Tabs.Tab>
    ...
    gestalt workflow triggers update <trigger-id> --provider local --type roadmap.item.synced
...
```json
{
  ...
  "triggers": [
    {
      "provider": "local",
      "match": { "type": "roadmap.sync.completed", "source": "roadmap" },
      "target": {
        "plugin": {
          "name": "slack",
          "operation": "chat.postMessage",
...
In that shape, the schedule only starts the first step. The
...
Schedules and triggers execute as the subject that created them.
