...
Gestalt workflows let you invoke an app operation or agent target later
...
`providers.workflow`, then define a workflow target for the app operation
...
target app operation, and you inspect or cancel the resulting runs through
...
Apps and other executable provider code can use `WorkflowManager` to create
...
capabilities. When `capabilities.workflow` is omitted, apps keep the legacy
...
the workflow-manager methods that app code may call:
...
`POST /api/v1/workflow/events`, or from app code through the plugin-facing
...
the host can invoke the target app operation or agent target under the right
...
allowed app operations that delayed execution may use later. Config-managed
...
when delayed work is ready to invoke a target app operation or agent target.
    ...
    let resp = host
        .invoke_operation(gestalt::InvokeWorkflowOperationInput {
            target: Some(gestalt::BoundWorkflowTarget::Plugin(
                gestalt::BoundWorkflowAppTarget {
                    app_name: "roadmap".into(),
