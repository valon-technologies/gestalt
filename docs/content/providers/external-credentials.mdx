...
Platform OAuth2 connections may use either `grantType: client_credentials` or
`grantType: refresh_token`. For `refresh_token`, put the deployer-owned
`refreshToken` in a secret ref and bind the connection with internal exposure
when only platform code should use it.
