...
`provider.source.path` points at the secrets provider's `plugin.yaml` during development. For production, use `provider.source.ref` and `version` to reference a published release:
