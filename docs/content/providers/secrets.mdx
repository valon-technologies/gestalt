...
secrets:
  provider:
    source:
      path: ./my-secrets/provider.yaml
  config:
    prefix: MY_APP_
```

`provider.source.path` points at the secrets provider's `provider.yaml` during development. For production, use `provider.source.ref` and `version` to reference a published release:
