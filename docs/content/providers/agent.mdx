...
<Tabs items={['Go', 'Python', 'Rust', 'TypeScript']}>
  ...
  <Tabs.Tab>
    ...
    def summarize_with_agent(request: gestalt.Request) -> dict[str, object]:
        with request.agent_manager() as agents:
            session = agents.create_session(
                gestalt.AgentManagerCreateSessionRequest(
            ...
            turn = agents.create_turn(
                gestalt.AgentManagerCreateTurnRequest(
                    ...
                    tool_refs=[
                        gestalt.AgentToolRef(plugin="roadmap", operation="sync")
                    ],
                )
            )

            events = agents.list_turn_events(
                gestalt.AgentManagerListTurnEventsRequest(
                    turn_id=turn.id,
                    limit=100,
                )
            )
...
<Tabs items={['Go', 'Python', 'Rust', 'TypeScript']}>
  ...
  <Tabs.Tab>
    ...
    def enqueue_agent_workflow(request: gestalt.Request, item_id: str) -> str:
        with request.workflow_manager() as workflows:
            queued = workflows.signal_or_start_run(
                gestalt.WorkflowManagerSignalOrStartRunRequest(
                    provider_name="local",
...
- [Agent](/providers/agent#building-your-own-agent-provider): implementing your own agent backend
