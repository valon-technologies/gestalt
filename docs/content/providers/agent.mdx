...
<Tabs items={['Go', 'Python', 'Rust', 'TypeScript']}>
  ...
  <Tabs.Tab>
    ```python
    from gestalt import AgentCreateSession, AgentCreateTurn
    from gestalt import AgentMessage, Request

    def summarize(request: Request) -> str:
        with request.agent() as agents:
            session = agents.create_session(
                AgentCreateSession(client_ref="roadmap-risk")
            )
  ...
  <Tabs.Tab>
    ```rust
    use gestalt::{
        AgentCreateSession, AgentCreateTurn,
        AgentMessage, Request,
    };

    async fn summarize(req: &Request) -> Result<String, Box<dyn std::error::Error>> {
        let mut agents = req.agent().await?;
        let session = agents.create_session(AgentCreateSession {
  ...
  <Tabs.Tab>
    ```ts
    import { Agent, type Request } from "@valon-technologies/gestalt";

    export async function summarize(request: Request) {
      const agents = new Agent(request);
