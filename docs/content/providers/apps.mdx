...
When the fleet config declares more apps than you run locally, layer a URL-only
overlay and pass a remote-issued bearer token at serve time. Local `gestaltd`
delegates missing providers to the remote instance over generated public gRPC
APIs; local apps and PATH overrides still win for the same provider name.
...
gestaltd serve \
  ...
  valon-tools/apps/ci-cd
```

Issue tokens with `POST /api/v1/tokens`, https://valon.tools/tokens, or
`gestalt tokens create`. See [CLI > Remote attach](/reference/cli#remote-attach).
