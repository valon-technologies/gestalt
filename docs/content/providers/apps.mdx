...
Delegate undeclared local providers to a remote deployment by layering a URL-only
overlay and passing a bearer token at serve time:
...
Serve multiple local apps while still delegating the rest of the fleet remotely:
