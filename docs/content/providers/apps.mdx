...
`api: false` or `mcp: false` to disable that ingress independently. These
flags affect both catalog discovery and direct invocation; calls from another
app through the host service remain available.

To make a hidden operation available only to one verified internal app, combine
the surface flags with `internalCallers`. Caller refs use the provider kind and
name, such as `app:data-platform-dashboard`:
  ...
  data-platform:
    allowedOperations:
      refresh:
        ...
        mcp: false
        internalCallers:
          - app:data-platform-dashboard
```

`internalCallers` must contain at least one ref. When it is set, internal calls
fail closed unless their verified caller provider exactly matches one of the
refs. Omitting it preserves unrestricted internal app and workflow calls.
Public HTTP and MCP requests continue to use the `api` and `mcp` flags.

Caller identity is preserved only across trusted provider and host-service
hops. Calls routed through another Gestalt deployment's public API fail closed;
that public hop does not accept caller identity supplied by the client.
