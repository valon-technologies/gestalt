...
Each operation is exposed to the public API and MCP by default. Add
`api: false` or `mcp: false` to disable that ingress independently. Set
`api: browserSession` to require a verified browser-login session. This requirement
also applies to internal calls; an app caller cannot substitute for the session.
API tokens, including tokens placed in cookies, do not qualify. Existing raw
session cookies require a fresh browser login before invoking these operations.
API/MCP exposure flags affect both catalog discovery and direct invocation;
`api: false` does not by itself disable internal host-service calls.

When both flags are explicitly `false`, the operation is private. Public
ingress is rejected, and an internal invocation must be allowed separately for
the original user and the verified caller provider by the existing
`AuthorizationProvider`. The caller is checked as the `kind:name` subject
(for example, `app:data-platform-dashboard`), and `allowedRoles` is applied to
each decision; it is not a caller-authorization bypass. Caller identity is
trusted only across host-managed provider hops, so a request routed through
another Gestalt deployment's public API fails closed.
...
Source manifests declare serial `install`, `build`, and `run` phases. Each
phase accepts an argv list, an object with `command`, or a list of steps:
...
See [App Manifests > Command Phases](/reference/app-manifests#command-phases).

## Static Assets

Unified apps may ship a browser frontend from the same package. During `build`,
Gestalt sets `GESTALT_BUILD_STATIC` to an absolute path under
`.gestalt/build-static/`. Your build script must write `index.html` there.
Gestalt stages the directory into `static/` in prepared and release archives.

Configure the public mount in deployment config:

```yaml
apps:
  support:
    source: ./support/manifest.yaml
    static:
      mount: /support
```

See [Applications](/applications#building-the-frontend) for Vite, webpack, and
...
- `install` — runs once before any build or run execution. Use it for
  ...
  lockfile.
- `build` — compiles the provider into the release artifact at
  `.gestaltd/bin/<name>` (derived from `source`). gestaltd verifies the output
  after `build.command` completes.
- `run` — serves the provider from source (backend kinds) or runs a long-lived
  dev server (UI kind) for local `gestaltd serve`.

Run order is `install` → `build` (for packaging/lock/sync) and `install` →
`run` (for local serve). All three are optional and additive; providers that
...
build:
  command: [bun, run, gestalt, build]
  inputs: [provider.ts, tsconfig.json, package.json]
run:
  command: [bun, run, gestalt, run, ".", "./provider.ts#provider"]
```
...
verification, or body shape needs to match the calling product.
...
source apps with optional `apps.<name>.static` mounts.
...
Run the app locally with the frontend dev proxy when the manifest declares
`run:` commands:
...
When the app needs supporting providers, extra plugins, or explicit static
mounts, layer real config overlays on top of the synthesized baseline:
...
When your config points at local app source paths and the manifest declares
`run:` commands, gestaltd proxies the frontend that binds `GESTALT_DEV_PORT`:
...
See [Applications > Local Development](/applications#local-development) and
[CLI > Local Frontend Development](/reference/cli#local-frontend-development)
for the `GESTALT_DEV_*` contract.
