...
A app package includes a manifest that defines metadata, connections, and
...
When executable
[plugins call other configured app operations](/applications#calling-another-plugin-from-your-application),
authorization scopes are threaded through the call stack for both users and
[service accounts](/service-accounts).

## Configuring a app provider
...
Use `egress.allowedHosts` when a app needs explicit outbound access:
...
Hosted HTTP endpoints let an external product call a app through a stable
...
A app can combine declarative passthrough surfaces with executable
...
a configured remote app and preserves the remote plugin's config. If more
