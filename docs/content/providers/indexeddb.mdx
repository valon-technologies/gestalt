...
Executable plugins must opt in through config before the host exposes the IndexedDB socket:
...
With a single binding, the host still exports the default `GESTALT_INDEXEDDB_SOCKET` env var for backward compatibility. If a plugin binds multiple IndexedDB providers, each one is exposed under a named env var and the SDKs let the plugin select the binding explicitly.
...
<Tabs items={['Go', 'Python', 'Rust', 'TypeScript']}>
  <Tabs.Tab>
    ...
    archiveDB, err := gestalt.IndexedDB("archive")
    if err != nil {
        log.Fatal(err)
    }
  ...
  <Tabs.Tab>
    ...
    archive_db = gestalt.IndexedDB("archive")
  ...
  <Tabs.Tab>
    ...
    let mut archive_db = gestalt::IndexedDB::connect_named("archive").await?;
  ...
  <Tabs.Tab>
    ...
    const archiveDb = new IndexedDB("archive");
