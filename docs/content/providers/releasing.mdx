...
Gestalt uses the same release system for executable provider packages, auth and datastore component packages, and packaged web UI bundles. Every package is described by a provider manifest file: `plugin.yaml`, `plugin.yml`, or `plugin.json`.
