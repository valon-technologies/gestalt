...
Gestalt uses the same release system for executable provider packages, auth and datastore component packages, and packaged web UI bundles. Every package is described by a provider manifest file: `provider.yaml`, `provider.yml`, or `provider.json`.
...
plugins:
  my-plugin:
    provider:
      source:
        path: ./plugins/my-plugin/provider.yaml
```
