...
Use the named helpers when a plugin binds more than one S3 provider: Go uses
`gestalt.S3("archive")`, Python uses `S3("archive")`, Rust uses
...
For the first-party config shape and binding model, see
[Configuring the first-party S3 provider](/providers/s3#configuring-the-first-party-s3-provider).
