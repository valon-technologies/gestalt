...
const APP_LIST_HEADERS: [&str; 5] = ["Name", "Description", "Connection", "Instance", "Status"];
...
impl<'a> ConnectionName<'a> {
    ...
    fn display(self) -> &'a str {
        if self.canonical() == APP_CONNECTION_NAME {
            APP_CONNECTION_ALIAS
        } else {
            self.canonical()
        }
    }
    ...
    match format {
        ...
        Format::Table => {
            ...
            for (index, item) in resp.as_array().unwrap_or(&Vec::new()).iter().enumerate() {
                if index > 0 {
                    rows.push(app_group_divider_row());
                }
                rows.extend(app_rows(item));
            }
            print_app_list_table(&rows);
        }
    }
    Ok(())
}

fn print_app_list_table(rows: &[Vec<String>]) {
    println!("{}", render_app_list_table(rows));
}

fn render_app_list_table(rows: &[Vec<String>]) -> String {
    let rendered = output::render_table(&APP_LIST_HEADERS, rows);
    rendered
        .lines()
        .map(expand_app_group_divider_line)
        .collect::<Vec<_>>()
        .join("\n")
}

fn expand_app_group_divider_line(line: &str) -> String {
    if !line.contains(APP_GROUP_DIVIDER_SENTINEL) {
    ...
    format!(
        ...
        APP_GROUP_DIVIDER.to_string().repeat(interior_width),
        &line[right_border..],
    )
}
...
fn app_group_divider_row() -> Vec<String> {
    vec![
        APP_GROUP_DIVIDER_SENTINEL.to_string(),
        String::new(),
        String::new(),
        String::new(),
        String::new(),
...
pub fn disconnect(
    ...
    let mut path = format!("/api/v1/apps/{name}");
    ...
    client
        .delete(&path)
        .with_context(|| format!("failed to disconnect app '{}'", name))?;
