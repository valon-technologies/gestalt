...
use super::workflow_target::{
    PluginTargetUpdate, build_plugin_target, literal_input, merge_plugin_target_flags,
    target_has_plugin, target_operation, target_plugin,
};
...
fn build_upsert_body(
    ...
    build_schedule_upsert_body(
        cron,
        timezone,
        build_plugin_target(plugin, operation, connection, instance, input),
        paused,
    )
}
