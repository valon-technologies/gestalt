...
pub fn create(client: &ApiClient, scopes: &str, expires_in: i64, format: Format) -> Result<()> {
...
pub fn list(client: &ApiClient, format: Format) -> Result<()> {
    ...
    for grant_id in &grants_resp.grant_ids {
        ...
        let created_at = if detail.created_at > 0 {
            serde_json::Value::String(unix_to_rfc3339(detail.created_at))
        } else {
            serde_json::Value::Null
        };
        let expires_at = if detail.expires_at > 0 {
            serde_json::Value::String(unix_to_rfc3339(detail.expires_at))
        } else {
            serde_json::Value::Null
        };
