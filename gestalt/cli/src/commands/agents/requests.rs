...
use crate::cli::{
    AgentSessionCreateArgs, AgentSessionUpdateArgs, AgentTurnCreateArgs, AgentTurnEventListArgs,
    AgentTurnEventStreamArgs,
};
...
use gestalt_sdk::public::generated::agent::{
    AgentExecutionStatus, AgentMessage, AgentMessagePart, AgentOutput, AgentSessionState,
    AgentTextOutput,
};
...
pub fn list_sessions(
    ...
    print_sessions(&serde_json::to_value(&resp)?, format);
    Ok(())
}
...
pub fn list_turns(
    ...
    print_turns(&serde_json::to_value(&resp)?, format);
    Ok(())
}
...
pub fn list_turn_events(
    ...
    print_turn_events(&serde_json::to_value(&resp)?, format);
    Ok(())
}
...
pub(crate) fn resume_latest_session_info(
    ...
    let sessions: Vec<AgentSessionInfo> = decode_json(serde_json::to_value(&resp)?)?;
    sessions
        .into_iter()
...
pub(crate) fn list_turn_event_values(client: &ApiClient, turn_id: &str) -> Result<Vec<Value>> {
    ...
    loop {
        ...
        let page: Vec<Value> = decode_json(serde_json::to_value(&resp)?)?;
        if page.is_empty() {
            return Ok(events);
        }
