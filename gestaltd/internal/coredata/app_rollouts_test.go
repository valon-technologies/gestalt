...
func TestAppInstanceMaterializationServiceListByInstance(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	svc := testutil.NewStubServices(t)
	for _, app := range []string{"g-issues", "g-slack"} {
		if _, err := svc.AppInstanceMaterializations.Acknowledge(ctx, &core.AppInstanceMaterialization{
			InstanceID: "replica-a",
			App:        app,
			Version:    "v1",
		}); err != nil {
			t.Fatalf("Acknowledge(%s): %v", app, err)
		}
	}
	if _, err := svc.AppInstanceMaterializations.Acknowledge(ctx, &core.AppInstanceMaterialization{
		...
		t.Fatalf("Acknowledge other instance: %v", err)
	}

	got, err := svc.AppInstanceMaterializations.ListByInstance(ctx, "replica-a")
	if err != nil {
		t.Fatalf("ListByInstance: %v", err)
	}
	apps := make(map[string]bool, len(got))
	for _, materialization := range got {
		if materialization.InstanceID != "replica-a" {
			t.Fatalf("materialization instance = %q, want replica-a", materialization.InstanceID)
		}
		apps[materialization.App] = true
	}
	if len(apps) != 2 || !apps["g-issues"] || !apps["g-slack"] {
		t.Fatalf("materialization apps = %#v, want g-issues and g-slack", apps)
	}
}
