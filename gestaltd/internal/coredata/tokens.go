...
func (s *TokenService) recordToToken(rec indexeddb.Record) (*core.IntegrationToken, error) {
	access, refresh, err := s.enc.DecryptTokenPair(
		tokenCiphertext(rec, "access_token_sealed", "access_token_encrypted"),
		tokenCiphertext(rec, "refresh_token_sealed", "refresh_token_encrypted"),
	)
	if err != nil {
...
func tokenCiphertext(rec indexeddb.Record, primary, fallback string) string {
	if value := recString(rec, primary); value != "" {
		return value
	}
	return recString(rec, fallback)
}
