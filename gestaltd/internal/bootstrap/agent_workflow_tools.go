...
func workflowSystemToolAgentTargetSchema() map[string]any {
	return workflowSystemToolTargetSchema()
}
	...
	if valueMap, ok := workflowSystemToolMap(value); ok {
		if len(valueMap) == 1 {
			for key, raw := range valueMap {
				...
				case "signalMetadata":
					return coreworkflow.Value{SignalMetadata: workflowSystemToolStringValue(raw)}, nil
				case "workflowContext":
					return coreworkflow.Value{WorkflowContext: workflowSystemToolStringValue(raw)}, nil
				case "stepOutput":
...
func workflowSystemToolApplyInheritedOutputDelivery(req agentSystemToolExecutionRequest, target *coreworkflow.Target) error {
	...
	lastStep.OutputDelivery = coreworkflow.CloneStepDelivery(req.InheritedOutputDelivery)
	return nil
}
...
func workflowSystemToolValueInfo(value coreworkflow.Value) any {
	...
	case strings.TrimSpace(value.SignalMetadata) != "":
		return map[string]any{"signalMetadata": value.SignalMetadata}
	case strings.TrimSpace(value.WorkflowContext) != "":
		return map[string]any{"workflowContext": value.WorkflowContext}
