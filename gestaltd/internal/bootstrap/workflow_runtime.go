...
func workflowEvaluateValue(value coreworkflow.Value, req coreworkflow.InvokeOperationRequest, outputs map[string]any, inputs map[string]any, allowInputs bool) (any, bool, error) {
	...
	case strings.TrimSpace(value.SignalMetadata) != "":
		...
		return workflowMapPathValue(signal.Metadata, value.SignalMetadata)
	case strings.TrimSpace(value.WorkflowContext) != "":
		return workflowMapPathValue(workflowInvocationContext(req), value.WorkflowContext)
...
func workflowTemplateExpressionValue(expr string, req coreworkflow.InvokeOperationRequest, outputs map[string]any, inputs map[string]any, allowInputs bool) (any, bool, error) {
	...
	case strings.HasPrefix(expr, "signalMetadata."):
		signal := workflowOutputDeliverySignal(req.Signals)
		if signal == nil {
			return nil, false, nil
		}
		return workflowMapPathValue(signal.Metadata, strings.TrimPrefix(expr, "signalMetadata."))
	case strings.HasPrefix(expr, "workflowContext."):
		return workflowMapPathValue(workflowInvocationContext(req), strings.TrimPrefix(expr, "workflowContext."))
	default:
