...
type startupProviderHandle[T any] struct {
	name    string
	target  startupProviderNode
	tracker *startupWaitTracker
	gate    startupGate[T]
}

func newStartupProviderHandle[T any](name string, target startupProviderNode, tracker *startupWaitTracker) *startupProviderHandle[T] {
	return &startupProviderHandle[T]{
...
func (h *startupProviderHandle[T]) publish(provider T) {
	h.gate.finish(provider, nil)
}

func (h *startupProviderHandle[T]) fail(err error) {
	var zero T
	h.gate.finish(zero, err)
}

func (h *startupProviderHandle[T]) await(ctx context.Context) (T, error) {
	var zero T
	if h == nil {
		return zero, fmt.Errorf("provider is not available")
	}
	done, err := h.beginCallerWait(ctx)
	if err != nil {
		return zero, err
	}
	defer done()
...
func (h *startupProviderHandle[T]) resolved() (T, bool, error) {
	if h == nil {
		var zero T
		return zero, false, nil
	}
	return h.gate.resolved()
}

func (h *startupProviderHandle[T]) beginCallerWait(ctx context.Context) (func(), error) {
	if h == nil || h.tracker == nil {
		return func() {}, nil
	}
	done, _, err := h.tracker.beginCallerProviderWait(ctx, h.target)
	return done, err
}
