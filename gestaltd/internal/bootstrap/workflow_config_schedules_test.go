...
func TestWorkflowConfigStepWhenPreservesEqualsPresence(t *testing.T) {
	...
	if got := workflowConfigStepWhen(&withNullEquals); got == nil || !got.EqualsSet {
		t.Fatalf("workflowConfigStepWhen with null equals = %#v, want EqualsSet", got)
	}
	...
	if got := workflowConfigStepWhen(&withoutEquals); got == nil || got.EqualsSet {
		t.Fatalf("workflowConfigStepWhen without equals = %#v, want EqualsSet=false", got)
	}
}
