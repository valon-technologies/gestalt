...
type ExternalCredentialsExecConfig struct {
	Command      string
	Args         []string
...
func NewExecutableExternalCredentialProvider(ctx context.Context, cfg ExternalCredentialsExecConfig) (core.ExternalCredentialProvider, error) {
	...
	if _, err := ConfigureRuntimeProvider(ctx, runtimeClient, proto.ProviderKind_PROVIDER_KIND_EXTERNAL_CREDENTIAL, cfg.Name, cfg.Config); err != nil {
		_ = proc.Close()
		return nil, err
	}
