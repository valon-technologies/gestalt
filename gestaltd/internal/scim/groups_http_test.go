...
func TestSCIMGroupsCRUDAndUserGroups(t *testing.T) {
	...
	if _, err := scim.WrapAuthorization(authorization, services.Users, scim.NewRuntime(secondService, config.ServerSCIMConfig{Clients: map[string]config.SCIMClientConfig{}})).AddRelationship(context.Background(), &proto.AddRelationshipRequest{Relationship: projected}); err != nil {
	...
	if _, err := scim.WrapAuthorization(authorization, services.Users, scim.NewRuntime(secondService, config.ServerSCIMConfig{Clients: map[string]config.SCIMClientConfig{}})).AddRelationship(context.Background(), &proto.AddRelationshipRequest{Relationship: external}); err != nil {
...
func TestSCIMGroupsSupportNestedMembershipAndNamespaceIsolation(t *testing.T) {
	...
	if _, err := scim.WrapAuthorization(authorization, services.Users, scim.NewRuntime(service, config.ServerSCIMConfig{Clients: map[string]config.SCIMClientConfig{}})).AddRelationship(context.Background(), &proto.AddRelationshipRequest{Relationship: &proto.Relationship{Tuple: childTuple, SourceLayer: proto.SourceLayer_SOURCE_LAYER_RUNTIME}}); err != nil {
		t.Fatal(err)
	}
