...
func scimRuntimeFor(service *scim.Service) *scim.Runtime {
	return scim.NewRuntime(service, config.ServerSCIMConfig{})
}
...
func TestSCIMProjectionFailureLeavesLiveStateForClientRetry(t *testing.T) {
	...
	if response, err := scim.WrapAuthorization(authorization, services.Users, scimRuntimeFor(service)).CheckAccess(context.Background(), request); err != nil || response.Allowed {
...
func TestSCIMCreateActiveSemantics(t *testing.T) {
	...
	authorizationGate := scim.WrapAuthorization(authorization, services.Users, scimRuntimeFor(service))
...
func TestSCIMClientNamespacesDoNotShareEligibility(t *testing.T) {
	...
	if response, err := scim.WrapAuthorization(authorization, services.Users, scimRuntimeFor(service)).CheckAccess(context.Background(), accessRequest(coreUser.ID)); err != nil || !response.Allowed {
...
func TestSCIMAuthorizationGateAppliesAtInvocationBroker(t *testing.T) {
	...
	broker := invocation.NewBroker(
		...
		invocation.WithAuthorizationProvider(scim.WrapAuthorization(authorization, services.Users, scimRuntimeFor(service))),
