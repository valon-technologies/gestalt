...
const (
	maxRootAppPromptExamples  = 5
	maxRootAppPromptTextRunes = 280
)

// AppPrompt is a deployer-owned prompt definition for the root app's app
// workspace promos. The stable ID keeps this shape usable if prompts become a
// first-class MCP surface later.
type AppPrompt struct {
	ID   string `yaml:"id"`
	Text string `yaml:"text"`
}

// RootAppPromptExamples returns the prompt bodies configured on the app
// mounted at /. The root app is the Home/default app, but its config key is
// deployment-defined (for example, apps.home), so lookup is based on the
// canonical root mount rather than a hard-coded app name.
func RootAppPromptExamples(apps map[string]*ProviderEntry) (map[string][]string, error) {
	rootName := ""
	var rootPrompts map[string][]string
	for name, entry := range apps {
		if entry == nil || entry.Static == nil || strings.TrimSpace(entry.Static.Mount) != "/" {
			continue
		}
		if rootName != "" {
			return nil, fmt.Errorf("apps.%s and apps.%s are both mounted at /", rootName, name)
		}
		rootName = name
		prompts, err := appPromptExamples(entry.Config)
		if err != nil {
...
func validateRootAppPromptConfig(entry *ProviderEntry) error {
	...
	_, err := appPromptExamples(entry.Config)
	return err
}

func appPromptExamples(node yaml.Node) (map[string][]string, error) {
	...
	result := make(map[string][]string, len(promptsNode.Content)/2)
	for i := 0; i+1 < len(promptsNode.Content); i += 2 {
		...
		if len(listNode.Content) > maxRootAppPromptExamples {
			return nil, fmt.Errorf("%s has %d prompts; maximum is %d", appName, len(listNode.Content), maxRootAppPromptExamples)
		}

		bodies := make([]string, 0, len(listNode.Content))
		...
		for index, promptNode := range listNode.Content {
			...
			bodies = append(bodies, prompt.Text)
		}
		if len(bodies) > 0 {
			result[appName] = bodies
		}
	}
	if len(result) == 0 {
		return nil, nil
