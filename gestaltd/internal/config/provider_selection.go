...
func (c *Config) SelectedModelProvider() (string, *ProviderEntry, error) {
	return ResolveSelectedHostProvider(HostProviderKindModel, c.Server.Providers.Selection(HostProviderKindModel), c.HostProviderEntries(HostProviderKindModel))
}
...
func (c *Config) EffectiveModelProvider(providerName string) (string, *ProviderEntry, error) {
	if c == nil {
		return "", nil, nil
	}
	providerName = strings.TrimSpace(providerName)
	if providerName != "" {
		entry, ok := c.Providers.Model[providerName]
		if !ok || entry == nil {
			return "", nil, fmt.Errorf("config validation: providers.model references unknown model %q", providerName)
		}
		return providerName, entry, nil
	}
	return c.SelectedModelProvider()
}
