...
func TestLoadLegacyConfigV5Migration(t *testing.T) {
	t.Parallel()

	t.Run("migrates apiVersion and plugins to apps", func(t *testing.T) {
		t.Parallel()
...
`)
		...
		if cfg.Apps["service-a"] == nil {
			t.Fatal(`Apps["service-a"] = nil`)
		}
		if len(cfg.Apps) != 1 {
	...
	t.Run("migrates apiVersion without plugins section", func(t *testing.T) {
		t.Parallel()
...
`)
	...
	t.Run("rejects both plugins and apps", func(t *testing.T) {
		t.Parallel()
...
plugins:
  one:
    source:
      path: /tmp/one/manifest.yaml
apps:
  two:
    source:
      path: /tmp/two/manifest.yaml
`)
		...
		if !strings.Contains(err.Error(), "cannot specify both top-level plugins and apps") {
			t.Fatalf("Load error = %v, want plugins/apps ambiguity error", err)
		}
	})
}
