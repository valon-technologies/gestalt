...
// ExternalCredentialsReferenced reports whether runtime needs an external credentials provider.
...
// IndexedDBIsReferenced reports whether runtime needs a top-level IndexedDB provider.
func IndexedDBIsReferenced(cfg *Config) bool {
	if cfg == nil {
		return false
	}
	if strings.TrimSpace(cfg.Server.Providers.IndexedDB) != "" || len(cfg.Providers.IndexedDB) > 0 {
		return true
	}
