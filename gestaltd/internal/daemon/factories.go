...
func setupBootstrapWithConfigPaths(configPaths []string, lockfilePath, artifactsDir string, locked, noSync bool, remote, remoteToken string, alpha bool, forcedDevAppKeys ...string) (*bootstrapEnv, error) {
	ctx, stop := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
	return setupBootstrapWithConfigPathsContext(ctx, stop, configPaths, lockfilePath, artifactsDir, locked, noSync, remote, remoteToken, alpha, forcedDevAppKeys...)
}

func setupBootstrapWithConfigPathsContext(ctx context.Context, stop context.CancelFunc, configPaths []string, lockfilePath, artifactsDir string, locked, noSync bool, remote, remoteToken string, alpha bool, forcedDevAppKeys ...string) (*bootstrapEnv, error) {
	cfg, err := loadConfigForExecutionAtPaths(configPaths, lockfilePath, artifactsDir, locked, noSync, forcedDevAppKeys...)
	if err != nil {
		stop()
		return nil, err
	}
	if err := config.ApplyServeRemoteOverrides(cfg, remote, remoteToken, alpha); err != nil {
		stop()
		return nil, err
	}
