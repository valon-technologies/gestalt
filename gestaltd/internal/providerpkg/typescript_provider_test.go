...
const (
	...
	typeScriptTestFileAPIModuleTarget   = "./fileapi.ts#fileapi"
	typeScriptTestFileAPITarget         = "fileapi:./fileapi.ts#fileapi"
...
func TestDetectTypeScriptComponentTarget(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			target: typeScriptTestFileAPITarget,
		},
		{
			name:   "indexeddb",
...
func TestHasSourceComponentPackage_TypeScript(t *testing.T) {
	...
	ok, err = HasSourceComponentPackage(root, providermanifestv1.KindFileAPI)
	if err != nil {
		t.Fatalf("HasSourceComponentPackage(fileapi): %v", err)
	}
	if ok {
		t.Fatal("HasSourceComponentPackage(fileapi) = true, want false")
	}
...
func mustWriteTypeScriptPackageConfig(t *testing.T, root string, gestalt map[string]string) {
	...
	var fields []string
	for _, key := range []string{typeScriptProviderKey, typeScriptPluginKey, typeScriptAuthKey, typeScriptFileAPIKey, typeScriptIndexedDBKey} {
...
func runtimeTargetModulePath(t *testing.T, target string) string {
	t.Helper()
	for _, prefix := range []string{"plugin:", "integration:", "auth:", "fileapi:", "indexeddb:", "secrets:", "telemetry:"} {
