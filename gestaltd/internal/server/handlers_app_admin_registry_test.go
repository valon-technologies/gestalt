...
func TestAppAdminRegistryHistoryPrefersStoredOutcomeOverLiveRollout(t *testing.T) {
	t.Parallel()
	...
	completedAt := deployedAt.Add(3*time.Minute + 8*time.Second)
	if err := services.AppVersionRolloutOutcomes.RecordComplete(context.Background(), request.ID, "g-issues", fixture.Version, completedAt); err != nil {
		t.Fatalf("RecordComplete: %v", err)
	}
	...
	if _, err := services.AppRollouts.MarkCompleteForRollout(context.Background(), rollout, deployedAt.Add(10*time.Minute)); err != nil {
		t.Fatalf("MarkCompleteForRollout: %v", err)
	}
