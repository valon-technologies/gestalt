...
func (s *Server) connectionInfosFromAdvertised(integration string, connections []advertisedConnection, instances []instanceInfo, preferences map[string]string, p *principal.Principal) []connectionDefInfo {
	infos := make([]connectionDefInfo, 0, len(connections))
	for i := range connections {
		conn := connections[i]
		if info, ok := s.connectionInfoFromAuth(integration, conn.Name, conn.InstanceConnection, conn.Def, instances, conn.IncludeWithoutAuth, preferences, p); ok {
...
func (s *Server) connectionInfoFromAuth(integration, name, instanceConnection string, conn config.ConnectionDef, instances []instanceInfo, includeWithoutAuth bool, preferences map[string]string, p *principal.Principal) (connectionDefInfo, bool) {
	...
	storedPreferred := preferences[connectionID]
