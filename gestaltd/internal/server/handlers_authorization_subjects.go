...
func (s *Server) managedSubjectExternalIdentityRows(ctx context.Context, subjectID string) ([]managedSubjectExternalIdentityInfo, error) {
	relationships, err := s.readAllAuthorizationRelationships(ctx, &core.ListRelationshipsRequest{
		PageSize: adminAuthorizationProviderReadPageSize,
		Filter: &core.RelationshipFilter{
			Target: &core.RelationshipTargetRef{Kind: &proto.RelationshipTarget_Subject{Subject: &core.SubjectRef{Type: authorization.ProviderSubjectTypeSubject, Id: strings.TrimSpace(subjectID)}}},
		},
	})
	if err != nil {
		return nil, err
	...
	for _, rel := range relationships {
		resource := authorization.RelationshipResource(rel)
		if rel == nil || resource == nil {
			continue
		}
		if resource.GetType() != authorization.ProviderResourceTypeExternalIdentity || authorization.RelationshipRelation(rel) != authorization.ProviderExternalIdentityRelationAssume {
			continue
		}
		ref, err := core.ExternalIdentityRefFromResourceID(resource.GetId())
		if err != nil {
			continue
		}
...
func (s *Server) deleteAuthorizationRelationships(ctx context.Context, relationships []*core.Relationship) error {
	...
	_ = modelID
	return s.writeAuthorizationRelationships(ctx, nil, relationshipTuples(rels))
}
