...
func newRuntimeLogHostTestClient(t *testing.T, socket string) proto.RuntimeLogHostClient {
	t.Helper()
	...
	return proto.NewRuntimeLogHostClient(conn)
}
...
func TestRuntimeLogHostServerAppendsLogsOverSDKTransport(t *testing.T) {
	...
	hostServices, err := StartHostServices([]HostService{{
		...
		Register: func(srv *grpc.Server) {
			proto.RegisterRuntimeLogHostServer(srv, NewRuntimeLogHostServer("modal", func(_ context.Context, runtimeProviderName, sessionID string, entries []runtimelogs.AppendEntry) (int64, error) {
				mu.Lock()
				defer mu.Unlock()
	...
	_, err = client.AppendLogs(context.Background(), &proto.AppendRuntimeLogsRequest{
		SessionId: "session-1",
		Logs: []*proto.RuntimeLogEntry{{
			Stream:     proto.RuntimeLogStream_RUNTIME_LOG_STREAM_RUNTIME,
			Message:    "runtime boot",
			ObservedAt: timestamppb.New(observedAt),
			SourceSeq:  7,
		}, {
			Stream:     proto.RuntimeLogStream_RUNTIME_LOG_STREAM_STDERR,
			Message:    "stderr line\n",
...
func TestRuntimeLogHostServerAppendsLogsAfterSessionStoppedOverSDKTransport(t *testing.T) {
	...
	hostServices, err := StartHostServices([]HostService{{
		Name: "runtime_logs",
		Register: func(srv *grpc.Server) {
			proto.RegisterRuntimeLogHostServer(srv, NewRuntimeLogHostServer("modal", store.AppendSessionLogs))
		},
	}})
	if err != nil {
		t.Fatalf("StartHostServices: %v", err)
	...
	_, err = client.AppendLogs(ctx, &proto.AppendRuntimeLogsRequest{SessionId: "session-1", Logs: []*proto.RuntimeLogEntry{{
		Stream:    proto.RuntimeLogStream_RUNTIME_LOG_STREAM_STDERR,
		Message:   "after stop\n",
		SourceSeq: 2,
	}}})
...
func TestRuntimeLogHostServerMapsUnknownSessionToNotFound(t *testing.T) {
	...
	client := newRuntimeLogHostTestClient(t, bindings[0].SocketPath)
	_, err = client.AppendLogs(ctx, &proto.AppendRuntimeLogsRequest{SessionId: "never-registered", Logs: []*proto.RuntimeLogEntry{{
		Stream:  proto.RuntimeLogStream_RUNTIME_LOG_STREAM_RUNTIME,
		Message: "should fail",
	}}})
...
func TestRuntimeLogHostServerKeepsStoppedSessionThroughEvictionPressure(t *testing.T) {
	...
	client := newRuntimeLogHostTestClient(t, bindings[0].SocketPath)
	_, err = client.AppendLogs(ctx, &proto.AppendRuntimeLogsRequest{SessionId: "stopping-session", Logs: []*proto.RuntimeLogEntry{{
		Stream:  proto.RuntimeLogStream_RUNTIME_LOG_STREAM_RUNTIME,
	...
	_, err = client.AppendLogs(ctx, &proto.AppendRuntimeLogsRequest{SessionId: "quiet-live-session", Logs: []*proto.RuntimeLogEntry{{
		Stream:  proto.RuntimeLogStream_RUNTIME_LOG_STREAM_RUNTIME,
