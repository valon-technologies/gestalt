...
func validateWorkflowStepWhenValue(index int, value coreworkflow.Value, previousSteps map[string]struct{}) error {
	if value.StepOutput != nil {
		...
		if stepID == "" {
			return fmt.Errorf("workflow target.steps[%d].when.value.step_output.step_id is required", index)
		}
		if _, ok := previousSteps[stepID]; !ok {
			return fmt.Errorf("workflow target.steps[%d].when.value.step_output.step_id %q must reference an earlier step", index, stepID)
		}
		if path == "" {
