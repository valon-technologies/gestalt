...
func workflowValueToProto(value coreworkflow.Value) (*proto.WorkflowValue, error) {
	...
	case strings.TrimSpace(value.SignalMetadata) != "":
		return &proto.WorkflowValue{Kind: &proto.WorkflowValue_SignalMetadata{SignalMetadata: &proto.WorkflowPathSource{Path: value.SignalMetadata}}}, nil
	case strings.TrimSpace(value.WorkflowContext) != "":
		return &proto.WorkflowValue{Kind: &proto.WorkflowValue_WorkflowContext{WorkflowContext: &proto.WorkflowPathSource{Path: value.WorkflowContext}}}, nil
...
func workflowValueFromProto(value *proto.WorkflowValue) coreworkflow.Value {
	...
	case *proto.WorkflowValue_SignalMetadata:
		return coreworkflow.Value{SignalMetadata: strings.TrimSpace(typed.SignalMetadata.GetPath())}
	case *proto.WorkflowValue_WorkflowContext:
		return coreworkflow.Value{WorkflowContext: strings.TrimSpace(typed.WorkflowContext.GetPath())}
