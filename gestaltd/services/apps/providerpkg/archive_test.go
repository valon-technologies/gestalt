...
func TestPackageRoundTripWithIconFile(t *testing.T) {
	...
	root := t.TempDir()
	sourceDir, manifest := mustWriteProviderPackageDir(t, root, "github.com/acme/apps/provider", "0.0.1-alpha.1", "provider")
	...
	archivePath := filepath.Join(root, "app.tar.gz")
...
func TestValidatePackageDirIconFileMissing(t *testing.T) {
	...
	root := t.TempDir()
	sourceDir, manifest := mustWriteProviderPackageDir(t, root, "github.com/acme/apps/provider", "0.2.0", "provider2")
	manifest.IconFile = "assets/icon.svg"
	mustWriteManifest(t, sourceDir, manifest)
...
func TestCreatePackageFromDirAndReadManifest(t *testing.T) {
	...
	manifest := mustRawManifestJSON(t, mustProviderManifest("github.com/acme/apps/provider", "0.0.1-alpha.1", testArtifactOS, testArtifactArch, artifactPath, sha256Hex("provider")))
	...
	if parsed.Source != "github.com/acme/apps/provider" {
