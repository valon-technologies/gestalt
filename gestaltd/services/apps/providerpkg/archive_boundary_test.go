...
func TestCreatePackageFromDirRejectsOutputInsideSource(t *testing.T) {
	...
	dir := t.TempDir()
	sourceDir, _ := mustWriteProviderPackageDir(t, dir, "github.com/acme/apps/provider", "0.0.1-alpha.1", "provider")
...
func TestInspectPackageRejectsProviderOwnedAllowedRoles(t *testing.T) {
	...
	archivePath := filepath.Join(t.TempDir(), "allowed-roles.tar.gz")
	...
	if err == nil {
		t.Fatal("expected provider-owned allowedRoles rejection")
	}
	if !strings.Contains(err.Error(), "allowedRoles") {
		t.Fatalf("unexpected error: %v", err)
	}
}
