...
func detectSourceComponent(root, kind, goos, goarch string) (sourceKind string, target string, err error) {
	...
	if kind == providermanifestv1.KindExternalCredentials || kind == providermanifestv1.KindAuthorization {
	...
	if manifest != nil {
		if manifest.Source != "" {
			...
			if last := parts[len(parts)-1]; last != "" {
				return slugAppName(last)
			}
		}
		if manifest.DisplayName != "" {
			return slugAppName(manifest.DisplayName)
		}
	}
	return slugAppName(fallback)
}

func slugAppName(value string) string {
