...
func TestParse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input: "github.com/testowner/testrepo/apps/testplugin",
			want:  Source{Host: HostGitHub, Owner: "testowner", Repo: "testrepo", Path: "apps/testplugin"},
		},
		{
		...
		{
			name:    "reject missing app segment",
...
func TestSourceStringRoundTrip(t *testing.T) {
	...
	const input = "github.com/testowner/testrepo/apps/testplugin"
	src, err := Parse(input)
	if err != nil {
...
func TestSourceAssetName(t *testing.T) {
	...
	src := Source{Host: HostGitHub, Owner: "testowner", Repo: "testrepo", Path: "apps/testplugin"}
	const want = "gestalt-app-testplugin_v1.2.3.tar.gz"
...
func TestSourceReleaseTag(t *testing.T) {
	...
	src := Source{Host: HostGitHub, Owner: "testowner", Repo: "testrepo", Path: "apps/testplugin"}
	const want = "apps/testplugin/v1.2.3"
...
func TestSourceAppName(t *testing.T) {
	...
	src := Source{Host: HostGitHub, Owner: "testowner", Repo: "testrepo", Path: "apps/testplugin"}
	if got := src.AppName(); got != "testplugin" {
		t.Errorf("AppName() = %q, want %q", got, "testplugin")
	}
}
