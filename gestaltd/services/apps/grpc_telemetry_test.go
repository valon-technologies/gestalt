...
func TestProviderServerGRPCOptionsRecordServerDurationWithTelemetryAttrs(t *testing.T) {
	...
	conn := newBufconnConnWithOptions(t,
		...
		func(srv *grpc.Server) {
			proto.RegisterAppProviderServer(srv, NewProviderServer(&coretesting.StubIntegration{N: providerName}))
		},
	)

	if _, err := proto.NewAppProviderClient(conn).GetMetadata(context.Background(), &emptypb.Empty{}); err != nil {
