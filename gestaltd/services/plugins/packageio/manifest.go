...
var validManifestKinds = map[string]bool{
	...
	providermanifestv1.KindModel:               true,
...
func ManifestKind(manifest *providermanifestv1.Manifest) (string, error) {
	...
	if !validManifestKinds[manifest.Kind] && !validManifestKinds[kind] {
		return "", fmt.Errorf("manifest kind %q is not valid; expected one of plugin, authentication, authorization, externalcredentials, indexeddb, cache, s3, workflow, agent, model, secrets, runtime, or ui", manifest.Kind)
	}
	return kind, nil
}
