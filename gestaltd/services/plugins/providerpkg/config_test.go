...
`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := ValidateConfigForManifest(manifestPath, manifest, providermanifestv1.KindPlugin, map[string]any{"api_key": "sk-test"}); err != nil {
				t.Fatalf("ValidateConfigForManifest(valid): %v", err)
			}
			if err := ValidateConfigForManifest(manifestPath, manifest, providermanifestv1.KindPlugin, map[string]any{"missing": true}); err == nil {
