...
func TestDetectSourceComponent_AuthorizationFallsBackToPython(t *testing.T) {
	t.Parallel()
...
`), 0o644)
	...
	kind, target, err := detectSourceComponent(root, providermanifestv1.KindAuthorization, runtime.GOOS, runtime.GOARCH)
	if err != nil {
		t.Fatalf("detectSourceComponent(authorization): %v", err)
	}
	if kind != sourceProviderKindPython {
		t.Fatalf("kind = %q, want %q", kind, sourceProviderKindPython)
	}
