...
func TestTransport_AppTCPTargetTokenEnv(t *testing.T) {
	...
	graphQLResult, err := app.InvokeGraphQL(context.Background(), "linear", "query { viewer { id } }", &client.AppInvokeGraphQLOptions{
	...
	decodedGraphQL, err := gestalt.InvokeGraphQL(context.Background(), app, "linear", " query { viewer { id } } ", &client.AppInvokeGraphQLOptions{
		IdempotencyKey: " graphql-call-43 ",
		Variables:      map[string]any{"team": "eng"},
	})
	if err != nil {
		t.Fatalf("gestalt.InvokeGraphQL helper: %v", err)
	}
	...
	if _, err := gestalt.InvokeGraphQL(context.Background(), app, "linear", "   ", nil); err == nil {
		t.Fatal("gestalt.InvokeGraphQL with empty document should fail")
	}
	...
	if got := harness.graphQL[0].GetVariables().AsMap(); len(got) != 1 || got["team"] != "eng" {
		t.Fatalf("graphql variables = %#v, want team=eng", got)
	}
}
