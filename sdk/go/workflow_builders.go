...
func workflowValueToProto(input WorkflowValue) (*proto.WorkflowValue, error) {
	...
	if strings.TrimSpace(input.SignalMetadata) != "" {
		set++
	}
	if strings.TrimSpace(input.WorkflowContext) != "" {
		set++
	}
...
func workflowValueFromProto(value *proto.WorkflowValue) WorkflowValue {
	...
	case *proto.WorkflowValue_SignalMetadata:
		return WorkflowValue{SignalMetadata: workflowPathSourcePath(typed.SignalMetadata)}
	case *proto.WorkflowValue_WorkflowContext:
		return WorkflowValue{WorkflowContext: workflowPathSourcePath(typed.WorkflowContext)}
