...
type transportKernelCase struct {
	...
	ExpectPrepareError *struct {
		Code int `json:"code"`
	} `json:"expectPrepareError"`
}
...
func TestFixtureCasesAreCovered(t *testing.T) {
	...
	for _, tc := range loadTransportKernelCases(t) {
		if tc.ExpectPrepare == nil && tc.ExpectDecode == nil && tc.ExpectGatewayError == nil && tc.ExpectGestaltError == nil && tc.ExpectPrepareError == nil {
...
func TestPrepareCasesFromFixture(t *testing.T) {
	...
	for _, tc := range loadTransportKernelCases(t) {
		...
		t.Run(tc.ID, func(t *testing.T) {
			...
			query, err := buildRESTQuery(method, tc.Request)
			if err != nil {
				t.Fatalf("buildRESTQuery: %v", err)
			}
...
func TestPrepareErrorCasesFromFixture(t *testing.T) {
	t.Parallel()
	for _, tc := range loadTransportKernelCases(t) {
		if tc.ExpectPrepareError == nil || tc.Request == nil {
			continue
		}
		tc := tc
		t.Run(tc.ID, func(t *testing.T) {
			t.Parallel()
			method := methodForPrepareCase(MethodAppInvoke, tc)
			if _, err := buildRESTPath(method, tc.Request); err != nil {
				if ge, ok := err.(*GestaltError); ok && int(ge.Code) == tc.ExpectPrepareError.Code {
					return
				}
			}
			if _, err := buildRESTQuery(method, tc.Request); err != nil {
				if ge, ok := err.(*GestaltError); ok && int(ge.Code) == tc.ExpectPrepareError.Code {
					return
				}
				t.Fatalf("buildRESTQuery: %v", err)
