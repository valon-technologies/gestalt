...
func (c *CacheClient) callCtx(ctx context.Context) context.Context {
	if c == nil {
		return ctx
	}
...
func (c *CacheClient) Get(ctx context.Context, key string) ([]byte, bool, error) {
	resp, err := c.client.Get(c.callCtx(ctx), &proto.CacheGetRequest{Key: key})
	if err != nil {
...
func (c *CacheClient) GetMany(ctx context.Context, keys []string) (map[string][]byte, error) {
	resp, err := c.client.GetMany(c.callCtx(ctx), &proto.CacheGetManyRequest{Keys: keys})
	if err != nil {
		return nil, err
	}
...
func (c *CacheClient) Set(ctx context.Context, key string, value []byte, opts CacheSetOptions) error {
	_, err := c.client.Set(c.callCtx(ctx), &proto.CacheSetRequest{
		Key:   key,
...
func (c *CacheClient) SetMany(ctx context.Context, entries []CacheEntry, opts CacheSetOptions) error {
	...
	_, err := c.client.SetMany(c.callCtx(ctx), &proto.CacheSetManyRequest{
...
func (c *CacheClient) Delete(ctx context.Context, key string) (bool, error) {
	resp, err := c.client.Delete(c.callCtx(ctx), &proto.CacheDeleteRequest{Key: key})
	if err != nil {
		return false, err
	}
...
func (c *CacheClient) DeleteMany(ctx context.Context, keys []string) (int64, error) {
	resp, err := c.client.DeleteMany(c.callCtx(ctx), &proto.CacheDeleteManyRequest{Keys: keys})
	if err != nil {
		return 0, err
	}
	return resp.GetDeleted(), nil
...
func (c *CacheClient) Touch(ctx context.Context, key string, ttl time.Duration) (bool, error) {
	resp, err := c.client.Touch(c.callCtx(ctx), &proto.CacheTouchRequest{Key: key, Ttl: cacheTTLToProto(ttl)})
	if err != nil {
		return false, err
	}
