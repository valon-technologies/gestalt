...
impl Cursor {
    ...
    async fn send_and_recv(
        ...
        let resp = self
            ...
            .message()
            .await?
            .ok_or_else(|| IndexedDBError::Status(tonic::Status::internal("cursor stream ended")))?;
...
async fn open_cursor_inner(
    ...
    let resp = stream
        .message()
        .await?
        .ok_or_else(|| IndexedDBError::Status(tonic::Status::internal("empty cursor response")))?;

    let entry = match resp.result {
        Some(pb::cursor_response::Result::Entry(entry)) => Some(entry),
        _ => None,
    };
...
fn typed_value_to_json(v: &pb::TypedValue) -> serde_json::Value {
    ...
    match &v.kind {
        ...
        Some(Kind::TimeValue(ts)) => serde_json::Value::String(format!(
            "{}.{}",
            ts.seconds, ts.nanos
        )),
