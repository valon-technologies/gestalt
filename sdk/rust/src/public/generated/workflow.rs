// Code generated by sdkgen. DO NOT EDIT.

//! Generated native types and clients for workflow.proto.

use crate::generated::v1;
use crate::public::generated::agent::AgentOutput;
use crate::public::generated::app::AgentToolRef;
use crate::public::generated::codec::workflow::{
    decode_wire_get_workflow_provider_run_events_response_json,
    decode_wire_get_workflow_provider_run_output_response_json,
    decode_wire_list_workflow_provider_definitions_response_json,
    decode_wire_list_workflow_provider_runs_response_json,
    decode_wire_signal_workflow_run_response_json, decode_wire_workflow_definition_json,
    decode_wire_workflow_run_json, encode_wire_apply_workflow_provider_definition_request_json,
    encode_wire_cancel_workflow_provider_run_request_json,
    encode_wire_delete_workflow_provider_definition_request_json,
    encode_wire_get_workflow_provider_definition_request_json,
    encode_wire_get_workflow_provider_run_events_request_json,
    encode_wire_get_workflow_provider_run_output_request_json,
    encode_wire_get_workflow_provider_run_request_json,
    encode_wire_list_workflow_provider_definitions_request_json,
    encode_wire_list_workflow_provider_runs_request_json,
    encode_wire_set_workflow_provider_activation_paused_request_json,
    encode_wire_set_workflow_provider_definition_paused_request_json,
    encode_wire_signal_or_start_workflow_provider_run_request_json,
    encode_wire_signal_workflow_provider_run_request_json,
    encode_wire_start_workflow_provider_run_request_json,
    from_wire_get_workflow_provider_run_events_response,
    from_wire_get_workflow_provider_run_output_response,
    from_wire_list_workflow_provider_definitions_response,
    from_wire_list_workflow_provider_runs_response, from_wire_signal_workflow_run_response,
    from_wire_workflow_definition, from_wire_workflow_run,
    to_wire_apply_workflow_provider_definition_request,
    to_wire_cancel_workflow_provider_run_request,
    to_wire_delete_workflow_provider_definition_request,
    to_wire_get_workflow_provider_definition_request,
    to_wire_get_workflow_provider_run_events_request,
    to_wire_get_workflow_provider_run_output_request, to_wire_get_workflow_provider_run_request,
    to_wire_list_workflow_provider_definitions_request,
    to_wire_list_workflow_provider_runs_request,
    to_wire_set_workflow_provider_activation_paused_request,
    to_wire_set_workflow_provider_definition_paused_request,
    to_wire_signal_or_start_workflow_provider_run_request,
    to_wire_signal_workflow_provider_run_request, to_wire_start_workflow_provider_run_request,
};
use crate::public::generated::metadata::*;
use crate::public::generated::rest_caller::RestCaller;
use crate::rpc_support::GestaltError;

/// Open enum for `gestalt.provider.v1.WorkflowRunStatus`; unknown numeric values are preserved.
pub type WorkflowRunStatus = i32;

/// Named values of `WorkflowRunStatus`.
pub mod workflow_run_status {
    /// WORKFLOW_RUN_STATUS_UNSPECIFIED.
    pub const WORKFLOW_RUN_STATUS_UNSPECIFIED: i32 = 0;
    /// WORKFLOW_RUN_STATUS_PENDING.
    pub const WORKFLOW_RUN_STATUS_PENDING: i32 = 1;
    /// WORKFLOW_RUN_STATUS_RUNNING.
    pub const WORKFLOW_RUN_STATUS_RUNNING: i32 = 2;
    /// WORKFLOW_RUN_STATUS_SUCCEEDED.
    pub const WORKFLOW_RUN_STATUS_SUCCEEDED: i32 = 3;
    /// WORKFLOW_RUN_STATUS_FAILED.
    pub const WORKFLOW_RUN_STATUS_FAILED: i32 = 4;
    /// WORKFLOW_RUN_STATUS_CANCELED.
    pub const WORKFLOW_RUN_STATUS_CANCELED: i32 = 5;
}

/// Open enum for `gestalt.provider.v1.WorkflowStepStatus`; unknown numeric values are preserved.
pub type WorkflowStepStatus = i32;

/// Named values of `WorkflowStepStatus`.
pub mod workflow_step_status {
    /// WORKFLOW_STEP_STATUS_UNSPECIFIED.
    pub const WORKFLOW_STEP_STATUS_UNSPECIFIED: i32 = 0;
    /// WORKFLOW_STEP_STATUS_PENDING.
    pub const WORKFLOW_STEP_STATUS_PENDING: i32 = 1;
    /// WORKFLOW_STEP_STATUS_RUNNING.
    pub const WORKFLOW_STEP_STATUS_RUNNING: i32 = 2;
    /// WORKFLOW_STEP_STATUS_SKIPPED.
    pub const WORKFLOW_STEP_STATUS_SKIPPED: i32 = 3;
    /// WORKFLOW_STEP_STATUS_SUCCEEDED.
    pub const WORKFLOW_STEP_STATUS_SUCCEEDED: i32 = 4;
    /// WORKFLOW_STEP_STATUS_FAILED.
    pub const WORKFLOW_STEP_STATUS_FAILED: i32 = 5;
    /// WORKFLOW_STEP_STATUS_UNKNOWN.
    pub const WORKFLOW_STEP_STATUS_UNKNOWN: i32 = 6;
}

/// Native message type for `gestalt.provider.v1.ApplyWorkflowProviderDefinitionRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ApplyWorkflowProviderDefinitionRequest {
    /// The `provider` field.
    pub provider: String,
    /// The `spec` field; None when unset.
    pub spec: Option<WorkflowDefinitionSpec>,
    /// The `idempotency_key` field.
    pub idempotency_key: String,
}

/// Native message type for `gestalt.provider.v1.BoundWorkflowTarget`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BoundWorkflowTarget {
    /// The `steps` field.
    pub steps: Vec<WorkflowStep>,
}

/// Native message type for `gestalt.provider.v1.CancelWorkflowProviderRunRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct CancelWorkflowProviderRunRequest {
    /// The `run_id` field.
    pub run_id: String,
    /// The `reason` field.
    pub reason: String,
    /// The `provider` field.
    pub provider: String,
}

/// Native message type for `gestalt.provider.v1.DeleteWorkflowProviderDefinitionRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct DeleteWorkflowProviderDefinitionRequest {
    /// The `definition_id` field.
    pub definition_id: String,
    /// The `provider` field.
    pub provider: String,
}

/// Native message type for `gestalt.provider.v1.GetWorkflowProviderDefinitionRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetWorkflowProviderDefinitionRequest {
    /// The `definition_id` field.
    pub definition_id: String,
    /// The `provider` field.
    pub provider: String,
}

/// Native message type for `gestalt.provider.v1.GetWorkflowProviderRunEventsRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetWorkflowProviderRunEventsRequest {
    /// The `run_id` field.
    pub run_id: String,
    /// The `provider` field.
    pub provider: String,
}

/// Native message type for `gestalt.provider.v1.GetWorkflowProviderRunEventsResponse`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetWorkflowProviderRunEventsResponse {
    /// The `events` field.
    pub events: Vec<WorkflowRunEvent>,
}

/// Native message type for `gestalt.provider.v1.GetWorkflowProviderRunOutputRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetWorkflowProviderRunOutputRequest {
    /// The `run_id` field.
    pub run_id: String,
    /// The `provider` field.
    pub provider: String,
}

/// Native message type for `gestalt.provider.v1.GetWorkflowProviderRunOutputResponse`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetWorkflowProviderRunOutputResponse {
    /// The `output` field; None when unset.
    pub output: Option<serde_json::Value>,
}

/// Native message type for `gestalt.provider.v1.GetWorkflowProviderRunRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetWorkflowProviderRunRequest {
    /// The `run_id` field.
    pub run_id: String,
    /// The `provider` field.
    pub provider: String,
}

/// Native message type for `gestalt.provider.v1.ListWorkflowProviderDefinitionsRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ListWorkflowProviderDefinitionsRequest {
    /// The `provider` field.
    pub provider: String,
}

/// Native message type for `gestalt.provider.v1.ListWorkflowProviderDefinitionsResponse`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ListWorkflowProviderDefinitionsResponse {
    /// The `definitions` field.
    pub definitions: Vec<WorkflowDefinition>,
}

/// Native message type for `gestalt.provider.v1.ListWorkflowProviderRunsRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ListWorkflowProviderRunsRequest {
    /// The `page_size` field.
    pub page_size: i32,
    /// The `page_token` field.
    pub page_token: String,
    /// The `status` field.
    pub status: WorkflowRunStatus,
    /// The `target_app` field.
    pub target_app: String,
    /// The `provider` field.
    pub provider: String,
}

/// Native message type for `gestalt.provider.v1.ListWorkflowProviderRunsResponse`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ListWorkflowProviderRunsResponse {
    /// The `runs` field.
    pub runs: Vec<WorkflowRun>,
    /// The `next_page_token` field.
    pub next_page_token: String,
}

/// Native message type for `gestalt.provider.v1.SetWorkflowProviderActivationPausedRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SetWorkflowProviderActivationPausedRequest {
    /// The `definition_id` field.
    pub definition_id: String,
    /// The `activation_id` field.
    pub activation_id: String,
    /// The `paused` field.
    pub paused: bool,
    /// The `provider` field.
    pub provider: String,
}

/// Native message type for `gestalt.provider.v1.SetWorkflowProviderDefinitionPausedRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SetWorkflowProviderDefinitionPausedRequest {
    /// The `definition_id` field.
    pub definition_id: String,
    /// The `paused` field.
    pub paused: bool,
    /// The `provider` field.
    pub provider: String,
}

/// Native message type for `gestalt.provider.v1.SignalOrStartWorkflowProviderRunRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SignalOrStartWorkflowProviderRunRequest {
    /// The `workflow_key` field.
    pub workflow_key: String,
    /// The `idempotency_key` field.
    pub idempotency_key: String,
    /// The `signal` field; None when unset.
    pub signal: Option<WorkflowSignal>,
    /// The `provider` field.
    pub provider: String,
    /// The `definition_id` field.
    pub definition_id: String,
    /// The `input` field; None when unset.
    pub input: Option<serde_json::Map<String, serde_json::Value>>,
    /// The `expected_definition_generation` field.
    pub expected_definition_generation: i64,
}

/// Native message type for `gestalt.provider.v1.SignalWorkflowProviderRunRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SignalWorkflowProviderRunRequest {
    /// The `run_id` field.
    pub run_id: String,
    /// The `signal` field; None when unset.
    pub signal: Option<WorkflowSignal>,
    /// The `provider` field.
    pub provider: String,
}

/// Native message type for `gestalt.provider.v1.SignalWorkflowRunResponse`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SignalWorkflowRunResponse {
    /// The `run` field; None when unset.
    pub run: Option<WorkflowRun>,
    /// The `signal` field; None when unset.
    pub signal: Option<WorkflowSignal>,
    /// The `started_run` field.
    pub started_run: bool,
    /// The `workflow_key` field.
    pub workflow_key: String,
}

/// Native message type for `gestalt.provider.v1.StartWorkflowProviderRunRequest`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct StartWorkflowProviderRunRequest {
    /// The `idempotency_key` field.
    pub idempotency_key: String,
    /// The `workflow_key` field.
    pub workflow_key: String,
    /// The `provider` field.
    pub provider: String,
    /// The `definition_id` field.
    pub definition_id: String,
    /// The `input` field; None when unset.
    pub input: Option<serde_json::Map<String, serde_json::Value>>,
    /// The `expected_definition_generation` field.
    pub expected_definition_generation: i64,
}

/// Values of the `trigger` oneof in `WorkflowActivation`; the message field is None when unset.
#[allow(clippy::enum_variant_names, clippy::large_enum_variant)]
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
pub enum WorkflowActivationTrigger {
    /// The `schedule` variant.
    Schedule(WorkflowScheduleActivation),
    /// The `event` variant.
    Event(WorkflowEventActivation),
}

/// Native message type for `gestalt.provider.v1.WorkflowActivation`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowActivation {
    /// The `id` field.
    pub id: String,
    /// The `input` field; None when unset.
    pub input: Option<WorkflowValue>,
    /// The `paused` field.
    pub paused: bool,
    /// The `trigger` oneof; None when unset.
    pub trigger: Option<WorkflowActivationTrigger>,
}

/// Native message type for `gestalt.provider.v1.WorkflowAgentMessage`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowAgentMessage {
    /// The `role` field.
    pub role: String,
    /// The `text` field; None when unset.
    pub text: Option<WorkflowText>,
    /// The `metadata` field; None when unset.
    pub metadata: Option<serde_json::Map<String, serde_json::Value>>,
}

/// Native message type for `gestalt.provider.v1.WorkflowArray`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowArray {
    /// The `values` field.
    pub values: Vec<WorkflowValue>,
}

/// Native message type for `gestalt.provider.v1.WorkflowDefinition`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowDefinition {
    /// The `id` field.
    pub id: String,
    /// The `generation` field.
    pub generation: i64,
    /// The `target` field; None when unset.
    pub target: Option<BoundWorkflowTarget>,
    /// The `activations` field.
    pub activations: Vec<WorkflowActivation>,
    /// The `paused` field.
    pub paused: bool,
    /// The `created_at` field; None when unset.
    pub created_at: Option<std::time::SystemTime>,
    /// The `updated_at` field; None when unset.
    pub updated_at: Option<std::time::SystemTime>,
    /// The `provider` field.
    pub provider: String,
    /// The `run_as` field.
    pub run_as: String,
}

/// Native message type for `gestalt.provider.v1.WorkflowDefinitionSpec`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowDefinitionSpec {
    /// The `id` field.
    pub id: String,
    /// The `target` field; None when unset.
    pub target: Option<BoundWorkflowTarget>,
    /// The `activations` field.
    pub activations: Vec<WorkflowActivation>,
    /// The `paused` field.
    pub paused: bool,
    /// The `run_as` field.
    pub run_as: String,
}

/// Native message type for `gestalt.provider.v1.WorkflowEvent`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowEvent {
    /// The `id` field.
    pub id: String,
    /// The `source` field.
    pub source: String,
    /// The `spec_version` field.
    pub spec_version: String,
    /// The `type` field.
    pub r#type: String,
    /// The `subject` field.
    pub subject: String,
    /// The `time` field; None when unset.
    pub time: Option<std::time::SystemTime>,
    /// The `datacontenttype` field.
    pub datacontenttype: String,
    /// The `data` field; None when unset.
    pub data: Option<serde_json::Map<String, serde_json::Value>>,
    /// The `extensions` field.
    pub extensions: std::collections::BTreeMap<String, serde_json::Value>,
}

/// Native message type for `gestalt.provider.v1.WorkflowEventActivation`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowEventActivation {
    /// The `match` field; None when unset.
    pub r#match: Option<WorkflowEventMatch>,
}

/// Native message type for `gestalt.provider.v1.WorkflowEventMatch`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowEventMatch {
    /// The `type` field.
    pub r#type: String,
    /// The `source` field.
    pub source: String,
    /// The `subject` field.
    pub subject: String,
}

/// Native message type for `gestalt.provider.v1.WorkflowEventTriggerInvocation`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowEventTriggerInvocation {
    /// The `activation_id` field.
    pub activation_id: String,
    /// The `event` field; None when unset.
    pub event: Option<WorkflowEvent>,
}

/// Native message type for `gestalt.provider.v1.WorkflowManualTrigger`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowManualTrigger {}

/// Native message type for `gestalt.provider.v1.WorkflowObject`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowObject {
    /// The `fields` field.
    pub fields: std::collections::BTreeMap<String, WorkflowValue>,
}

/// Native message type for `gestalt.provider.v1.WorkflowPathSource`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowPathSource {
    /// The `path` field.
    pub path: String,
}

/// Native message type for `gestalt.provider.v1.WorkflowRun`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowRun {
    /// The `id` field.
    pub id: String,
    /// The `status` field.
    pub status: WorkflowRunStatus,
    /// The `target` field; None when unset.
    pub target: Option<BoundWorkflowTarget>,
    /// The `trigger` field; None when unset.
    pub trigger: Option<WorkflowRunTrigger>,
    /// The `created_at` field; None when unset.
    pub created_at: Option<std::time::SystemTime>,
    /// The `started_at` field; None when unset.
    pub started_at: Option<std::time::SystemTime>,
    /// The `completed_at` field; None when unset.
    pub completed_at: Option<std::time::SystemTime>,
    /// The `status_message` field.
    pub status_message: String,
    /// The `output` field; None when unset.
    pub output: Option<serde_json::Value>,
    /// The `workflow_key` field.
    pub workflow_key: String,
    /// The `provider` field.
    pub provider: String,
    /// The `definition_id` field.
    pub definition_id: String,
    /// The `run_as` field.
    pub run_as: String,
    /// The `input` field; None when unset.
    pub input: Option<serde_json::Map<String, serde_json::Value>>,
    /// The `definition_generation` field.
    pub definition_generation: i64,
    /// The `current_step_id` field.
    pub current_step_id: String,
    /// The `steps` field.
    pub steps: Vec<WorkflowStepExecution>,
}

/// Native message type for `gestalt.provider.v1.WorkflowRunEvent`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowRunEvent {
    /// The `id` field.
    pub id: String,
    /// The `run_id` field.
    pub run_id: String,
    /// The `step_id` field.
    pub step_id: String,
    /// The `type` field.
    pub r#type: String,
    /// The `data` field; None when unset.
    pub data: Option<serde_json::Map<String, serde_json::Value>>,
    /// The `created_at` field; None when unset.
    pub created_at: Option<std::time::SystemTime>,
}

/// Values of the `kind` oneof in `WorkflowRunTrigger`; the message field is None when unset.
#[allow(clippy::enum_variant_names, clippy::large_enum_variant)]
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
pub enum WorkflowRunTriggerKind {
    /// The `manual` variant.
    Manual(WorkflowManualTrigger),
    /// The `schedule` variant.
    Schedule(WorkflowScheduleTrigger),
    /// The `event` variant.
    Event(WorkflowEventTriggerInvocation),
}

/// Native message type for `gestalt.provider.v1.WorkflowRunTrigger`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowRunTrigger {
    /// The `kind` oneof; None when unset.
    pub kind: Option<WorkflowRunTriggerKind>,
}

/// Native message type for `gestalt.provider.v1.WorkflowScheduleActivation`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowScheduleActivation {
    /// The `cron` field.
    pub cron: String,
    /// The `timezone` field.
    pub timezone: String,
}

/// Native message type for `gestalt.provider.v1.WorkflowScheduleTrigger`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowScheduleTrigger {
    /// The `activation_id` field.
    pub activation_id: String,
    /// The `scheduled_for` field; None when unset.
    pub scheduled_for: Option<std::time::SystemTime>,
}

/// Native message type for `gestalt.provider.v1.WorkflowSignal`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowSignal {
    /// The `id` field.
    pub id: String,
    /// The `name` field.
    pub name: String,
    /// The `payload` field; None when unset.
    pub payload: Option<serde_json::Map<String, serde_json::Value>>,
    /// The `metadata` field; None when unset.
    pub metadata: Option<serde_json::Map<String, serde_json::Value>>,
    /// The `created_at` field; None when unset.
    pub created_at: Option<std::time::SystemTime>,
    /// The `idempotency_key` field.
    pub idempotency_key: String,
    /// The `sequence` field.
    pub sequence: i64,
}

/// Values of the `action` oneof in `WorkflowStep`; the message field is None when unset.
#[allow(clippy::enum_variant_names, clippy::large_enum_variant)]
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
pub enum WorkflowStepAction {
    /// The `app` variant.
    App(WorkflowStepAppCall),
    /// The `agent` variant.
    Agent(WorkflowStepAgentTurn),
}

/// Native message type for `gestalt.provider.v1.WorkflowStep`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowStep {
    /// The `id` field.
    pub id: String,
    /// The `inputs` field.
    pub inputs: std::collections::BTreeMap<String, WorkflowValue>,
    /// The `when` field; None when unset.
    pub when: Option<WorkflowStepWhen>,
    /// The `timeout_seconds` field.
    pub timeout_seconds: i32,
    /// The `metadata` field; None when unset.
    pub metadata: Option<serde_json::Map<String, serde_json::Value>>,
    /// The `action` oneof; None when unset.
    pub action: Option<WorkflowStepAction>,
}

/// Native message type for `gestalt.provider.v1.WorkflowStepAgentTurn`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowStepAgentTurn {
    /// The `provider` field.
    pub provider: String,
    /// The `model` field.
    pub model: String,
    /// The `session_key` field.
    pub session_key: String,
    /// The `prompt` field; None when unset.
    pub prompt: Option<WorkflowText>,
    /// The `messages` field.
    pub messages: Vec<WorkflowAgentMessage>,
    /// The `tools` field.
    pub tools: Vec<AgentToolRef>,
    /// The `output` field; None when unset.
    pub output: Option<AgentOutput>,
    /// The `model_options` field; None when unset.
    pub model_options: Option<serde_json::Map<String, serde_json::Value>>,
}

/// Native message type for `gestalt.provider.v1.WorkflowStepAppCall`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowStepAppCall {
    /// The `name` field.
    pub name: String,
    /// The `operation` field.
    pub operation: String,
    /// The `input` field; None when unset.
    pub input: Option<WorkflowValue>,
    /// The `connection` field.
    pub connection: String,
    /// The `instance` field.
    pub instance: String,
    /// The `credential_mode` field.
    pub credential_mode: String,
}

/// Native message type for `gestalt.provider.v1.WorkflowStepAttempt`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowStepAttempt {
    /// The `id` field.
    pub id: String,
    /// The `status` field.
    pub status: WorkflowStepStatus,
    /// The `idempotency_key` field.
    pub idempotency_key: String,
    /// The `input` field; None when unset.
    pub input: Option<serde_json::Value>,
    /// The `output` field; None when unset.
    pub output: Option<serde_json::Value>,
    /// The `status_message` field.
    pub status_message: String,
    /// The `started_at` field; None when unset.
    pub started_at: Option<std::time::SystemTime>,
    /// The `completed_at` field; None when unset.
    pub completed_at: Option<std::time::SystemTime>,
}

/// Native message type for `gestalt.provider.v1.WorkflowStepExecution`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowStepExecution {
    /// The `step_id` field.
    pub step_id: String,
    /// The `status` field.
    pub status: WorkflowStepStatus,
    /// The `attempts` field.
    pub attempts: Vec<WorkflowStepAttempt>,
    /// The `input` field; None when unset.
    pub input: Option<serde_json::Value>,
    /// The `output` field; None when unset.
    pub output: Option<serde_json::Value>,
    /// The `status_message` field.
    pub status_message: String,
    /// The `skip_reason` field.
    pub skip_reason: String,
    /// The `started_at` field; None when unset.
    pub started_at: Option<std::time::SystemTime>,
    /// The `completed_at` field; None when unset.
    pub completed_at: Option<std::time::SystemTime>,
}

/// Native message type for `gestalt.provider.v1.WorkflowStepInputSource`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowStepInputSource {
    /// The `step_id` field.
    pub step_id: String,
    /// The `path` field.
    pub path: String,
}

/// Native message type for `gestalt.provider.v1.WorkflowStepOutputSource`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowStepOutputSource {
    /// The `step_id` field.
    pub step_id: String,
    /// The `path` field.
    pub path: String,
}

/// Native message type for `gestalt.provider.v1.WorkflowStepWhen`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowStepWhen {
    /// The `value` field; None when unset.
    pub value: Option<WorkflowValue>,
    /// The `equals` field; None when unset.
    pub equals: Option<serde_json::Value>,
}

/// Native message type for `gestalt.provider.v1.WorkflowText`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowText {
    /// The `template` field.
    pub template: String,
}

/// Values of the `kind` oneof in `WorkflowValue`; the message field is None when unset.
#[allow(clippy::enum_variant_names, clippy::large_enum_variant)]
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
pub enum WorkflowValueKind {
    /// The `literal` variant.
    Literal(serde_json::Value),
    /// The `object` variant.
    Object(WorkflowObject),
    /// The `array` variant.
    Array(WorkflowArray),
    /// The `template` variant.
    Template(WorkflowText),
    /// The `input` variant.
    Input(WorkflowPathSource),
    /// The `signal` variant.
    Signal(WorkflowPathSource),
    /// The `step_output` variant.
    StepOutput(WorkflowStepOutputSource),
    /// The `step_input` variant.
    StepInput(WorkflowStepInputSource),
}

/// Native message type for `gestalt.provider.v1.WorkflowValue`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowValue {
    /// The `kind` oneof; None when unset.
    pub kind: Option<WorkflowValueKind>,
}

/// gRPC client for the public gestaltd surface.
pub struct WorkflowGRPC {
    inner: v1::workflow_client::WorkflowClient<crate::public::grpc_auth::AuthChannel>,
    timeout: Option<std::time::Duration>,
    request_context: Option<v1::RequestContext>,
}

impl WorkflowGRPC {
    /// Creates a client over an injected tonic channel.
    pub fn new(
        channel: tonic::transport::Channel,
        auth: std::sync::Arc<dyn crate::public::auth::Auth>,
    ) -> Self {
        Self::with_request_context(channel, auth, None)
    }

    /// Creates a client that injects request context into outgoing RPCs.
    pub fn with_request_context(
        channel: tonic::transport::Channel,
        auth: std::sync::Arc<dyn crate::public::auth::Auth>,
        request_context: Option<v1::RequestContext>,
    ) -> Self {
        Self {
            inner: v1::workflow_client::WorkflowClient::with_interceptor(
                channel,
                crate::public::grpc_auth::AuthInterceptor::new(auth),
            ),
            timeout: None,
            request_context,
        }
    }

    /// Sets the default per-request timeout.
    pub fn with_timeout(mut self, timeout: std::time::Duration) -> Self {
        self.timeout = Some(timeout);
        self
    }

    pub async fn apply_definition(
        &mut self,
        request: ApplyWorkflowProviderDefinitionRequest,
    ) -> Result<WorkflowDefinition, GestaltError> {
        let mut wire_request = to_wire_apply_workflow_provider_definition_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let response = self
            .inner
            .apply_definition(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        Ok(from_wire_workflow_definition(response))
    }

    pub async fn get_definition(
        &mut self,
        request: GetWorkflowProviderDefinitionRequest,
    ) -> Result<WorkflowDefinition, GestaltError> {
        let mut wire_request = to_wire_get_workflow_provider_definition_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let response = self
            .inner
            .get_definition(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        Ok(from_wire_workflow_definition(response))
    }

    pub async fn list_definitions(
        &mut self,
        request: ListWorkflowProviderDefinitionsRequest,
    ) -> Result<Vec<WorkflowDefinition>, GestaltError> {
        let mut wire_request = to_wire_list_workflow_provider_definitions_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let wire = self
            .inner
            .list_definitions(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        let response = from_wire_list_workflow_provider_definitions_response(wire);
        Ok(response.definitions)
    }

    pub async fn set_definition_paused(
        &mut self,
        request: SetWorkflowProviderDefinitionPausedRequest,
    ) -> Result<WorkflowDefinition, GestaltError> {
        let mut wire_request = to_wire_set_workflow_provider_definition_paused_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let response = self
            .inner
            .set_definition_paused(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        Ok(from_wire_workflow_definition(response))
    }

    pub async fn set_activation_paused(
        &mut self,
        request: SetWorkflowProviderActivationPausedRequest,
    ) -> Result<WorkflowDefinition, GestaltError> {
        let mut wire_request = to_wire_set_workflow_provider_activation_paused_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let response = self
            .inner
            .set_activation_paused(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        Ok(from_wire_workflow_definition(response))
    }

    pub async fn delete_definition(
        &mut self,
        request: DeleteWorkflowProviderDefinitionRequest,
    ) -> Result<(), GestaltError> {
        let mut wire_request = to_wire_delete_workflow_provider_definition_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        self.inner
            .delete_definition(tonic_request)
            .await
            .map_err(GestaltError::from)?;
        Ok(())
    }

    pub async fn start_run(
        &mut self,
        request: StartWorkflowProviderRunRequest,
    ) -> Result<WorkflowRun, GestaltError> {
        let mut wire_request = to_wire_start_workflow_provider_run_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let response = self
            .inner
            .start_run(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        Ok(from_wire_workflow_run(response))
    }

    pub async fn list_runs(
        &mut self,
        request: ListWorkflowProviderRunsRequest,
    ) -> Result<ListWorkflowProviderRunsResponse, GestaltError> {
        let mut wire_request = to_wire_list_workflow_provider_runs_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let response = self
            .inner
            .list_runs(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        Ok(from_wire_list_workflow_provider_runs_response(response))
    }

    pub async fn get_run(
        &mut self,
        request: GetWorkflowProviderRunRequest,
    ) -> Result<WorkflowRun, GestaltError> {
        let mut wire_request = to_wire_get_workflow_provider_run_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let response = self
            .inner
            .get_run(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        Ok(from_wire_workflow_run(response))
    }

    pub async fn get_run_events(
        &mut self,
        request: GetWorkflowProviderRunEventsRequest,
    ) -> Result<Vec<WorkflowRunEvent>, GestaltError> {
        let mut wire_request = to_wire_get_workflow_provider_run_events_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let wire = self
            .inner
            .get_run_events(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        let response = from_wire_get_workflow_provider_run_events_response(wire);
        Ok(response.events)
    }

    pub async fn get_run_output(
        &mut self,
        request: GetWorkflowProviderRunOutputRequest,
    ) -> Result<Option<serde_json::Value>, GestaltError> {
        let mut wire_request = to_wire_get_workflow_provider_run_output_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let wire = self
            .inner
            .get_run_output(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        let response = from_wire_get_workflow_provider_run_output_response(wire);
        Ok(response.output)
    }

    pub async fn cancel_run(
        &mut self,
        request: CancelWorkflowProviderRunRequest,
    ) -> Result<WorkflowRun, GestaltError> {
        let mut wire_request = to_wire_cancel_workflow_provider_run_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let response = self
            .inner
            .cancel_run(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        Ok(from_wire_workflow_run(response))
    }

    pub async fn signal_run(
        &mut self,
        request: SignalWorkflowProviderRunRequest,
    ) -> Result<SignalWorkflowRunResponse, GestaltError> {
        let mut wire_request = to_wire_signal_workflow_provider_run_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let response = self
            .inner
            .signal_run(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        Ok(from_wire_signal_workflow_run_response(response))
    }

    pub async fn signal_or_start_run(
        &mut self,
        request: SignalOrStartWorkflowProviderRunRequest,
    ) -> Result<SignalWorkflowRunResponse, GestaltError> {
        let mut wire_request = to_wire_signal_or_start_workflow_provider_run_request(request);
        if wire_request.context.is_none() {
            wire_request.context = self.request_context.clone();
        }
        let mut tonic_request = tonic::Request::new(wire_request);
        if let Some(timeout) = self.timeout {
            tonic_request.set_timeout(timeout);
        }
        let response = self
            .inner
            .signal_or_start_run(tonic_request)
            .await
            .map_err(GestaltError::from)?
            .into_inner();
        Ok(from_wire_signal_workflow_run_response(response))
    }
}

pub struct WorkflowREST {
    transport: std::sync::Arc<crate::public::rest_transport::RestTransport>,
}

impl WorkflowREST {
    pub fn new(transport: std::sync::Arc<crate::public::rest_transport::RestTransport>) -> Self {
        Self { transport }
    }

    pub fn apply_definition(
        &self,
        request: ApplyWorkflowProviderDefinitionRequest,
    ) -> Result<WorkflowDefinition, GestaltError> {
        let wire_request = to_wire_apply_workflow_provider_definition_request(request);
        let request_json =
            encode_wire_apply_workflow_provider_definition_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_APPLY_DEFINITION,
            request_json,
            &mut response_json,
        )?;
        let wire_response = decode_wire_workflow_definition_json(&response_json)?;
        let response = from_wire_workflow_definition(wire_response);
        Ok(response)
    }

    pub fn get_definition(
        &self,
        request: GetWorkflowProviderDefinitionRequest,
    ) -> Result<WorkflowDefinition, GestaltError> {
        let wire_request = to_wire_get_workflow_provider_definition_request(request);
        let request_json = encode_wire_get_workflow_provider_definition_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_GET_DEFINITION,
            request_json,
            &mut response_json,
        )?;
        let wire_response = decode_wire_workflow_definition_json(&response_json)?;
        let response = from_wire_workflow_definition(wire_response);
        Ok(response)
    }

    pub fn list_definitions(
        &self,
        request: ListWorkflowProviderDefinitionsRequest,
    ) -> Result<Vec<WorkflowDefinition>, GestaltError> {
        let wire_request = to_wire_list_workflow_provider_definitions_request(request);
        let request_json =
            encode_wire_list_workflow_provider_definitions_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_LIST_DEFINITIONS,
            request_json,
            &mut response_json,
        )?;
        let wire_response =
            decode_wire_list_workflow_provider_definitions_response_json(&response_json)?;
        let response = from_wire_list_workflow_provider_definitions_response(wire_response);
        Ok(response.definitions)
    }

    pub fn set_definition_paused(
        &self,
        request: SetWorkflowProviderDefinitionPausedRequest,
    ) -> Result<WorkflowDefinition, GestaltError> {
        let wire_request = to_wire_set_workflow_provider_definition_paused_request(request);
        let request_json =
            encode_wire_set_workflow_provider_definition_paused_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_SET_DEFINITION_PAUSED,
            request_json,
            &mut response_json,
        )?;
        let wire_response = decode_wire_workflow_definition_json(&response_json)?;
        let response = from_wire_workflow_definition(wire_response);
        Ok(response)
    }

    pub fn set_activation_paused(
        &self,
        request: SetWorkflowProviderActivationPausedRequest,
    ) -> Result<WorkflowDefinition, GestaltError> {
        let wire_request = to_wire_set_workflow_provider_activation_paused_request(request);
        let request_json =
            encode_wire_set_workflow_provider_activation_paused_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_SET_ACTIVATION_PAUSED,
            request_json,
            &mut response_json,
        )?;
        let wire_response = decode_wire_workflow_definition_json(&response_json)?;
        let response = from_wire_workflow_definition(wire_response);
        Ok(response)
    }

    pub fn delete_definition(
        &self,
        request: DeleteWorkflowProviderDefinitionRequest,
    ) -> Result<(), GestaltError> {
        let wire_request = to_wire_delete_workflow_provider_definition_request(request);
        let request_json =
            encode_wire_delete_workflow_provider_definition_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_DELETE_DEFINITION,
            request_json,
            &mut response_json,
        )?;
        Ok(())
    }

    pub fn start_run(
        &self,
        request: StartWorkflowProviderRunRequest,
    ) -> Result<WorkflowRun, GestaltError> {
        let wire_request = to_wire_start_workflow_provider_run_request(request);
        let request_json = encode_wire_start_workflow_provider_run_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_START_RUN,
            request_json,
            &mut response_json,
        )?;
        let wire_response = decode_wire_workflow_run_json(&response_json)?;
        let response = from_wire_workflow_run(wire_response);
        Ok(response)
    }

    pub fn list_runs(
        &self,
        request: ListWorkflowProviderRunsRequest,
    ) -> Result<ListWorkflowProviderRunsResponse, GestaltError> {
        let wire_request = to_wire_list_workflow_provider_runs_request(request);
        let request_json = encode_wire_list_workflow_provider_runs_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_LIST_RUNS,
            request_json,
            &mut response_json,
        )?;
        let wire_response = decode_wire_list_workflow_provider_runs_response_json(&response_json)?;
        let response = from_wire_list_workflow_provider_runs_response(wire_response);
        Ok(response)
    }

    pub fn get_run(
        &self,
        request: GetWorkflowProviderRunRequest,
    ) -> Result<WorkflowRun, GestaltError> {
        let wire_request = to_wire_get_workflow_provider_run_request(request);
        let request_json = encode_wire_get_workflow_provider_run_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_GET_RUN,
            request_json,
            &mut response_json,
        )?;
        let wire_response = decode_wire_workflow_run_json(&response_json)?;
        let response = from_wire_workflow_run(wire_response);
        Ok(response)
    }

    pub fn get_run_events(
        &self,
        request: GetWorkflowProviderRunEventsRequest,
    ) -> Result<Vec<WorkflowRunEvent>, GestaltError> {
        let wire_request = to_wire_get_workflow_provider_run_events_request(request);
        let request_json = encode_wire_get_workflow_provider_run_events_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_GET_RUN_EVENTS,
            request_json,
            &mut response_json,
        )?;
        let wire_response =
            decode_wire_get_workflow_provider_run_events_response_json(&response_json)?;
        let response = from_wire_get_workflow_provider_run_events_response(wire_response);
        Ok(response.events)
    }

    pub fn get_run_output(
        &self,
        request: GetWorkflowProviderRunOutputRequest,
    ) -> Result<Option<serde_json::Value>, GestaltError> {
        let wire_request = to_wire_get_workflow_provider_run_output_request(request);
        let request_json = encode_wire_get_workflow_provider_run_output_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_GET_RUN_OUTPUT,
            request_json,
            &mut response_json,
        )?;
        let wire_response =
            decode_wire_get_workflow_provider_run_output_response_json(&response_json)?;
        let response = from_wire_get_workflow_provider_run_output_response(wire_response);
        Ok(response.output)
    }

    pub fn cancel_run(
        &self,
        request: CancelWorkflowProviderRunRequest,
    ) -> Result<WorkflowRun, GestaltError> {
        let wire_request = to_wire_cancel_workflow_provider_run_request(request);
        let request_json = encode_wire_cancel_workflow_provider_run_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_CANCEL_RUN,
            request_json,
            &mut response_json,
        )?;
        let wire_response = decode_wire_workflow_run_json(&response_json)?;
        let response = from_wire_workflow_run(wire_response);
        Ok(response)
    }

    pub fn signal_run(
        &self,
        request: SignalWorkflowProviderRunRequest,
    ) -> Result<SignalWorkflowRunResponse, GestaltError> {
        let wire_request = to_wire_signal_workflow_provider_run_request(request);
        let request_json = encode_wire_signal_workflow_provider_run_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_SIGNAL_RUN,
            request_json,
            &mut response_json,
        )?;
        let wire_response = decode_wire_signal_workflow_run_response_json(&response_json)?;
        let response = from_wire_signal_workflow_run_response(wire_response);
        Ok(response)
    }

    pub fn signal_or_start_run(
        &self,
        request: SignalOrStartWorkflowProviderRunRequest,
    ) -> Result<SignalWorkflowRunResponse, GestaltError> {
        let wire_request = to_wire_signal_or_start_workflow_provider_run_request(request);
        let request_json =
            encode_wire_signal_or_start_workflow_provider_run_request_json(&wire_request);
        let mut response_json = serde_json::Value::Null;
        RestCaller::call_unary(
            self.transport.as_ref(),
            &METHOD_WORKFLOW_SIGNAL_OR_START_RUN,
            request_json,
            &mut response_json,
        )?;
        let wire_response = decode_wire_signal_workflow_run_response_json(&response_json)?;
        let response = from_wire_signal_workflow_run_response(wire_response);
        Ok(response)
    }
}
