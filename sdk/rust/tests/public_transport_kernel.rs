...
struct TransportKernelCase {
    ...
    #[serde(rename = "expectPrepareError")]
    expect_prepare_error: Option<GestaltErrorExpect>,
}
...
fn fixture_cases_are_covered() {
    for case in load_cases() {
        let covered = case.expect_prepare.is_some()
            ...
            || case.expect_prepare_error.is_some();
...
fn prepare_cases_from_fixture() {
    for case in load_cases() {
        ...
        let query = build_rest_query(&method, request).expect(&case.id);
...
#[test]
fn prepare_error_cases_from_fixture() {
    for case in load_cases() {
        let Some(ref expect) = case.expect_prepare_error else {
            continue;
        };
        let Some(ref request) = case.request else {
            continue;
        ...
        let method = method_for_prepare_case(&case);
        let err = build_rest_query(&method, request).expect_err(&case.id);
