...
| Host-service clients | [`Cache`], [`S3`], [`WorkflowHost`], [`Workflow`], [`AgentHost`], [`Agent`], [`App`] | Calling sibling services exposed to a provider process by `gestaltd`. |
...
`S3`, `WorkflowHost`, `Workflow`, `AgentHost`, `Agent`, and
`App`.
...
- `Cache`, `S3`, `WorkflowHost`, `Workflow`, `AgentHost`,
  `Agent`, and `App` call sibling host services.
