...
func TestBuildParsesAnnotations(t *testing.T) {
	...
	if got := methods["Fetch"].Signature; !reflect.DeepEqual(got, []string{"id"}) {
		t.Errorf("Fetch signature = %v, want [id]", got)
	}
	if got := methods["Fetch"].OptionalSignature; !reflect.DeepEqual(got, []string{"limit"}) {
		t.Errorf("Fetch optional_signature = %v, want [limit]", got)
	}
	if got := methods["Read"].Initial; !reflect.DeepEqual(got, &model.Initial{Oneof: "body", HeaderField: "header", ChunkField: "data"}) {
	...
	if got := methods["Call"].JsonResult; !reflect.DeepEqual(got, &model.JsonResult{Status: "status", Body: "body"}) {
		t.Errorf("Call json_result = %+v", got)
	}
}
...
func TestBuildRejectsBadJsonResult(t *testing.T) {
	...
	all := diags.All()
	wants := []string{
		`json_result status "status" must be a plain int32 field`,
		`json_result body "body" must be a bytes field`,
		"json_result annotation requires a unary method",
		`optional_signature field "id" repeats the signature list`,
		`optional_signature names unknown request field "nope"`,
		"already carries a response collapse",
	}
	if len(all) != len(wants) {
		t.Fatalf("diagnostics = %d, want %d:\n%v", len(all), len(wants), diags.Err())
	}
