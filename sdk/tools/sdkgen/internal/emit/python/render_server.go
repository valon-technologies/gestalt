...
type serverRenderer struct {
	...
	needsClientStream bool
	needsServerStream bool
...
func (r *serverRenderer) handlerParams(m *model.Method) (params, retType string) {
	input, output := "", ""
	if !m.InputIsEmpty {
		input = "_native." + localName(m.Input.FullName)
	}
	if !m.OutputIsEmpty {
		output = "_native." + localName(m.Output.FullName)
	}
	switch m.Stream {
	case model.ServerStream:
		r.needsServerStream = true
		if input != "" {
			params = "request: " + input + ", stream: ServerStream[" + output + "]"
		} else {
			params = "stream: ServerStream[" + output + "]"
		}
		retType = "None"
	case model.ClientStream:
		r.needsClientStream = true
...
func (r *serverRenderer) providerFieldParams(m *model.Method) string {
	...
	for _, name := range append(append([]string{}, m.Signature...), m.OptionalSignature...) {
		...
		param := pyName(field.Name) + ": " + r.providerFieldType(field)
		if contains(m.OptionalSignature, name) {
			param += " | None = None"
		}
...
func (r *serverRenderer) providerFieldType(field *model.Field) string {
	...
	case model.KindRepeated:
		typ = "list[" + r.providerRefType(field.Elem) + "]"
	case model.KindMap:
		typ = "dict[" + scalarType(field.MapKey) + ", " + r.providerRefType(field.MapValue) + "]"
	default:
...
func (r *serverRenderer) providerRefType(ref *model.TypeRef) string {
	...
	case model.KindBytes:
		return "bytes"
	case model.KindEnum:
		return "_native." + localName(ref.Enum)
	case model.KindMessage:
		return "_native." + localName(ref.Message)
	case model.KindJSONStruct, model.KindJSONValue:
		r.needsAny = true
		return "Any"
	case model.KindJSONNull, model.KindUnit:
		return "None"
	case model.KindTimestamp:
		r.needsDatetime = true
		return "datetime.datetime"
	case model.KindDuration:
		r.needsDatetime = true
		return "datetime.timedelta"
	case model.KindRPCStatus:
		return "_native.RpcStatus"
	default:
		return "Any"
	}
}
...
func (r *serverRenderer) providerFieldArgs(m *model.Method, request string) string {
	...
	for _, name := range append(append([]string{}, m.Signature...), m.OptionalSignature...) {
		field := findField(m.Input, name)
		if field != nil {
			args = append(args, request+"."+pyName(field.Name))
		}
	}
...
func (r *serverRenderer) renderDispatchMethod(m *model.Method, op, codecMod string) {
	...
	if m.Stream == model.ServerStream {
		...
		outType := "Any"
		if !m.OutputIsEmpty {
			outType = "_native." + localName(m.Output.FullName)
		}
		fmt.Fprintf(&r.body, "        values: list[%s] = []\n", outType)
		if m.InputIsEmpty {
			fmt.Fprintf(&r.body, "        native_stream = ServerStream(values.append)\n")
		} else {
			fmt.Fprintf(&r.body, "        native_req = %s.%s(request)\n        native_stream = ServerStream(values.append)\n", codecMod, fromWireFunc(m.Input.FullName))
		}
		fmt.Fprintf(&r.body, "        try:\n            self._handler.%s(%s)\n        except Exception as err:\n            return status_error(context, %q, err)\n", snakeCase(m.Name), func() string {
			if m.InputIsEmpty {
				return "native_stream"
			}
			return "native_req, native_stream"
		}(), op)
		if m.OutputIsEmpty {
			r.body.WriteString("        return iter(())\n\n")
		} else {
			fmt.Fprintf(&r.body, "        return (_codec.%s(value) for value in values)\n\n", toWireFunc(m.Output.FullName))
		}
		return
	}
	if m.Stream == model.ClientStream || m.Stream == model.Bidi {
		if m.Stream == model.ClientStream {
			fmt.Fprintf(&r.body, "    def %s(self, request_iterator: Any, context: grpc.ServicerContext) -> Any:\n", methodName)
		} else {
			fmt.Fprintf(&r.body, "    def %s(self, request_iterator: Any, context: grpc.ServicerContext) -> Any:\n", methodName)
		}
		inFunc := fromWireFunc(m.Input.FullName)
		fmt.Fprintf(&r.body, "        native_requests = (%s.%s(frame) for frame in request_iterator)\n", codecMod, inFunc)
		if m.Stream == model.ClientStream {
			fmt.Fprintf(&r.body, "        native_stream = ClientStream(native_requests)\n        try:\n            response = self._handler.%s(native_stream)\n        except Exception as err:\n            return status_error(context, %q, err)\n", snakeCase(m.Name), op)
			if m.OutputIsEmpty {
				r.needsEmptyPb = true
				r.body.WriteString("        return _empty.Empty()\n\n")
			} else {
				fmt.Fprintf(&r.body, "        return %s.%s(response)\n\n", codecMod, toWireFunc(m.Output.FullName))
			}
		} else {
			outType := "Any"
			if !m.OutputIsEmpty {
			...
			fmt.Fprintf(&r.body, "        values: list[%s] = []\n        native_stream = BidiStream(native_requests, values.append)\n        try:\n            self._handler.%s(native_stream)\n        except Exception as err:\n            return status_error(context, %q, err)\n", outType, snakeCase(m.Name), op)
			if m.OutputIsEmpty {
				r.body.WriteString("        return iter(())\n\n")
			} else {
				fmt.Fprintf(&r.body, "        return (_codec.%s(value) for value in values)\n\n", toWireFunc(m.Output.FullName))
	...
	if m.InputIsEmpty {
		...
		callArgs := "native_req"
		if m.ProviderInput == model.ProviderInputClientSignature {
			callArgs = r.providerFieldArgs(m, "native_req")
		}
		if m.OutputIsEmpty {
			r.needsEmptyPb = true
			fmt.Fprintf(&r.body, "        native_req = %s.%s(request)\n        try:\n            self._handler.%s(%s)\n        except Exception as err:\n            status_error(context, %q, err)\n            return\n        return _empty.Empty()\n\n", codecMod, inFunc, snakeCase(m.Name), callArgs, op)
		} else {
			outFunc := toWireFunc(m.Output.FullName)
			fmt.Fprintf(&r.body, "        native_req = %s.%s(request)\n        try:\n            response = self._handler.%s(%s)\n        except Exception as err:\n            return status_error(context, %q, err)\n        return %s.%s(response)\n\n", codecMod, inFunc, snakeCase(m.Name), callArgs, op, codecMod, outFunc)
		}
	}
}
...
func (r *serverRenderer) assembleServer(base string) string {
	...
	if r.needsAny {
		stdlib = append(stdlib, fromImport("typing", []string{"Any"}))
	}
	if len(stdlib) > 0 {
		b.WriteString("\n")
	...
	if r.needsBidiStream {
		streamNames = append(streamNames, "BidiStream")
	}
	if r.needsClientStream {
		streamNames = append(streamNames, "ClientStream")
	}
	if r.needsServerStream {
		streamNames = append(streamNames, "ServerStream")
	}
	streamNames = append(streamNames, "status_error")
	b.WriteString(fromImport(".support", streamNames))
	if r.needsEmptyPb {
