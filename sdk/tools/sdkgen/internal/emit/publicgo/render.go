...
func (r *renderer) valueType(ref *model.TypeRef) string {
	...
	case model.KindRPCStatus:
		return "*RpcStatus"
	default:
		panic(fmt.Sprintf("publicgo: no value type for kind %d", ref.Kind))
	}
}
...
func (r *renderer) fieldType(f *model.Field) string {
	...
	case model.KindRPCStatus:
		return "*RpcStatus"
	default:
		panic(fmt.Sprintf("publicgo: no field type for kind %d (field %s)", f.Kind, f.Name))
	}
}
...
func (r *renderer) wireValueType(ref *model.TypeRef) string {
	...
	default:
		panic(fmt.Sprintf("publicgo: no wire value type for kind %d", ref.Kind))
	}
}
...
func (r *renderer) valueToWire(ref *model.TypeRef, expr string) string {
	...
	default:
		panic(fmt.Sprintf("publicgo: no wire conversion for kind %d", ref.Kind))
	}
}
...
func (r *renderer) valueFromWire(ref *model.TypeRef, expr string) string {
	...
	default:
		panic(fmt.Sprintf("publicgo: no native conversion for kind %d", ref.Kind))
	}
}
...
func (r *renderer) fieldToWire(f *model.Field) conversionParts {
	...
	default:
		panic(fmt.Sprintf("publicgo: unsupported singular field kind %d (field %s)", f.Kind, f.Name))
	}
}
...
func (r *renderer) fieldConvToWire(f *model.Field, expr string) string {
	...
	case model.KindRPCStatus:
		return "toWireStatus(" + expr + ")"
	default:
		panic(fmt.Sprintf("publicgo: no field converter for kind %d", f.Kind))
	}
}
...
func (r *renderer) renderGRPCClient(svc *model.Service) {
	...
	doc := fmt.Sprintf("%s is the generated gRPC client for the public %s surface.\nEvery transport error is converted to *GestaltError.", name, svc.FullName)
	...
	fmt.Fprintf(&r.body, "// New%sGRPC creates a %s over an injected gRPC connection.\n", wireName, name)
	fmt.Fprintf(&r.body, "func New%sGRPC(conn grpc.ClientConnInterface) *%s {\n", wireName, name)
	fmt.Fprintf(&r.body, "\treturn &%s{client: proto.New%sClient(conn)}\n}\n\n", name, wireName)
	if contextFieldOf(svc) != nil {
		fmt.Fprintf(&r.body, "// New%sGRPCWithContext creates a %s that injects request context into outgoing RPCs.\n", wireName, name)
		fmt.Fprintf(&r.body, "func New%sGRPCWithContext(conn grpc.ClientConnInterface, context *proto.RequestContext) *%s {\n", wireName, name)
		fmt.Fprintf(&r.body, "\treturn &%s{client: proto.New%sClient(conn), context: context}\n}\n\n", name, wireName)
	}

	for _, method := range svc.Methods {
		r.renderGRPCMethod(name, method)
	}
}
...
func (r *renderer) renderRESTClient(svc *model.Service) {
	...
	doc := fmt.Sprintf("%s is the generated REST client for the public %s surface.\nEvery transport error is converted to *GestaltError.", name, svc.FullName)
	r.writeIdentDoc("", doc, svc.Doc)
	fmt.Fprintf(&r.body, "type %s struct {\n\ttransport RESTCaller\n}\n\n", name)
	fmt.Fprintf(&r.body, "// New%sREST creates a %s over the given REST transport.\n", wireName, name)
	fmt.Fprintf(&r.body, "func New%sREST(transport RESTCaller) *%s {\n", wireName, name)
	...
	for _, method := range svc.Methods {
		...
		r.renderRESTMethod(name, wireName, method)
	}
}

func (r *renderer) renderGRPCMethod(svcName string, m *model.Method) {
	r.renderMethod(svcName, m)
}

func (r *renderer) renderRESTMethod(svcName, wireName string, m *model.Method) {
	constName := fmt.Sprintf("Method%s%s", wireName, m.Name)
	params, arg, prep := r.methodRequestREST(m)
	...
	case m.OutputIsEmpty:
		...
		fmt.Fprintf(&r.body, "\tif _, err := c.transport.CallUnary(ctx, %s, %s, nil); err != nil {\n", constName, arg)
	...
	case collapse != nil:
		...
		fmt.Fprintf(&r.body, "\tresponse, err := c.transport.CallUnary(ctx, %s, %s, &%s{})\n", constName, arg, wireMessage(m.Output.FullName))
		fmt.Fprintf(&r.body, "\tif err != nil {\n\t\treturn %s, toGestaltError(err)\n\t}\n", strings.Join(collapse.zero, ", "))
		fmt.Fprintf(&r.body, "\twire := response.(*%s)\n", wireMessage(m.Output.FullName))
		restCollapse := *collapse
		for i, line := range restCollapse.lines {
			restCollapse.lines[i] = strings.ReplaceAll(line, "response", "wire")
		}
		for _, line := range restCollapse.lines {
			r.body.WriteString("\t" + strings.ReplaceAll(line, fromWireFunc(responseType)+"(response)", fromWireFunc(responseType)+"(wire)") + "\n")
		}
		r.body.WriteString("}\n\n")
	default:
		responseType := r.messageType(m.Output.FullName)
		...
		fmt.Fprintf(&r.body, "\treturn %s(response.(*%s)), nil\n}\n\n", fromWireFunc(responseType), wireMessage(m.Output.FullName))
	}
}

func (r *renderer) methodRequestREST(m *model.Method) (param, arg string, prep []string) {
	if m.InputIsEmpty {
		r.features.emptypb = true
		return "", "nil", nil
	}
	requestType := r.messageType(m.Input.FullName)
	return ", request *" + requestType, toWireFunc(requestType) + "(request)", nil
}
...
func (r *renderer) renderStreamWrapper(svcName string, m *model.Method) {
	...
	if m.OutputIsEmpty || (m.InputIsEmpty && m.Stream != model.ServerStream) {
		panic(fmt.Sprintf("publicgo: google.protobuf.Empty stream frames are not supported (%s.%s)", svcName, m.Name))
	}
	streamType := svcName + m.Name + "Stream"
