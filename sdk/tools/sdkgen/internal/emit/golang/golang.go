// Package golang is the Go SDK emitter. It renders native types, wire-stub
// conversions, and per-service clients from the normalized model into the
// sdk/go root package.
package golang
...
func (*Emitter) Emit(schema *model.Schema) (*fileset.FileSet, error) {
	...
	for _, g := range groupFiles(services, messages, enums) {
		...
		if err := set.Add("client/"+g.base+".go", []byte(r.assemble())); err != nil {
			return nil, err
		}
	}
	return set, nil
