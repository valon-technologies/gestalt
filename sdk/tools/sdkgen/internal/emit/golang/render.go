...
func (r *renderer) renderClient(svc *model.Service) {
	...
	if svc.HostBinding != "" {
		r.renderConnect(name, svc.HostBinding)
	}
...
func (r *renderer) renderConnect(svcName, binding string) {
	...
	fmt.Fprintf(&r.body, "func Connect%s(ctx context.Context, name string) (*%s, error) {\n", svcName, svcName)
	fmt.Fprintf(&r.body, "\ttarget, token, err := host.Target(%q)\n", binding)
	r.body.WriteString("\tif err != nil {\n\t\treturn nil, toGestaltError(err)\n\t}\n")
	fmt.Fprintf(&r.body, "\tconn, err := host.DialService(ctx, %q, target, token, name)\n", binding)
	r.body.WriteString("\tif err != nil {\n\t\treturn nil, toGestaltError(err)\n\t}\n")
	fmt.Fprintf(&r.body, "\treturn New%s(conn), nil\n}\n\n", svcName)
}
...
// pointer parameters, already placed last by validation) and a collapsed
