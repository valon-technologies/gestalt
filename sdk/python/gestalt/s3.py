...
class PresignMethodValues:
    ...
    GET: PresignMethod = 1
    PUT: PresignMethod = 2
    DELETE: PresignMethod = 3
    HEAD: PresignMethod = 4
...
def read_object_chunk_result_meta(kind: ReadObjectChunkResult) -> S3ObjectMeta | None:
    """Return the ``meta`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, ReadObjectChunkMeta) else None


def read_object_chunk_result_data(kind: ReadObjectChunkResult) -> bytes | None:
    """Return the ``data`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, ReadObjectChunkData) else None
...
def write_object_request_msg_open(
    kind: WriteObjectRequestMsg,
) -> WriteObjectOpen | None:
    """Return the ``open`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WriteObjectRequestOpen) else None


def write_object_request_msg_data(kind: WriteObjectRequestMsg) -> bytes | None:
    """Return the ``data`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WriteObjectRequestData) else None
    ...
    def head_object(
        ...
        *,
        ...
        response = _support.call_unary(
            lambda: self._stub.HeadObject(
                _codec.to_wire_head_object_request(request), timeout=self._timeout
            )
        )
        ...
        *,
        ...
        _support.call_unary(
            lambda: self._stub.DeleteObject(
                _codec.to_wire_delete_object_request(request), timeout=self._timeout
            )
        )
    ...
    def list_objects(
        ...
        *,
        ...
        if request is None:
            ...
            raise ValueError("pass either request or keyword arguments, not both")
        response = _support.call_unary(
            lambda: self._stub.ListObjects(
                _codec.to_wire_list_objects_request(request), timeout=self._timeout
            )
        )
    ...
    def copy_object(
        ...
        *,
        ...
        if request is None:
            ...
            raise ValueError("pass either request or keyword arguments, not both")
        response = _support.call_unary(
            lambda: self._stub.CopyObject(
                _codec.to_wire_copy_object_request(request), timeout=self._timeout
            )
        )
        ...
        *,
        ...
        response = _support.call_unary(
            lambda: self._stub.PresignObject(
                _codec.to_wire_presign_object_request(request), timeout=self._timeout
            )
        )
