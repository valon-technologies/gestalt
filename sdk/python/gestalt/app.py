...
class ConnectionModeValues:
    ...
    UNSPECIFIED: ConnectionMode = 0
    NONE: ConnectionMode = 1
    SUBJECT: ConnectionMode = 2
...
class ProviderMetadata:
    ...
    workflow_definition_specs: list[bytes] = field(default_factory=list)
...
class AppProvider:
    ...
    def get_metadata(self) -> ProviderMetadata:
        response = _support.call_unary(
            lambda: self._stub.GetMetadata(_empty.Empty(), timeout=self._timeout)
        )
        ...
        *,
        ...
        response = _support.call_unary(
            lambda: self._stub.StartProvider(
                _codec.to_wire_start_provider_request(request), timeout=self._timeout
            )
        )
    ...
    def execute(
        ...
        *,
        ...
        if request.context is None and self._context is not None:
            request = replace(request, context=self._context)
        response = _support.call_unary(
            lambda: self._stub.Execute(
                _codec.to_wire_execute_request(request), timeout=self._timeout
            )
        )
        return _codec.from_wire_operation_result(response)
