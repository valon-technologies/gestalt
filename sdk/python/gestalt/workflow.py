...
def workflow_activation_trigger_schedule(
    kind: WorkflowActivationTrigger,
) -> WorkflowScheduleActivation | None:
    """Return the ``schedule`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowActivationSchedule) else None


def workflow_activation_trigger_event(
    kind: WorkflowActivationTrigger,
) -> WorkflowEventActivation | None:
    """Return the ``event`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowActivationEvent) else None
...
def workflow_run_trigger_kind_manual(
    kind: WorkflowRunTriggerKind,
) -> WorkflowManualTrigger | None:
    """Return the ``manual`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowRunTriggerManual) else None


def workflow_run_trigger_kind_schedule(
    kind: WorkflowRunTriggerKind,
) -> WorkflowScheduleTrigger | None:
    """Return the ``schedule`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowRunTriggerSchedule) else None


def workflow_run_trigger_kind_event(
    kind: WorkflowRunTriggerKind,
) -> WorkflowEventTriggerInvocation | None:
    """Return the ``event`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowRunTriggerEvent) else None
...
def workflow_step_action_app(kind: WorkflowStepAction) -> WorkflowStepAppCall | None:
    """Return the ``app`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowStepApp) else None


def workflow_step_action_agent(
    kind: WorkflowStepAction,
) -> WorkflowStepAgentTurn | None:
    """Return the ``agent`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowStepAgent) else None
...
def workflow_value_kind_literal(kind: WorkflowValueKind) -> JsonValue | None:
    """Return the ``literal`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowValueLiteral) else None


def workflow_value_kind_object(kind: WorkflowValueKind) -> WorkflowObject | None:
    """Return the ``object`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowValueObject) else None


def workflow_value_kind_array(kind: WorkflowValueKind) -> WorkflowArray | None:
    """Return the ``array`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowValueArray) else None


def workflow_value_kind_template(kind: WorkflowValueKind) -> WorkflowText | None:
    """Return the ``template`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowValueTemplate) else None


def workflow_value_kind_input(kind: WorkflowValueKind) -> WorkflowPathSource | None:
    """Return the ``input`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowValueInput) else None


def workflow_value_kind_signal(kind: WorkflowValueKind) -> WorkflowPathSource | None:
    """Return the ``signal`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowValueSignal) else None


def workflow_value_kind_step_output(
    kind: WorkflowValueKind,
) -> WorkflowStepOutputSource | None:
    """Return the ``step_output`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowValueStepOutput) else None


def workflow_value_kind_step_input(
    kind: WorkflowValueKind,
) -> WorkflowStepInputSource | None:
    """Return the ``step_input`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, WorkflowValueStepInput) else None
