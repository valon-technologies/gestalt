...
def agent_output_kind_text(kind: AgentOutputKind) -> AgentTextOutput | None:
    """Return the ``text`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, AgentOutputText) else None


def agent_output_kind_structured(kind: AgentOutputKind) -> AgentStructuredOutput | None:
    """Return the ``structured`` payload when ``kind`` holds that variant, else None."""
...
def agent_tool_config_source_none(kind: AgentToolConfigSource) -> AgentNoTools | None:
    """Return the ``none`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, AgentToolConfigNone) else None


def agent_tool_config_source_catalog(
    kind: AgentToolConfigSource,
) -> AgentCatalogToolConfig | None:
    """Return the ``catalog`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, AgentToolConfigCatalog) else None
...
def agent_turn_output_text(kind: AgentTurnOutput) -> AgentTurnTextOutput | None:
    """Return the ``text`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, AgentTurnText) else None


def agent_turn_output_structured(
    kind: AgentTurnOutput,
) -> AgentTurnStructuredOutput | None:
    """Return the ``structured`` payload when ``kind`` holds that variant, else None."""
    return kind.value if isinstance(kind, AgentTurnStructured) else None
