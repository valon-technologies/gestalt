...
test("Agent forwards request context across session and turn calls", async () => {
  ...
  try {
    ...
    const agent = Agent.connect(undefined, { context });

    // Sparse Init construction: unset fields read as their proto defaults.
    const session = await agent.createSession("workflow-agent", "", "gpt-test", {
    ...
    const fetchedSession = await agent.getSession("session-1");
...
test("Agent still requires host service socket configuration", () => {
  ...
  try {
    delete process.env[ENV_HOST_SERVICE_SOCKET];
    expect(() => Agent.connect()).toThrow("agent: GESTALT_HOST_SERVICE_SOCKET is not set");
  } finally {
    if (previousSocket === undefined) {
      delete process.env[ENV_HOST_SERVICE_SOCKET];
    } else {
...
test("Agent honors tcp target env and relay token env", async () => {
  ...
  try {
    ...
    const response = await agent.getSession("session-1");
