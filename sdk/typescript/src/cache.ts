...
export class Cache {
  ...
  async get(key: string): Promise<Uint8Array | undefined> {
    ...
    const response = fromWireCacheGetResponse(
      await callUnary(() => this.client.get(toWireCacheGetRequest(request))),
    );
  ...
  async getRaw(request: CacheGetRequest): Promise<CacheGetResponse> {
    const response = await callUnary(() =>
      this.client.get(toWireCacheGetRequest(request)),
    );
  ...
  async getMany(keys: string[]): Promise<{ [key: string]: Uint8Array }> {
    ...
    const response = fromWireCacheGetManyResponse(
      await callUnary(() =>
        this.client.getMany(toWireCacheGetManyRequest(request)),
      ),
    );
  ...
  async setMany(entries: CacheSetEntry[], ttl?: DurationMs): Promise<void> {
    const request: CacheSetManyRequest = {
      entries,
      ...(ttl !== undefined ? { ttl } : {}),
    };
    await callUnary(() =>
      this.client.setMany(toWireCacheSetManyRequest(request)),
    );
  }
  ...
  async delete(key: string): Promise<boolean> {
    ...
    const response = fromWireCacheDeleteResponse(
      await callUnary(() =>
        this.client.delete(toWireCacheDeleteRequest(request)),
      ),
    );
  ...
  async deleteMany(keys: string[]): Promise<bigint> {
    ...
    const response = fromWireCacheDeleteManyResponse(
      await callUnary(() =>
        this.client.deleteMany(toWireCacheDeleteManyRequest(request)),
      ),
    );
    return response.deleted;
  }
  ...
  async touch(key: string, ttl?: DurationMs): Promise<boolean> {
    const request: CacheTouchRequest = {
      key,
      ...(ttl !== undefined ? { ttl } : {}),
    };
    const response = fromWireCacheTouchResponse(
      await callUnary(() =>
        this.client.touch(toWireCacheTouchRequest(request)),
      ),
    );
