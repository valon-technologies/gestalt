...
/** An IndexedDB query argument: a single key or a key range. */
...
/** An index to add to a store that an earlier revision already created. */
export interface AddIndexDeclaration {
  store: string;
  index: IndexSchema;
}

/** A reference to an existing index to remove. */
...
/** Transaction-scoped, data-only, idempotent handle. */
...
/** Transaction-scoped, data-only view of an object store. */
...
/** Options controlling how the runner discovers and runs revisions. */
 ...
 * pending. Releases the lease and never leaves it held on error.
 */
export async function runMigrations(
  ...
  try {
    ...
    const currentHead = latestAppliedId(revisions, applied);
    for (const revision of revisions) {
      ...
      try {
        ...
        throw new MigrationError(
          ...
          { current: currentHead, attempted: attemptedHead, cause: error },
        );
      }
    }
  } finally {
...
function startLeaseRenewal(
  ...
  const timer = setInterval(() => {
    void db.acquireLock(key, holder, ttlMs).catch(() => {});
  }, interval);
...
async function releaseQuietly(
  ...
  try {
    ...
    // A lapsed lease degrades to redundant work, never corruption.
  }
}
