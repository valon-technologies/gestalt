// Code generated by sdkgen. DO NOT EDIT.

import type { JsonObject, JsonValue } from "@bufbuild/protobuf";
import { createClient } from "@connectrpc/connect";
import type { Client, Transport } from "@connectrpc/connect";

import {
  createHostServiceGrpcTransport,
  hostServiceMetadataInterceptors,
  parseHostServiceTarget,
  requireHostServiceTarget,
} from "./host-service.ts";

import * as wire from "./internal/gen/v1/app_pb.ts";
import { fromWireGetSessionCatalogResponse, fromWireOperationResult, fromWireProviderMetadata, fromWireResolveHTTPSubjectResponse, fromWireStartProviderResponse, toWireAppInvokeGraphQLRequest, toWireAppInvokeRequest, toWireExecuteRequest, toWireGetSessionCatalogRequest, toWireResolveHTTPSubjectRequest, toWireStartProviderRequest } from "./internal/codec/app.ts";
import { callUnary } from "./internal/codec/support.ts";

/**
 * ConnectionMode describes which credential sources a provider accepts.
 */
export const ConnectionMode = {
  CONNECTION_MODE_UNSPECIFIED: 0,
  CONNECTION_MODE_NONE: 1,
  CONNECTION_MODE_SUBJECT: 2,
} as const;

export type ConnectionMode = number;

/**
 * AccessContext describes the host-side access decision for an operation.
 */
export interface AccessContext {
  policy: string;
  role: string;
}

export interface AgentInvocationContext {
  providerName: string;
  sessionId: string;
  turnId: string;
}

export interface AgentToolRef {
  app: string;
  operation: string;
  connection: string;
  instance: string;
  title: string;
  description: string;
  credentialMode: string;
  system: string;
  runAs?: SubjectContext;
}

/**
 * AppInvokeGraphQLRequest invokes the raw GraphQL surface on another plugin
 * through Gestalt.
 */
export interface AppInvokeGraphQLRequest {
  app: string;
  document: string;
  variables?: JsonObject;
  connection: string;
  instance: string;
  idempotencyKey: string;
  context?: RequestContext;
}

/**
 * AppInvokeRequest invokes a declared operation on another app through Gestalt.
 */
export interface AppInvokeRequest {
  app: string;
  operation: string;
  params?: JsonObject;
  connection: string;
  instance: string;
  idempotencyKey: string;
  credentialMode: string;
  context?: RequestContext;
}

/**
 * Catalog is the static or request-scoped executable surface exposed by a
 * provider.
 */
export interface Catalog {
  name: string;
  displayName: string;
  description: string;
  iconSvg: string;
  operations: CatalogOperation[];
}

/**
 * CatalogOperation is one executable operation exposed by an integration
 * provider.
 */
export interface CatalogOperation {
  id: string;
  method: string;
  title: string;
  description: string;
  inputSchema: string;
  outputSchema: string;
  annotations?: OperationAnnotations;
  parameters: CatalogParameter[];
  requiredScopes: string[];
  tags: string[];
  readOnly: boolean;
  visible?: boolean;
  transport: string;
  allowedRoles: string[];
}

/**
 * CatalogParameter describes one input parameter surfaced in the generated
 * catalog for an operation.
 */
export interface CatalogParameter {
  name: string;
  type: string;
  description: string;
  required: boolean;
  default?: JsonValue;
}

/**
 * ConnectionParamDef describes one provider-defined connection parameter.
 */
export interface ConnectionParamDef {
  required: boolean;
  description: string;
  defaultValue: string;
  from: string;
  field: string;
}

/**
 * CredentialContext describes the resolved credential used for an operation.
 */
export interface CredentialContext {
  mode: string;
  subjectId: string;
  connection: string;
  instance: string;
}

/**
 * ExecuteRequest invokes one executable operation.
 */
export interface ExecuteRequest {
  operation: string;
  params?: JsonObject;
  token: string;
  connectionParams: { [key: string]: string };
  invocationId: string;
  context?: RequestContext;
  idempotencyKey: string;
}

/**
 * GetSessionCatalogRequest asks a provider for request-scoped catalog
 * extensions.
 */
export interface GetSessionCatalogRequest {
  token: string;
  connectionParams: { [key: string]: string };
  invocationId: string;
  context?: RequestContext;
}

/**
 * GetSessionCatalogResponse returns request-scoped catalog extensions.
 */
export interface GetSessionCatalogResponse {
  catalog?: Catalog;
}

/**
 * HTTPSubjectRequest carries one verified hosted HTTP request into an optional
 * plugin-local subject resolution hook.
 */
export interface HTTPSubjectRequest {
  binding: string;
  method: string;
  path: string;
  contentType: string;
  headers: { [key: string]: StringList };
  query: { [key: string]: StringList };
  params?: JsonObject;
  rawBody: Uint8Array;
  securityScheme: string;
  verifiedSubject: string;
  verifiedClaims: { [key: string]: string };
}

/**
 * HostContext describes stable public host metadata available to provider code.
 */
export interface HostContext {
  publicBaseUrl: string;
}

export interface InvocationContext {
  requestId: string;
  depth: number;
  callChain: string[];
  surface: string;
  internalConnectionAccess: boolean;
  connection: string;
}

/**
 * OperationAnnotations carries optional host hints about how an operation
 * behaves.
 */
export interface OperationAnnotations {
  readOnlyHint?: boolean;
  idempotentHint?: boolean;
  destructiveHint?: boolean;
  openWorldHint?: boolean;
}

/**
 * OperationResult is the serialized result returned from an Execute call.
 */
export interface OperationResult {
  status: number;
  body: Uint8Array;
  headers: { [key: string]: StringList };
}

/**
 * ProviderContext identifies the provider process that received the request
 * context from the host and is trusted to propagate it back to host services.
 */
export interface ProviderContext {
  kind: string;
  name: string;
}

/**
 * ProviderMetadata describes an integration provider's static capabilities.
 */
export interface ProviderMetadata {
  name: string;
  displayName: string;
  description: string;
  connectionMode: ConnectionMode;
  authTypes: string[];
  connectionParams: { [key: string]: ConnectionParamDef };
  staticCatalog?: Catalog;
  supportsSessionCatalog: boolean;
  minProtocolVersion: number;
  maxProtocolVersion: number;
}

/**
 * RequestContext bundles the caller, credential, access, and host metadata for
 * one operation.
 */
export interface RequestContext {
  subject?: SubjectContext;
  credential?: CredentialContext;
  access?: AccessContext;
  workflow?: JsonObject;
  host?: HostContext;
  /**
   * Original agent caller when an agent tool executes with delegated run-as identity.
   */
  agentSubject?: SubjectContext;
  caller?: ProviderContext;
  invocation?: InvocationContext;
  /**
   * Agent tool refs granted to the operation request, when the request is
   * executing as an agent tool.
   */
  toolRefs: AgentToolRef[];
  /**
   * Preserves the distinction between an omitted tool-ref context and an
   * explicitly empty inherited tool-ref context.
   */
  toolRefsSet: boolean;
  requestMeta?: RequestMetaContext;
  agent?: AgentInvocationContext;
}

export interface RequestMetaContext {
  clientIp: string;
  remoteAddr: string;
  userAgent: string;
}

/**
 * ResolveHTTPSubjectRequest asks a provider to map a verified hosted HTTP
 * request to a concrete Gestalt subject before normal operation authorization
 * and dispatch.
 */
export interface ResolveHTTPSubjectRequest {
  request?: HTTPSubjectRequest;
  context?: RequestContext;
}

/**
 * ResolveHTTPSubjectResponse returns the concrete Gestalt subject a hosted HTTP
 * request should execute as. An unset subject means "fall back to the binding
 * subject". When reject_status is set, the host should reject the inbound
 * request with the provided status and message.
 */
export interface ResolveHTTPSubjectResponse {
  subject?: SubjectContext;
  rejectStatus: number;
  rejectMessage: string;
}

/**
 * StartProviderRequest configures an integration provider for one runtime
 * session.
 */
export interface StartProviderRequest {
  name: string;
  config?: JsonObject;
  protocolVersion: number;
}

/**
 * StartProviderResponse confirms the protocol version the provider is serving.
 */
export interface StartProviderResponse {
  protocolVersion: number;
}

/**
 * StringList is a helper map value for repeated HTTP header and query values.
 */
export interface StringList {
  values: string[];
}

/**
 * SubjectContext identifies the caller that initiated an operation.
 */
export interface SubjectContext {
  id: string;
  credentialSubjectId: string;
  email: string;
  displayName: string;
  scopes: string[];
  permissions: SubjectPermissionContext[];
}

export interface SubjectPermissionContext {
  app: string;
  operations: string[];
  allOperations: boolean;
}

export class App {
  private readonly client: Client<typeof wire.App>;

  constructor(transport: Transport) {
    this.client = createClient(wire.App, transport);
  }

  static connect(name?: string): App {
    const { target, token } = requireHostServiceTarget("app");
    const transport = createHostServiceGrpcTransport(
      parseHostServiceTarget("app", target),
      hostServiceMetadataInterceptors(token, name?.trim() ?? ""),
    );
    return new App(transport);
  }

  async invoke(app: string, operation: string, connection: string, instance: string, idempotencyKey: string, credentialMode: string, params?: JsonObject): Promise<OperationResult> {
    const request: AppInvokeRequest = { app, operation, connection, instance, idempotencyKey, credentialMode, ...(params !== undefined ? { params } : {}) };
    const response = await callUnary(() => this.client.invoke(toWireAppInvokeRequest(request)));
    return fromWireOperationResult(response);
  }

  async invokeRaw(request: AppInvokeRequest): Promise<OperationResult> {
    const response = await callUnary(() => this.client.invoke(toWireAppInvokeRequest(request)));
    return fromWireOperationResult(response);
  }

  async invokeGraphQL(app: string, document: string, connection: string, instance: string, idempotencyKey: string, variables?: JsonObject): Promise<OperationResult> {
    const request: AppInvokeGraphQLRequest = { app, document, connection, instance, idempotencyKey, ...(variables !== undefined ? { variables } : {}) };
    const response = await callUnary(() => this.client.invokeGraphQL(toWireAppInvokeGraphQLRequest(request)));
    return fromWireOperationResult(response);
  }

  async invokeGraphQLRaw(request: AppInvokeGraphQLRequest): Promise<OperationResult> {
    const response = await callUnary(() => this.client.invokeGraphQL(toWireAppInvokeGraphQLRequest(request)));
    return fromWireOperationResult(response);
  }

}

/**
 * AppProvider models the shared Gestalt integration-provider protocol.
 */
export class AppProvider {
  private readonly client: Client<typeof wire.AppProvider>;

  constructor(transport: Transport) {
    this.client = createClient(wire.AppProvider, transport);
  }

  async getMetadata(): Promise<ProviderMetadata> {
    const response = await callUnary(() => this.client.getMetadata({}));
    return fromWireProviderMetadata(response);
  }

  async startProvider(name: string, protocolVersion: number, config?: JsonObject): Promise<StartProviderResponse> {
    const request: StartProviderRequest = { name, protocolVersion, ...(config !== undefined ? { config } : {}) };
    const response = await callUnary(() => this.client.startProvider(toWireStartProviderRequest(request)));
    return fromWireStartProviderResponse(response);
  }

  async startProviderRaw(request: StartProviderRequest): Promise<StartProviderResponse> {
    const response = await callUnary(() => this.client.startProvider(toWireStartProviderRequest(request)));
    return fromWireStartProviderResponse(response);
  }

  async execute(operation: string, token: string, invocationId: string, idempotencyKey: string, params?: JsonObject): Promise<OperationResult> {
    const request: ExecuteRequest = { operation, token, invocationId, idempotencyKey, connectionParams: {}, ...(params !== undefined ? { params } : {}) };
    const response = await callUnary(() => this.client.execute(toWireExecuteRequest(request)));
    return fromWireOperationResult(response);
  }

  async executeRaw(request: ExecuteRequest): Promise<OperationResult> {
    const response = await callUnary(() => this.client.execute(toWireExecuteRequest(request)));
    return fromWireOperationResult(response);
  }

  async resolveHTTPSubject(request: ResolveHTTPSubjectRequest): Promise<ResolveHTTPSubjectResponse> {
    const response = await callUnary(() => this.client.resolveHTTPSubject(toWireResolveHTTPSubjectRequest(request)));
    return fromWireResolveHTTPSubjectResponse(response);
  }

  async getSessionCatalog(token: string, invocationId: string): Promise<GetSessionCatalogResponse> {
    const request: GetSessionCatalogRequest = { token, invocationId, connectionParams: {} };
    const response = await callUnary(() => this.client.getSessionCatalog(toWireGetSessionCatalogRequest(request)));
    return fromWireGetSessionCatalogResponse(response);
  }

  async getSessionCatalogRaw(request: GetSessionCatalogRequest): Promise<GetSessionCatalogResponse> {
    const response = await callUnary(() => this.client.getSessionCatalog(toWireGetSessionCatalogRequest(request)));
    return fromWireGetSessionCatalogResponse(response);
  }

}
