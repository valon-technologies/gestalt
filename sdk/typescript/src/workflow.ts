...
export type WorkflowValueKind =
  ...
  | { case: "workflowContext"; value: string }
...
export interface WorkflowStepPluginCall {
...
export type WorkflowStepActionKind =
  | { case: "plugin"; value: WorkflowStepPluginCall }
...
export function workflowValue(input: WorkflowValue = {}): WorkflowValue {
  ...
  const selected = [
    ...
    input.signalMetadata === undefined ? undefined : "signalMetadata",
    input.workflowContext === undefined ? undefined : "workflowContext",
  ...
  switch (selected[0]) {
    ...
    case "signalMetadata":
      return { kind: { case: "signalMetadata", value: input.signalMetadata ?? "" } };
    case "workflowContext":
      return { kind: { case: "workflowContext", value: input.workflowContext ?? "" } };
    default:
      return {
        kind: {
          case: "stepOutput",
...
export function workflowValueInputFromValue(
  ...
  switch (kind?.case) {
    ...
    case "workflowContext":
      return { workflowContext: kind.value };
    case "stepOutput":
...
/** Creates a workflow plugin step call from native input. */
...
/** Returns native input copied from a workflow plugin step call. */
export function workflowStepPluginCallInputFromCall(
  input?: WorkflowStepPluginCall,
): WorkflowStepPluginCall | undefined {
  if (input === undefined) {
    return undefined;
  }
  return {
...
export function workflowStep(input: WorkflowStep = {}): WorkflowStep {
  const plugin = input.plugin ?? (input.action?.case === "plugin" ? input.action.value : undefined);
  const agent = input.agent ?? (input.action?.case === "agent" ? input.action.value : undefined);
  if (plugin !== undefined && agent !== undefined) {
    throw new Error("workflow step must set either plugin or agent");
  }
  const action = plugin !== undefined
    ? { case: "plugin" as const, value: workflowStepPluginCall(plugin) }
    : agent !== undefined
  ...
  return {
    ...
    plugin: action.case === "plugin" ? action.value : undefined,
...
export function workflowStepInputFromStep(
  ...
  const plugin = input.plugin ?? (action?.case === "plugin" ? action.value : undefined);
  ...
  return {
    ...
    plugin: workflowStepPluginCallInputFromCall(plugin),
    agent: workflowStepAgentTurnInputFromTurn(agent),
    action: action === undefined
      ? undefined
      : action.case === "plugin"
        ? { case: "plugin", value: workflowStepPluginCallInputFromCall(action.value)! }
        : action.case === "agent"
...
export function workflowValueFromProto(
  ...
  switch (input.kind.case) {
    ...
    case "signalMetadata":
      return { kind: { case: "signalMetadata", value: input.kind.value.path } };
    case "workflowContext":
      return { kind: { case: "workflowContext", value: input.kind.value.path } };
    case "stepOutput":
...
export function workflowStepPluginCallToProto(
  input?: WorkflowStepPluginCall | undefined,
): ProtoWorkflowStepPluginCall | undefined {
  if (input === undefined) {
    return undefined;
  }
  return create(WorkflowStepPluginCallSchema, {
    name: input.name ?? "",
    operation: input.operation ?? "",
...
export function workflowStepPluginCallFromProto(
  input?: ProtoWorkflowStepPluginCall | undefined,
): WorkflowStepPluginCall | undefined {
  if (input === undefined) {
    return undefined;
  }
...
export function workflowStepToProto(input: WorkflowStep): ProtoWorkflowStep {
  ...
  return create(WorkflowStepSchema, {
    ...
    action: action?.case === "plugin"
      ? { case: "plugin", value: workflowStepPluginCallToProto(action.value)! }
      : action?.case === "agent"
...
export function workflowStepFromProto(input: ProtoWorkflowStep): WorkflowStep {
  const action = input.action.case === "plugin"
    ? { case: "plugin" as const, value: workflowStepPluginCallFromProto(input.action.value)! }
