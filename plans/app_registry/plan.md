...
12. Registry-only app config and conditional bootstrap — declare an app slot in `config.yaml` without a git SHA; skip lock/sync baking for that app; start at boot when a fleet-known version exists in IndexedDB, otherwise wait for the first registry install. See [config.md](./config.md#registry-only-app-source), [lifecycle.md](./lifecycle.md#registry-only-apps).
