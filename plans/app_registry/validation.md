...
| Workflow target app | Candidate `DeclaredWorkflowDefinitions` app-call steps vs deploy `config.apps` | `workflow_target_app_missing` |
| Workflow target operation | App-call step `operation` vs target published `interface` | `workflow_target_operation_missing`, `workflow_target_operation_metadata_missing` |
...
| `workflow_target_app_missing` | **400** | none | Workflow app step `name` is not a deploy config app slot |
| `workflow_target_operation_missing` | **400** | none | Target published `interface` does not expose the workflow app step `operation` |
| `workflow_target_operation_metadata_missing` | **400** | none | Operation check needed but fleet-known target's `versions/{version}.json` is missing |
...
Published app artifacts can declare workflow definitions. At startup, gestaltd reads them from the started provider as `DeclaredWorkflowDefinitions` and registers them with the workflow manager. Config-managed `workflows.definitions` entries are validated during config load; packaged app workflow declarations are not, so `InstallValidator` must validate them before `AppendRequest`.

Workflow definitions are not part of the `versions/{version}.json` contract (`interface`, `requires`, `compatibility` only). Install validation loads them from the candidate artifact or from workflow metadata copied into the registry entry at publish time.

For each workflow definition declared by the candidate:

1. Walk every app-call step (`steps[].app.name`, `steps[].app.operation`).
2. Require `steps[].app.name` to match a deploy config app slot (`config.apps` key on the handling replica).
3. When the target app is fleet-known or deploy-pinned, require `steps[].app.operation` to exist on that target's published `interface`.

Self-targets are allowed when the step names the candidate app (for example, `g-issues` calling `g-issues.handle_slack_event`).

### Metadata fetch scope

| Check | Uses deploy config `config.apps` | Fetches `versions/{version}.json` |
|-------|----------------------------------|-------------------------------------|
| Workflow target app | yes | no |
| Workflow target operation | yes | yes, when the target is fleet-known — missing → `workflow_target_operation_metadata_missing` |

Operation metadata is not fetched when the workflow step omits `operation` or when the target app is deploy-pinned with no fleet-known version.
...
| Workflow target app | Installing `g-issues` whose packaged `slack_v2_smoke_test` workflow targets `gIssues` while deploy config only defines `g-issues`. |
| Workflow target operation | Installing an app whose workflow calls `slack.postMessage` while fleet-known `slack` no longer publishes that operation in `interface`. |
