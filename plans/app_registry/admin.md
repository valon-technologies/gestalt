...
1. **Handler placement** — Add `handlers_admin_app_rollout.go` and register routes in `mountAdminAppInstallReadRoutes` (or a new `mountAdminAppRolloutRoutes` group).
2. **List projection** — `registry-apps` list is a server-side join of deploy config + IndexedDB; keep it out of the poller hot path.
3. **Cohort math** — Reuse the same enrollment cutoff rules as `poller.updateRolloutOutcome` so the UI matches rollout completion.
4. **UI assets** — Prefer small vanilla JS modules embedded via `go:embed` to match the current admin shell; avoid a separate frontend build until the surface grows.
5. **Multi-replica E2E** — Update [tests.md](./tests.md#multi-replica-materialization-ack-e2e) to poll `GET …/materializations` instead of IndexedDB.
