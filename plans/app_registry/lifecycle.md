...
At boot, `StartAppProviders` skips registry-only apps with no fleet-known version in `app_version_change_requests`. When IndexedDB has a known version, bootstrap materializes from `{artifactsDir}/registry-installed/{app}/{version}` if needed and starts the provider — the same mount path used by catalog-driven restarts. The catalog poller handles first install and upgrades for replicas that are not yet converged.
