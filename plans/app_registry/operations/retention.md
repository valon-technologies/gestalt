...
`app_version_change_requests` is the source of truth for fleet deploy history and historical redeploy deadlines. `retention.json` remains authoritative for `publishedAt`, unused retention, and sticky `lockedAt` after prune. Deployment-state projection and admission ignore stale or missing `deployableUntil` rows in `retention.json` for deployed versions.
