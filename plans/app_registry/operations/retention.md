...
Admission checks the selected version while holding the app-scoped install lock. A historical version is eligible only when it is not locked and the request arrives before `deployableUntil`. The deadline comes from `from_version_deployable_until` on the change-request chain — the same value written when the outgoing version was deactivated. Selecting it appends another change request and makes it desired; when it later stops being desired, it receives a new deadline based on the configured `deployedRetention`.

**Source of truth split:** `app_version_change_requests` is authoritative for fleet deploy history and historical redeploy deadlines. `retention.json` is authoritative for `publishedAt`, unused retention, and sticky `lockedAt` after prune. Reader-side deployability projection and admission checks use the change-request chain for deployed versions; stale or missing `deployableUntil` rows in `retention.json` do not affect UI state or selection.
