...
In-flight and recently failed app registry publishes on `/apps/{app}/admin` before `index.json` is updated at the end of CI.

## Overview

`gestaltd app registry publish` updates `index.json` last, after packaging and artifact upload. Without pending catalogs, operators saw nothing on `/apps/{app}/admin` during a long CI run.

Mutable `pending.json` and `failed.json` per app record publish state in GCS. `gestaltd serve` reads both catalogs over HTTP and exposes them on `GET /api/v1/apps/{app}/admin/registry`. The `gestalt-providers` app admin UI merges pending, failed, and published rows in **Published snapshots**.

Scope is `/apps/{app}/admin` only — the embedded fleet `/admin` registry UI does not show publishing state.
...
Pending and failed versions are **not** installable. Install handlers read only published `versions/{version}.json` entries via `index.json`.
...
`pending.json` and `failed.json` are mutable catalogs. Updates use optimistic concurrency: read generation, merge, upload with `if-generation-match`.

Document shapes: [PendingIndex](../architecture/models.md#pendingindex), [FailedIndex](../architecture/models.md#failedindex).
...
Durations are computed at read/UI time — not stored as separate GCS fields. Field definitions: [`publishStartedAt`](../architecture/models.md#publishedversion).
...
`gestaltd serve` cannot write GCS. Prune helpers run on the **`gestaltd app registry pending` write path** (CI).

`gestaltd app registry pending set` always:

1. Removes `failed.{version}` for the version being upserted (workflow retry).
2. Runs `PrunePendingIndex` (skips the target version; stale → `failed.json`; already-published → drop from pending).
3. Runs `PruneFailedIndex` (drop entries older than **30 days** or already published).
4. Upserts `pending.{version}` with `phase=publishing`.

`pending clear` and `pending fail` are idempotent when the version is absent from pending. `pending fail` does not overwrite an existing `failed.json` entry.
...
HTTP GET `apps/{app}/pending.json` and `apps/{app}/failed.json`. Missing object → empty catalog.

### App-Admin API

`GET /api/v1/apps/{app}/admin/registry` includes `pendingVersions[]` and `failedVersions[]` alongside `publishedVersions[]`.

Merge rules:

- Each version appears in at most one of `pendingVersions`, `failedVersions`, and `publishedVersions`.
...
├── <a href="../architecture/models.md#pendingindex">models.md</a> — PendingIndex and <a href="../architecture/models.md#failedindex">FailedIndex</a>
├── <a href="./admin.md">admin.md</a> — app admin UI capabilities
├── <a href="./lifecycle.md">lifecycle.md</a> — app-admin HTTP API
└── <a href="../project/tests.md#pending-catalog-write-path">tests.md</a> — write-path and admin tests
</pre>
