...
- For deployed versions, deployment-state projection reads `deployableUntil` from `from_version_deployable_until` on the change-request chain. `retention.json` supplies `publishedAt`, unused retention, and sticky `lockedAt` only.
